...
static dsite_t *MasterSiteList = NULL;
static dsite_t **NextSite = &MasterSiteList;
...
static void WriteSiteRectList(Display *dpy, Window win, Atom prop)
{
	...
	/* hier stand bis zum 2.5.2017 sizeof(int), was in der 64-Bit - Welt
	 * zu kurz ist
	 */
	array = (unsigned long *)malloc(8*MAX(numrects,1) * sizeof(unsigned long));
...
static Bool convertTarget(Display *dpy, Window requestor, Atom target, Atom property)
{
	...
	if (target == AtomTargets) {
		...
		data[nelements++] = AtomSunDragDropSiteRects;
		data[nelements++] = AtomUser;
		data[nelements++] = AtomTopLevels;
		type = XA_ATOM;
		format = 32;
	}
	else if (target == AtomTimestamp) {
		data[nelements++] = selectionTime;
		type = XA_INTEGER;
		format = 32;
	}
	else if (target == AtomTopLevels) {
		...
		for (cli = ListEnum(&l); cli != NULL; cli = ListEnum(&l)) {
			if (cli->framewin && cli->framewin->fcore.panewin &&
				cli->framewin->fcore.panewin->core.kind == WIN_PANE)
			{
				data[nelements++] = PANEWINOFCLIENT(cli);
			}
		}
		/* a NULL-termination */
		data[nelements++] = 0L;
...
static void handleDSDMrequest(XEvent *event)
{
	...
	if (selectionOwned) {
		if (request->target == AtomMultiple) {
			...
			dra_olwm_trace(300, "rcv selection request for %ld\n", request->target);
