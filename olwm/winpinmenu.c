 ...
 */
static int eventButtonPress(Display *dpy, XEvent *event, WinPinMenu *winInfo)
{
    if (! StartMenuGrabs(dpy, winInfo)) return 1;
	...
	MenuInfo 	*origMenuInfo;
{
	...
	*newMenuInfo = *origMenuInfo;
	...
	atomList[0] = (Atom)15;            /* bitmask */
	atomList[1] = AtomWTOther;         /* win_type */
	atomList[2] = AtomMenuLimited;     /* menu_type */
	atomList[3] = (Atom)PIN_IN;        /* pin_state */
	atomList[4] = (Atom)0;             /* cancel */
	XChangeProperty(dpy, win, AtomWinAttr, AtomWinAttr,
		32, PropModeReplace, (unsigned char *)atomList, 5);
