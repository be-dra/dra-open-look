 ...
 */
static void drawIconBorder(Display *dpy, WinIconFrame *winInfo, Bool select)
{
	 ...
	 */
	if (select) {
		XDrawRectangle(dpy, w, borderGC, x, y, width, height);
		XDrawRectangle(dpy, w, borderGC, x + 1, y + 1, width - 2, height - 2);
		XDrawRectangle(dpy, w, borderGC, x + 2, y + 2, width - 4, height - 4);
	}
	else {
		if (GRV.ui_style != UIS_2D_BW) {
			XDrawRectangle(dpy, w, workspaceGC, x, y, width, height);
			XDrawRectangle(dpy, w, workspaceGC, x + 1, y+1, width-2, height-2);
			XDrawRectangle(dpy, w, workspaceGC, x + 2, y+2, width-4, height-4);
		}
		else {
			XDrawRectangle(dpy, w, workspaceGC, x, y, width, height);
...
static int drawIcon(dpy, winInfo)
Display       *dpy;
WinIconFrame *winInfo;
{
