...
static void *dra_collect_move_windows(Client *cli, List **winlistp)
{
	if (cli->wmState == NormalState && cli->framewin) {
		/* the idea is: don't move notices with a notice_emanation.
		 * Those notices can be moved
		 *  · individually
		 *  · as an element in a selection set
		 *  · as a member of a group-managed 
		 *
		 * Ref (jhvrefvhgerg)
		 */
		if (cli->wmDecors->notice_emanation == None) {
			*winlistp = ListCons(cli->framewin, *winlistp);
		}
	}
	return NULL;
}
...
void UserMoveWindows(Client *cli, XEvent *first)
{
	...
	if (first == NULL || first->type == KeyPress) {
		timestamp = (first == NULL) ? LastEventTime : first->xkey.time;
	...
	if (IsSelected(cli)) {
		...
		int sel_cnt = 0;
		Client *dra_cli = (Client *) 0;

		while (dra_cli = EnumSelections(dra_cli)) ++sel_cnt;

		if (sel_cnt == 1 &&
				cli->groupmask == GROUP_LEADER &&
				(cli->protocols & GROUP_MANAGED) != 0) {
			winlist = ListCons(mstuff.frame, NULL_LIST);
			GroupApply(cli->groupid, dra_collect_move_windows, &winlist,
					GROUP_DEPENDENT);
		}
		else {
		...
		/* can we accept (later) an empty mstuff.winlist ??
		 *
		 * Yes, we can: the notice will not be moved
		 */
	...
	if (XGrabKeyboard(dpy, cli->scrInfo->rootid, False,
					GrabModeAsync, GrabModeAsync, timestamp) != GrabSuccess) {
		/* note: not fatal */
...
static void moveUpdate(MoveClosure *mstuff)
{
	if (!mstuff->dragwin)
		(void)ListApply(mstuff->winlist, drawOneBox, mstuff);
...
static void moveKeyDelta(MoveClosure *mstuff, int dx, int dy)
{
	mstuff->constraint = Unconstrained;
    ...
    MoveClosure *mstuff;
{
	...
	{
	    mstuff->constraint = Unconstrained;
		mstuff->curX = event->xkey.x_root;
		mstuff->curY = event->xkey.y_root;
	    moveUpdate(mstuff);
	}
	break;
...
static void * moveOneWindow(WinGenericFrame *win, MoveClosure *mstuff)
{
	...
	XMoveWindow(mstuff->dpy, win->core.self,
			win->core.x + offX, win->core.y + offY);
	if (win == mstuff->frame)
 ...
 */
static void * drawOneBox(w, mstuff)
    WinGenericFrame *w;
    MoveClosure *mstuff;
{
    int offX, offY;
    ...
    MoveClosure *mstuff;
{
    ...
    XUngrabPointer(mstuff->dpy, LastEventTime);
    XUngrabKeyboard(mstuff->dpy, LastEventTime);
