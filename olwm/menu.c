...
#ifndef ButtonMark_Pos
#define olgx_draw_accel_button(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p) \
	olgx_draw_button(a, b, c, d, e, f, g, p)
#endif

#ifndef ButtonSpace_Width
#  define ButtonSpace_Width(info)         ((info)->mm_width)
#endif

#ifndef MainLabel_Pos
#  define MainLabel_Pos(info, x) (ButtonEndcap_Width(info) + x)
#endif /* MainLabel_Pos */
...
#define MI_BUTTONINDENT(gis) (TextScrollButton_Width(gis) + 2 - ButtonEndcap_Width(gis))
		...
		for (i = 0; i < mInfo->menu->buttonCount; i++) {
			...
			if (pb->has_submenu) {
				if (strcmp(pb->label[pb->which], DefPtr->Name) == 0) {
					...
					if (DefPtr->DefaultButton <
								mInfo->buttons[i].subMenu->menu->buttonCount)
						mInfo->buttons[i].subMenu->menu->buttonDefault =
											DefPtr->DefaultButton;
					return 1;
				}
			}
		}
    ...
    MenuInfo *mInfo;
{
	...
	for (bindex = 0; bindex < menu->buttonCount; bindex++) {
		...
		if (!bi->button->visible
							|| bi->button->label[bi->button->which] ==
							NULL) continue;
    ...
    MenuInfo *mInfo;
{
	...
	for (bindex = 0; bindex < menu->buttonCount; bindex++) {
		...
		if (XRectInRegion(region, bi->buttonX, bi->buttonY,
					mInfo->maxbuttonWidth, bi->buttonHeight + HEAD_VSPACE))
			bi->flDirty = True;
	}
}
...
void DrawMenu(Display *dpy, MenuInfo *mInfo)
{
	...
	if (mInfo->origmenuInfo != NULL || (GRV.ui_style != UIS_3D_COLOR)) {
		XFillRectangle(dpy, win, windowGC, 0, 0,
							mInfo->menuWidth, mInfo->menuHeight);
	}
	if (mInfo->origmenuInfo == NULL) {
		XDrawRectangle(dpy, win, gisNormal->gc_rec[OLGX_BLACK]->gc,
					0, 0, mInfo->menuWidth-1, mInfo->menuHeight-1);
	}
	/* Draw the menu title. */
	if ((mtit = menuTitle(mInfo)) != NULL) {
		...
		olgx_draw_text_ledge(gisNormal, win,
							...
							mInfo->menuWidth - (MENU_TITLE_MARGIN * 2));
	}
	/* Draw the buttons. */
	for (bindex = 0; bindex < menu->buttonCount; bindex++) {
		ButtonInfo *bi = &mInfo->buttons[bindex];
    ...
    WinGeneric *winInfo;
{
	...
	grabstat = XGrabPointer(dpy, WinRootID(winInfo),
						...
						None, GRV.MenuPointer, LastEventTime);
	...
	grabstat = XGrabKeyboard(dpy, WinRootID(winInfo),
						False, GrabModeAsync, GrabModeAsync, LastEventTime);
    ...
    Bool	 flbutton;
{
	...
	dra_setup_menu_color(menuInfo->menuWin, winInfo->core.client);
	...
	switch (pevent->type) {
		...
		case ButtonRelease:
			...
			flDoSetDefault =
					(pevent->xbutton.state & ModMaskMap[MOD_SETDEFAULT]);
			break;
		...
		case KeyRelease:
			...
			flDoSetDefault = (FindModifierMask(pevent->xkey.keycode) ==
								ModMaskMap[MOD_SETDEFAULT]);
			break;
		default:
			return;
	}
	...
	if (GRV.menuClickExecutesDefault && !flbutton) {
		menuTrackMode = MODE_DRAG;
	}
	showMenu(dpy, menuInfo, x, y, !flbutton, flbutton);
}
    ...
    MenuInfo *mi;
{
	...
	for (i = 0; i < mi->menu->buttonCount; i++, bi++) {
		...
		bi->buttonX = MI_BUTTONINDENT(gisButton);
		bi->buttonY = nextY;
    ...
    Menu       *menu;
{
	...
	for (i = 0; i < menu->buttonCount; i++) {
		...
		if (pb->label[pb->which] == NULL) {
			...
			maxLabWidth =
								MAX(maxLabWidth,
								buttonTextWidth(pb->label[pb->which]));
		...
		if (bi->has_accel && menu->wantAccelerators) {
			...
			maxAccelModWidth =
								MAX(maxAccelModWidth,
								buttonTextWidth(bi->accel_modifier));
			maxAccelKeyWidth =
								MAX(maxAccelKeyWidth,
								buttonTextWidth(bi->accel_key));
		}
	}
	...
	if (menuInfo->hasAccelerators) {
		menuInfo->labelPos =MainLabel_Pos(gisButton,MI_BUTTONINDENT(gisButton));
		menuInfo->buttonMarkPos = ButtonMark_Pos(gisButton, menuInfo->labelPos,
		...
		maxLabWidth = menuInfo->accKeyPos + maxAccelKeyWidth -
							menuInfo->labelPos;
	}
	else if (hasSubMenu) {
	...
	if (menu->title != NULL) {
		...
		menuInfo->titleHeight = HEAD_VSPACE +
							...
							PUSHPINHEIGHT(gisNormal)) + HEAD_VSPACE + 4;

		if (menu->hasPushPin) {
			menuInfo->titleX = MI_BUTTONINDENT(gisButton) +
			...
			menWidth = 2 * MI_BUTTONINDENT(gisButton) +
							MAX(maxLabWidth,
								PushPinOut_Width(gisNormal) + BUTT_HSPACE +
								menuInfo->titleWidth);
		}
		...
		menWidth = 2 * MI_BUTTONINDENT(gisButton) + maxLabWidth;
	 ...
	 */
	menHeight = menuInfo->titleHeight + HEAD_VSPACE + heightofbuttons +
						HEAD_VSPACE;
	 ...
	 */
	menuInfo->pushPinX = MI_BUTTONINDENT(gisButton);
    ...
    ScreenInfo *scrInfo;
{
	...
	if (maxDepth > scrInfo->menuCache->maxDepth) {
		scrInfo->menuCache->menuWinList =
							MemRealloc(scrInfo->menuCache->menuWinList,
							maxDepth * sizeof(struct _winmenu *));

		for (i = scrInfo->menuCache->maxDepth; i < maxDepth; i++) {
			scrInfo->menuCache->menuWinList[i] =
								MakeMenu(dpy, scrInfo->rootwin);
		}
    ...
    Bool	frombutton;
{
	...
	if (GRV.menuClickExecutesDefault && !frombutton) {
		WinGeneric *winInfo = menuInfo->menuWin;
		Graphics_info *gi = WinGI(winInfo, NORMAL_GINFO);

		x -= (PushPinOut_Width(gi) >> 1);
		if (x < 0)
			x = 0;
	...
	if ((x + menuInfo->menuWidth + MENU_SHADOW_OFFSET) > dpyWidth)
		x = dpyWidth - menuInfo->menuWidth - MENU_SHADOW_OFFSET;

	if ((y + menuInfo->menuHeight + MENU_SHADOW_OFFSET) > dpyHeight)
		y = dpyHeight - menuInfo->menuHeight - MENU_SHADOW_OFFSET;
    ...
    int	new;
{
	...
	if (mInfo->menu->client) {
		XChangeProperty(dpy, PANEWINOFCLIENT(mInfo->menu->client),
							AtomWinMenuDefault, XA_INTEGER, 32,
			...
			if (lastPress.type == KeyPress) {
				...
				if (isEnabled(mInfo, mInfo->litButton)) {
					...
					if (mInfo->litButton == mInfo->ringedButton)
						mInfo->ringedButton = NOBUTTON;
					SetButton(dpy, mInfo, mInfo->litButton, True,
										flDoSetDefault);
				}
			}
			lastPress = *pevent;
			...
			if (BUTTON_INDEX_OK(mInfo, bindex)) {
				if (mInfo->menu->buttons[bindex]->has_submenu) {
					...
					if ((deltaX > GRV.DragRightDistance) ||
						(curX > (menuX + mInfo->buttons[bindex].buttonX
									+ mInfo->maxbuttonWidth
									- ButtonEndcap_Width(gisButton)
									- MenuMark_Width(gisNormal))))
					{
						(void)activateSubMenu(dpy, mInfo, bindex,
											pevent->xmotion.x_root);
						minX = curX;
					}
				}
			}
    ...
    MenuInfo   *mInfo;
{
	...
	switch (status) {
		...
		case ACTION_RIGHT:
			...
			if (BUTTON_INDEX_OK(mInfo, bindex)
								&& mInfo->menu->buttons[bindex]->has_submenu ==
								1) {
				int menuat;
				...
				if (mInfo->menuWin->core.kind == WIN_MENU)
					...
					WinGenericFrame *parent =
										...
										(WinGenericFrame *) mInfo->menuWin->
										core.parent;
					assert(parent);
					menuat =
										parent->core.x + parent->core.width -
										LOC_CURSOR_SIZE;
				}
    ...
    XEvent     *event;
{
	...
	switch (event->type) {
		...
		case Expose:
			...
			break;
		default:
			fputs(GetString("olwm: wrong event type passed to menuSearch\n"),
								stderr);
			return (MenuInfo *) 0;
	}
	...
	return (event->type ==
						Expose) ? (MenuInfo *) 0 : menuInfoTable[topMenu - 1];
}
    ...
    int        *bindex;
{
	...
	if (!PointInRect(rx, ry, menuDim.x, menuDim.y, menuDim.width,
											menuDim.height)) return ML_OFFMENU;
	...
	for (i = 0; i < menuInfo->menu->buttonCount; i++) {
		...
		if (PointInRect(ex, ey,
												...
												menuInfo->buttons[i].buttonY +
												yoff, menuInfo->maxbuttonWidth,
												menuInfo->buttons[i].
												buttonHeight)) {
			/*
    ...
    int         flsetdefault;
{
	...
	if (mInfo->pinIn != state) {
		...
		if (((mInfo->menu->buttonDefault == PINBUTTON) && !state)
							|| flsetdefault) olgx_flags =
								OLGX_DEFAULT | OLGX_PUSHPIN_OUT;
		else
			olgx_flags = (state ? OLGX_PUSHPIN_IN : OLGX_PUSHPIN_OUT);

		olgx_draw_pushpin(gisNormal, win,
							mInfo->pushPinX, mInfo->pushPinY, olgx_flags);
	}
}
    ...
    int         x;
{
	...
	if (!mInfo->childActive && BUTTON_INDEX_OK(mInfo, bindex)
		...
		dra_setup_menu_color(subMenu->menuWin, mInfo->menuWin->core.client);
    ...
    Bool         fldraw;
{
	...
	y +=
						(mInfo->buttons[bindex].buttonY +
						(mInfo->buttons[bindex].buttonHeight / 2));
    ...
    Bool	fInverse;
{
	...
	if (!pb->enabled)
		state |= OLGX_INACTIVE;
	else /*don't let default buttons have rings */ if (fDefault)
		state |= OLGX_DEFAULT;
	...
	if (menuInfo->hasAccelerators) {
		...
		olgx_draw_accel_button(WinGI(winInfo, BUTTON_GINFO),
							winInfo->core.self,
							bi->buttonX, bi->buttonY + menuInfo->buttonOffset,
							menuInfo->maxbuttonWidth, 0,
							pb->label[pb->which],
							MainLabel_Pos(WinGI(winInfo, BUTTON_GINFO),
												bi->buttonX),
							bi->accel_modifier, menuInfo->accModPos, marktype,
							menuInfo->buttonMarkPos, bi->accel_key,
							menuInfo->accKeyPos, NULL, state | TextOLGX);
	}
	else {
		if (pb->has_submenu)
...
isClick(e1, e2)
    ...
    XEvent     *e2;
{
	return (ABS(eventX(e1) - eventX(e2)) <= GRV.ClickMoveThreshold &&
						ABS(eventY(e1) - eventY(e2)) <= GRV.ClickMoveThreshold
						&& eventTime(e2) - eventTime(e1) <=
						GRV.DoubleClickTime);
}
...
/* barray: array of button pointers */
{
	...
	if (new != NULL) {
		...
		memcpy((void *)new->buttons, (void *)barray,
							ctbuttons * sizeof(Button **));
	}
	return new;
}
    ...
    ScreenInfo *scrInfo;
{
	...
	menuCache->menuInfoList =
						(MenuInfo **) MemAlloc(sizeof(MenuInfo) *
						menuCache->maxSlots);
    ...
    WinGenericFrame	*win;
{
	...
	while (1) {
		...
		if (menu->buttonDefault < 0 || menu->buttonDefault >= menu->buttonCount) {
			return True;
		}
