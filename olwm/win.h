/* @(#) win.h V1.5 96/06/10 06:16:14 $Id: win.h,v 2.2 2025/01/11 19:51:15 dra Exp $ */
...
typedef struct {
	...
	/* this is an optional override-redirect window that is supposed to
	 * be always BEHIND the notice window
	 */
	Window notice_emanation;
} WMDecorations;
...
extern Graphics_info *dra_WinGI(WinGeneric *w, int idx);
extern GC dra_WinGC(WinGeneric *w, int idx);
#  define WinGI(w,idx)	dra_WinGI((WinGeneric *)w,idx)
#  define WinGC(w,idx)	dra_WinGC((WinGeneric *)w,idx)
...
typedef struct _winiconpane {
	...
	Pixmap		iconMask;
	int         iconPixmapDepth; /* the ICCCM said (still says???) depth 1 */
} WinIconPane;
