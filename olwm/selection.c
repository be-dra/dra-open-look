    ...
    Atom property;
{
    ...
    if (target == AtomTargets) {
		...
		dra_olwm_trace(300, "in processPrimaryTarget, TIMESTAMP\n");
		...
		dra_olwm_trace(300, "in processPrimaryTarget, DRAWABLE\n");
		...
		dra_olwm_trace(300, "in processPrimaryTarget, NAME\n");
		...
		dra_olwm_trace(300, "in processPrimaryTarget, CLIENT_WINDOW\n");
		data[0] = NoFocusWin;
		nelements = 1;
		type = XA_WINDOW;
		format = 32;
...
static void handlePrimary(XEvent *event)
{
	...
	dra_olwm_trace(350, "in handlePrimary, target = %d\n", request->target);
...
void AddSelection(Client *cli, Time timestamp)
{
	...
	if (selectList == NULL_LIST)
	{
		 ...
		 */
		dra_olwm_trace(350, "owning PRIMARY\n");
		XSetSelectionOwner(cli->dpy, XA_PRIMARY, NoFocusWin, timestamp);
		if (NoFocusWin != XGetSelectionOwner(cli->dpy, XA_PRIMARY)) {
			dra_olwm_trace(350, "... failed\n");
		    return;
		}
	} else {
 ...
 */
void SelectionRegister(Atom selection, SelectionHandler handler)
{
...
void SelectionResponse(XEvent *event)
{
	...
	dra_olwm_trace(380, "in SelectionResponse for selection %d\n", selection);
	...
	if (event->type == SelectionRequest) {
		...
		dra_olwm_trace(310, "selection %d is unregistered\n", selection);
