...
static int yposCloseButton(Client *cli, Graphics_info *gis)
{
	if (Abbrev_MenuButton_Height(gis) < headerHeight(cli, gis))
		return (ResizeArm_Height(gis) + (headerHeight(cli,
								gis) - Abbrev_MenuButton_Height(gis)) / 2);
	else
...
#define Win3D(w) (GRV.ui_style == UIS_3D_COLOR)
...
static void makeResizeCorners(Display *dpy, WinPaneFrame *wf, int wid, int high)
{
	wf->resizeCorner[upleft] = MakeResize(dpy, wf, upleft, 0, 0);
...
void CreateResizeCorners(Display *dpy, WinPaneFrame *wf)
{
	makeResizeCorners(dpy, wf, wf->core.width, wf->core.height);
}
...
int wid,high;
{
	...
	if (cli->wmDecors->flags & WMDecorationResizeable) {
		makeResizeCorners(dpy, wf, wid, high);
	}
...
Time	timestamp;
{
	...
	if (frameInfo->fcore.fullsize) {	/* going to restore/normal */
		...
		if ((client->normHints) &&
			...
			/* however, idiots like firefox come with the max size
			 * (16384, 16384).... we restrict them to a little less than
			 * the screen size.
			 */
			if (width > 10000)
				width = DisplayWidth(client->dpy,client->screen) - 150;
			if (height > 10000) 
				height = DisplayHeight(client->dpy,client->screen) - 100;
		}
 ...
 * devices.
 */
void FrameAllowEvents(Client *cli, Time time)
{
	if (!GRV.FocusFollowsMouse) {
		switch (cli->focusMode) {
			case Passive:
...
void FrameInit(Display *dpy)
{
	...
	classPaneFrame.core.xevents[EnterNotify] = (EvFunc)eventEnterNotify;
	classPaneFrame.core.xevents[LeaveNotify] = (EvFunc)eventLeaveNotify;
	classPaneFrame.core.xevents[FocusIn] = GFrameEventFocus;
	classPaneFrame.core.xevents[FocusOut] = GFrameEventFocus;
	classPaneFrame.core.xevents[SelectionRequest] =
											(EvFunc)dra_quick_handle_selection;
	classPaneFrame.core.xevents[SelectionClear] =
											(EvFunc)dra_quick_handle_selection;
    ...
    int x, y;
{
    ...
    x -= dx;
    y -= dy;
	/* hier bekommen die einschlägigen Idioten Koordinaten (-5,-25):
...
void FrameUnparentPane(Client *cli, WinPaneFrame *winFrame, WinPane *winPane)
{
	...
	if (winPane->core.kind != WIN_MENU) {
		...
		if (GRV.RestackWhenWithdraw) {
			XConfigureWindow(cli->dpy, winPane->core.self, xwc_mask, &xwc);
		}
	}
}
...
void FrameSetBusy(WinPaneFrame *win, Bool newBusy)
{
	if (newBusy) {
		...
		(WinFunc(win->winBusy, core.destroyfunc)) (win->core.client->dpy,
				win->winBusy);
	}
	WinCallDraw(win);
}
...
void FrameWarpPointer(Client *cli)
{
	...
	/* hier stand bis zum 2.5.2017 "int *warpParam", was in der 64-Bit - Welt
	 * zu kurz ist, denn ein Property vom Format 32 wird absurderweise
	 * auf long bzw unsigned long abgebildet
	 */
	long *warpParam;
	...
	if (cli->protocols & IS_WS_PROPS)
		dra_olws_started(cli->dpy, -1);

	if (!GRV.PopupJumpCursor)
		return;

	if (cli->protocols & NO_WARPING)
		return;
	 ...
	 */
	if (cli->protocols & WARP_TO_PIN) {
		warpParam = NULL;
	}
	else {
		if (warpParam==NULL && !(cli->wmDecors->flags & WMDecorationWarpToPin))
		{
			return;
		}
	}
	...
	/* warp the pointer */
	XWarpPointer(cli->dpy, None, dest,
			0, 0, 0, 0, cli->warpInfo.warpToX, cli->warpInfo.warpToY);

	frameInfo->pointerIsWarped = True;

	/* finally, set the focus if necessary */
	/* The OL Spec (implicitely) assumes that a popup window that is popped
	 * up (and therefore the pointer is warped) always gets the input
	 * focus (or, at least is offered the input focus).
	 *
	 * I think this should be independent of the AutoInputFocus setting.
	 */
...
void FrameUnwarpPointer(Client *cli)
{
	...
	if (frameInfo->pointerIsWarped) {
		...
		if (cli->protocols & WARP_BACK) {
			XWarpPointer(cli->dpy, None, cli->scrInfo->rootid,
					0, 0, 0, 0,
...
void FrameUpdateShape(Client *cli, WinPaneFrame *winInfo)
{
	...
	if (cli->isShaped) {
		XShapeCombineShape(cli->dpy, winInfo->core.self, ShapeBounding,
				widthLeftFrame(winInfo), heightTopFrame(winInfo),
				winInfo->fcore.panewin->core.self, ShapeBounding, ShapeSet);
		if (cli->isSelected && (cli->wmDecors->flags &WMDecorationResizeable)) {
			XRectangle rects[8];
