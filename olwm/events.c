...
static int explicitPointerGrab = False;
...
static void nextEventOrTimeout(dpy, event)
	Display *dpy;
	XEvent *event;
{
	...
	while (XPending(dpy) == 0 && ready <= 0) {
		int pip = dra_get_pipe(), selval;
	    ...
	    polltime.tv_usec = 0;
	    FD_SET(pip, &rdset);
	    selval = select(MAX(fd, pip)+1,&rdset,&wrset,&xset,&polltime);
		switch (selval) {
			case -1: 
			...
			case 0:
				break;
			default:
				if (FD_ISSET(pip, &rdset)) dra_dispatch_signal(dpy);
				if (FD_ISSET(fd, &rdset) || FD_ISSET(fd, &xset)) {
					ready = 1;
				}
				break;
		}
    ...
    Display *dpy;
{
	...
	for (;;) {
		 ...
		 */
		/* I have forgotten why I wanted to allow synthetic mouse events... */
		if (event.xany.send_event) {
			if (event.type <= KeyRelease && event.type >= KeyPress) continue;
			if (event.type == ButtonPress) {
				dra_olwm_trace(234, "seen a synthetic ButtonPress\n");
			}
		}
		...
		if (event.type == ClientMessage &&
			event.xclient.message_type == AtomPropagateEvent &&
			event.xclient.format == 32)
		{
			/* This ClientMessage is sent by clients that want to have
			 * the behaviour of their control areas as described in the
			 * OpenLook spec : the background of control areas show the 
			 * Window menu etc
			 */
			XEvent		remapped;
			WinGeneric *ww;
			unsigned long ul = event.xclient.data.l[0];

			remapped = event;
			remapped.type = ((ul >> 24) & 0xff);
			remapped.xbutton.state = (ul & 0xffff);

			/* xmotion and xbutton have (almost) equal structures
			 * except for is_hint/button:
			 */
			if (remapped.type == MotionNotify) {
				remapped.xmotion.is_hint = ((ul >> 16) & 0xff);
			}
			else {
				remapped.xbutton.button = ((ul >> 16) & 0xff);
			}

			remapped.xbutton.send_event = 0;
			ul = event.xclient.data.l[1];
			remapped.xbutton.x = ((ul >> 16) & 0xffff);
			remapped.xbutton.y = (ul & 0xffff);
			remapped.xbutton.time = event.xclient.data.l[2];
			remapped.xbutton.subwindow = 0;
			remapped.xbutton.same_screen = 1;

		
			/* now, this event is sent on the PANE, but we need it
			 * on the frame:
			 */
			ww = lookupWindow(&remapped);
			if (ww) {
				WinGeneric *parent = ww->core.parent;

				if (parent) {
					remapped.xbutton.x_root = remapped.xbutton.x
											+ parent->core.x
											+ ww->core.x;
					remapped.xbutton.y_root = remapped.xbutton.y
											+ parent->core.y
											+ ww->core.y;

					if (remapped.type == ButtonPress) {
						/* Ref (lkhrgdbtvybdrtvge) */
						/* otherwise 'window moving' does not work as expected.
						 * The corresponding XUngrabPointer is in wingframe.c
						 */
						XGrabPointer(dpy, parent->core.self, False,
		    					ButtonPressMask
								| ButtonMotionMask
								| ButtonReleaseMask,
		    					GrabModeAsync, GrabModeAsync, None, None,
		    					remapped.xbutton.time);
					}
					else if (remapped.type == ButtonRelease) {
						/* not sure: is it possible that a mouse click in
						 * such a control area is so quick that the
						 * ButtonRelease is still seen by the client (before
						 * our mouse grab is in effect) ?
						 */
						XUngrabPointer(dpy, remapped.xbutton.time);
					}
				}
				else {
					remapped.xbutton.x_root = remapped.xbutton.x + ww->core.x;
					remapped.xbutton.y_root = remapped.xbutton.y + ww->core.y;
				}
				/*
				 ...
				 */
				if ( interposer != NULL)
					dispatchInterposer(dpy, &remapped);
				else
					dispatchEvent(dpy, &remapped, parent);
			}
		}
		else if (event.type == ClientMessage &&
			event.xclient.message_type == AtomMenuFull &&
			event.xclient.format == 32)
		{
			WinGeneric *ww = lookupWindow(&event);
			Atom func = (Atom)event.xclient.data.l[0];

			if (func == AtomMenuFunctionClose) {
				WindowOpenCloseAction(dpy, ww, NULL, 0);
			}
			else if (func == AtomMenuFunctionFullsize) {
				WindowFullRestoreSizeAction(dpy, ww, NULL, 0);
			}
			else if (func == AtomMenuFunctionProps) {
				WindowPropsAction(dpy, ww, NULL, 0);
			}
			else if (func == AtomMenuFunctionBack) {
				WindowBackAction(dpy, ww, NULL, 0);
			}
			else if (func == AtomMenuFunctionRefresh) {
				WindowRefreshAction(dpy, ww, NULL, 0);
			}
			else if (func == AtomMenuFunctionQuit) {
				WindowQuitAction(dpy, ww, NULL, 0);
			}
		}
		else {
			/*
