...
WinGeneric *winInfo;
{
	 ...
	 */
	while (XCheckTypedWindowEvent(dpy, winInfo->core.self, KeyPress, &dummy))
		/*EMPTY*/;
	while (XCheckTypedWindowEvent(dpy, winInfo->core.self, KeyRelease, &dummy))
		/*EMPTY*/;
	while (XCheckTypedWindowEvent(dpy, winInfo->core.self, ButtonPress, &dummy))
		/*EMPTY*/;
...
Time TimeFresh(void)
{
				   ...
				   NoFocusWin, PropertyNotify, &e))
	{
		...
		timestamp = CurrentTime;   /* should not happen... */
