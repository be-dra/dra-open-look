...
char win_c_sccsid[] = "@(#) %M% V%I% %E% %U% $Id: win.c,v 2.1 2024/09/20 19:59:01 dra Exp $";
 ...
 */
Bool WinShowHelp(dpy,win,mousex,mousey)
Display		*dpy;
WinGeneric	*win;
int		mousex,mousey;
{
	...
	if (win->core.helpstring) {
		if (win->core.client) {
			if (ClientShowHelp(win->core.client, mousex, mousey,
									win->core.helpstring))
				return True;
