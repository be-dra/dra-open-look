...
#ifdef linux
extern int LogoutFunc();
extern int ShutdownSystemFunc();
extern int RebootSystemFunc();
#endif /* linux */
extern int RestartWindowSystemFunc();
...
static void SetWindowMenuLabels();
static void SetWindowMenuTitle();

void WindowMenuCreate(dpy)
    Display    *dpy;
{
    SetWindowMenuLabels();
    SetWindowMenuTitle();
...
static char *alternate_menu_file = (char *)0;

void dra_alternate_menu_file(dpy, nam)
	Display *dpy;
	char *nam;
{
    GRV.AutoReReadMenuFile = False;
	if (alternate_menu_file) XFree(alternate_menu_file);
	alternate_menu_file = nam;

	ReInitUserMenu(dpy, True);
}

void dra_reread_user_menu(dpy)
	Display *dpy;
{
	if (alternate_menu_file) XFree(alternate_menu_file);
	alternate_menu_file = (char *)0;
	...
	for (; fgets(line, sizeof(line), stream); (*lineno)++) {
		...
		if (strcmp(prog, "END") == 0) {
			...
			if (currentMenu->menulabel != NULL &&
				...
				fprintf(stderr,
						GetString
						("olwm: menu label mismatch '%s' <> '%s' in file %s, line %d\n"),
						label, currentMenu->menulabel, filename, *lineno);
				return (MENU_FATAL);
			}
		...
		if (strcmp(prog, "XDG_MENU") == 0) {
			/* we act as if the line contained
			 * '"Label" INCLUDE $HOME/.xview/XDGmenu'
			 */
			strcpy(prog, "INCLUDE");
			strcpy(args, "$HOME/.xview/XDGmenu");
		}
		...
		while (!done) {
			switch (lookupToken(prog, &(currentButton->func))) {
				...
				case PinToken:
					 ...
					 */
					fputs(GetString
							("menu title and END required before PIN keyword.\n"),
							stderr);
					return (MENU_FATAL);
					/* NOTREACHED */
					break;
...
PRINT_SCREEN        --- NOT YET IMPLEMENTED
START_DSDM
STOP_DSDM
*/
...
#ifdef linux
    {	"LOGOUT",	    LogoutFunc,	    ServiceToken	},
    {	"SHUTDOWN_SYSTEM",	    ShutdownSystemFunc,	    ServiceToken	},
    {	"REBOOT_SYSTEM",	    RebootSystemFunc,	    ServiceToken	},
#endif /* linux */
    {	"RESTART_WINDOW_SYSTEM",    RestartWindowSystemFunc, ServiceToken	},
    {	"REFRESH",	    RefreshFunc,	    ServiceToken	},
    {	"CLIPBOARD",	    ClipboardFunc,	    ServiceToken	},
    {	"PRINT_SCREEN",	    PrintScreenFunc,	    ServiceToken	},
    {	"EXIT",	            (FuncPtr)ExitFunc,		    ServiceToken	},
    {	"EXIT_NO_CONFIRM",  (FuncPtr)ExitNoConfirmFunc,	    ServiceToken	},
    {	"WMEXIT",	    (FuncPtr)ExitOLWM,		    ServiceToken	},
    {	"PROPERTIES",	    PropertiesFunc,	    ServiceToken	},
    {	"NOP",		    NopFunc,		    ServiceToken	},
    {	"DEFAULT",	    NULL,		    DefaultToken	},
    {	"MENU",		    NULL,		    MenuToken		},
    {	"XDG_MENU",		    NULL,		    MenuToken		},
    {	"END",		    NULL,		    EndToken		},
    {	"PIN",		    NULL,		    PinToken		},
...
extern void ReplaceChars(char *buff, char *sstr, char rc);
 ...
 * we go along.
 */
static Menu * buildFromSpec(Display *dpy, menudata *pmenu, char *deftitle)
{
	Menu *m;
	Button *b;
	int ii;
...
void SetWindowMenuLabels()
{
	...
	moveButton.visible = ! GRV.pureOpenLookWindowMenus;
	resizeButton.visible = ! GRV.pureOpenLookWindowMenus;
    ...
    Bool flnotitle;
{
	...
	switch (decor->menu_type) {
		case MENU_FULL:
			...
			ToggleEnabled(menu, basewin_propsitem,
						  ((cli->protocols & HAS_PROPWIN) != 0));
    ...
    void *data;
{
	...
	MenuShowSync(win->core.client->dpy, win, sdi.menu, eve, setFrameDefault,
						&sdi, (eve->type == KeyPress)
						|| (eve->type == KeyRelease), flbutton);
}
