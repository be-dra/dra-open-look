...
static Bool convertWM_Starget(dpy, requestor, target, property)
    Display *dpy;
    Window requestor;
    Atom target;
    Atom property;
{
    unsigned long data[100];	/* long enough for most return values */
    unsigned char *propdata = (unsigned char *) data;
    int format, nelements = 0;
    Atom type;

	if (target == AtomTargets) {
		data[nelements++] = AtomTargets;
		data[nelements++] = AtomTimestamp;
		data[nelements++] = AtomMultiple;
		data[nelements++] = AtomVersion;
		data[nelements++] = AtomUser;
		/* we do not tell the SHUTDOWN and REBOOT atoms... */
		type = XA_ATOM;
		format = 32;
	}
	...
	else if (target == AtomUser) {
		struct passwd *ppw;

		ppw = getpwuid(getuid());
		if (ppw) {
			strncpy((char *)data, ppw->pw_name, sizeof(data)-1);
			nelements = (int)strlen((char *)data);
			type = XA_STRING;
			format = 8;
		}
		else return False;
	}
	else if (target == AtomVersion) {
		data[0] = 2;
		data[1] = 0;
		nelements = 2;
		type = XA_INTEGER;
    ...
    if (propdata != (unsigned char *) data) MemFree(propdata);
    return True;
}
 ...
 * handleWM_Srequest
 *
 * Respond to a SelectionRequest or SelectionClear event on the WM_S 
 * selection.
 */
static void handleWM_Srequest(XEvent *event)
{
	...
	if (request->target == AtomMultiple) {
		if (request->property != None) {
			...
			if (pairs != NULL) {
				...
				for (i = 0; i + 1 < nitems; i += 2) {
					...
					if (!convertWM_Starget(request->display,
									request->requestor, pairs[i],
									pairs[i + 1])) {
						pairs[i + 1] = None;
						writeback = True;
					}
		...
		if (convertWM_Starget(request->display, request->requestor,
...
#endif /* __STDC__ */
{
	...
	if (scrInfo->iscolor &&
		...
		scrInfo->colorInfo.workspaceColor = scrInfo->colorInfo.white;
...
static void setScreenWorkspaceBitmap(Display *dpy, ScreenInfo *scrInfo)
{
	...
	if (! scrInfo->iscolor && scrInfo->depth > 8) {
		XColor bgcol;

		if (XParseColor(dpy, scrInfo->colormap, GRV.blackNWhiteWorkspaceColor,
									&bgcol))
		{
			if (XAllocColor(dpy, scrInfo->colormap, &bgcol)) {
				XSetWindowBackground(dpy,scrInfo->rootid, bgcol.pixel);
				return;
			}
		}
	}
	...
	ScreenInfo	*scrInfo;
{
	...
	if (GRV.ui_style != UIS_2D_BW
		&& (scrInfo->visual->class == TrueColor
			|| scrInfo->visual->class == DirectColor))
	{
		scrInfo->pixmap[MENU_SHADOW_BITMAP] = (Pixmap)0;
	}
	else {
		...
		scrInfo->pixmap[MENU_SHADOW_BITMAP] = XCreateBitmapFromData(dpy,
				scrInfo->rootid,
				((scrInfo->depth >= 2)?(char *)gray50_bits:(char *)gray75_bits),
 ...
 */
static void initGCs(dpy,scrInfo)
	Display		*dpy;
	ScreenInfo	*scrInfo;
{
	XGCValues values;
	...
	scrInfo->gc[BUSY_GC] = XCreateGC(dpy,
			scrInfo->pixmap[PROTO_DRAWABLE],
			(GCFunction | GCForeground | GCGraphicsExposures |
					GCStipple | GCFillStyle), &values);

#ifdef BUSY_ICONS_DID_NOT_WORK
	/* 
	 * Create a GC for busy icons
	 */
	values.function = GXcopy;
	values.foreground = scrInfo->colorInfo.fgColor;
	values.fill_style = FillStippled;
	...
	values.graphics_exposures = False;
	values.subwindow_mode = IncludeInferiors;
	scrInfo->gc[ICON_BUSY_GC] = XCreateGC(dpy,
			scrInfo->pixmap[PROTO_DRAWABLE],
			(GCFunction | GCForeground | GCGraphicsExposures |
					GCStipple | GCFillStyle | GCSubwindowMode), &values);
	...
	/*
	 * Create a GC for the drawing of menu shadows
	 */
	if (GRV.ui_style != UIS_2D_BW
			&& (scrInfo->visual->class == TrueColor
					|| scrInfo->visual->class == DirectColor)) {
		Visual *vis = scrInfo->visual;
		...
		/* that '0x95' is not at all a secret - it is just the result
		 * of many tryouts
		 */
		values.foreground =
				(vis->red_mask & (rf * 0x95 * lowbit(vis->red_mask))) +
				(vis->green_mask & (gf * 0x95 * lowbit(vis->green_mask))) +
				(vis->blue_mask & (bf * 0x95 * lowbit(vis->blue_mask)));

		values.function = GXand;
		scrInfo->gc[MENU_SHADOW_GC] = XCreateGC(dpy,
				scrInfo->pixmap[PROTO_DRAWABLE],
				GCForeground | GCFunction, &values);
	}
	else {
		values.fill_style = FillStippled;
		values.foreground = BlackPixel(dpy, scrInfo->screen);
		values.stipple = scrInfo->pixmap[MENU_SHADOW_BITMAP];
		scrInfo->gc[MENU_SHADOW_GC] = XCreateGC(dpy,
				scrInfo->pixmap[PROTO_DRAWABLE],
...
static void updateScreenWindowColor(Display *dpy, ScreenInfo	*scrInfo)
{
	...
	olgx_set_single_color(scrInfo->gi[REVPIN_GINFO],
			OLGX_BG3,scrInfo->colorInfo.bg3Color,OLGX_SPECIAL);
	dra_update_window_color(scrInfo);
}
...
static void updateScreenForegroundColor(Display *dpy, ScreenInfo	*scrInfo)
{
	...
	dra_update_foreground_color(scrInfo);
}
	...
	Display		*dpy;
{
	...
	{
		extern void dra_sel_init(Display *, Window, Time);
		Atom atom;
		char selname[20];
		...
		if (GRV.SingleScreen) {
			...
			atom = XInternAtom(dpy, selname, False);
			XSetSelectionOwner(dpy, atom, NoFocusWin, selectionTime);
    		SelectionRegister(atom, handleWM_Srequest);
		}
		else {
			for (scr=0; scr<ScreenCount(dpy); scr++) {
				sprintf(selname, "WM_S%d", scr);
...
fprintf(stderr, "si = %p\n", si);
fprintf(stderr, "si->rootwin = %p\n", si->rootwin);
		/* hier sterben wir manchmal: */
		(*(WinFunc(si->rootwin, core.destroyfunc)))(dpy, si->rootwin);
	}
	XSetInputFocus(dpy, PointerRoot, RevertToPointerRoot, LastEventTime);
}
 ...
 */
int ReparentScreens(Display *dpy)
{
	ScreenInfo	*si;
	List		*l = ScreenInfoList;
    ...
    XSetWindowAttributes	*attr;
{
	...
	if (scrInfo->winCacheCount > 0) {
		...
		win = XCreateWindow(scrInfo->dpy, parent, x, y, w, h, 0,
				scrInfo->depth, InputOutput, scrInfo->visual, attrmask, attr);
	}
	assert(win != 0);
	return win;
}
 ...
 * GRV.WindowCacheSize.
 */
void ScreenUpdateWinCacheSize(dpy)
    Display		*dpy;
{
	List *l = ScreenInfoList;
	ScreenInfo *si;
