 ...
 */
void ClientUpdateWindowState(cli,event)
	Client		*cli;
...
/* ClientShowProps - send a ClientMessage of type _OL_SHOW_PROPS
 */
...
void	*junk;
{
	...
	if (! (cli->flags & CLOlwmOwned)) {
		/* to avoid BadWindow errors, just kill the primary base window */
		if (cli->protocols & SECONDARY_BASE) return NULL;
		XKillClient(cli->dpy,ClientPane(cli));
	}
	return NULL;
}
...
Client *cli;
{
	...
	if (cli->client_colors) dra_free_client_info(dpy, cli->client_colors);
	...
	if (setfocus)
	    ClientFocusTopmost(dpy, scrInfo, LastEventTime);
}
...
Client *ClientRaiseTransients(Client *cli)
{
	...
	while ((tc = ListEnum(&l))) {
		if (tc->transientFor == PANEWINOFCLIENT(cli)) {
			if (tc->wmDecors) {
				if (tc->wmDecors->notice_emanation != None) {
					XRaiseWindow(tc->dpy, tc->wmDecors->notice_emanation);
				}
			}
...
/* static Client *firstTransient(Client *cli) */
/* { */
/* 	List *l = ActiveClientList; */
/* 	Client *tc; */
/*  */
...
void dra_all_recolored_clients(pproc, extra)
    void (*pproc)();
    void *extra;
{
	List	*l = ActiveClientList;
	...
	for (cli = ListEnum(&l); cli != NULL; cli = ListEnum(&l)) {
		if (cli->groupmask == GROUP_LEADER && cli->client_colors) {
			(*pproc)(cli, extra);
		}
	}
}

static void *dra_to_front(Client *cli, int unused)
{
	if (cli->framewin) {
		if (cli->wmDecors) {
			if (cli->wmDecors->notice_emanation != None) {
				XRaiseWindow(cli->dpy, cli->wmDecors->notice_emanation);
			}
		}
...
static void *dra_to_back(Client *cli, int unused)
{
	if (cli->framewin) {
		XLowerWindow(cli->dpy, cli->framewin->core.self);
		if (cli->wmDecors) {
			if (cli->wmDecors->notice_emanation != None) {
				XLowerWindow(cli->dpy, cli->wmDecors->notice_emanation);
			}
		}
	}
	return NULL;
...
void ClientFront(Client *cli)
{
	...
	case NormalState:
		...
		if (cli->groupmask == GROUP_LEADER &&
			(cli->protocols & GROUP_MANAGED) != 0)
		{
			/* then the group members */
    		GroupApply(cli->groupid, dra_to_front, 0,
								GROUP_INDEPENDENT|GROUP_DEPENDENT);
		}

		/* at last the transients */
		firsttransient = ClientRaiseTransients(cli);
		if (firsttransient != NULL) {
		    xwc.sibling = firsttransient->framewin->core.self;
			if (firsttransient->wmDecors) {
				if (firsttransient->wmDecors->notice_emanation != None) {
					xwc.sibling = firsttransient->wmDecors->notice_emanation;
				}
			}
	...
	switch (cli->wmState) {
		case NormalState:
			if (cli->transientFor != 0 && GRV.KeepTransientsAbove) {
				...
				if (owner == NULL || owner->core.kind != WIN_PANE) {
					XLowerWindow(cli->dpy, cli->framewin->core.self);
				}
				else {
				...
				if (cli->groupmask == GROUP_LEADER &&
						(cli->protocols & GROUP_MANAGED) != 0) {
					GroupApply(cli->groupid, dra_to_back, 0,
							GROUP_INDEPENDENT | GROUP_DEPENDENT);
				}
				XLowerWindow(cli->dpy, cli->framewin->core.self);
			}
 ...
 */
static void ClientUpdateBusy(Client *cli, XPropertyEvent *event)
{
	long *newBusyPtr;
	int newBusy;
...
void ClientActivate(Display *dpy, Client *cli, Time time)
{
	...
	if (cli == NULL) {
		...
		if (GRV.RaiseOnActivate) ClientFront(cli);
...
static void dra_new_menu_filename(Client *cli, XPropertyEvent	*event)
{
	char *name;
	Atom type;
	int format;
	...
	if (event->state != PropertyNewValue) return;

	dra_olwm_trace(100, "olwm: reading alternate menu file...\n");

	if (XGetWindowProperty(event->display, event->window, event->atom,
					0, 10000, True, event->atom, &type, &format,
					&len, &rest, (unsigned char **)&name) == Success)
	{
		/* the workspace props appl might run somewhere and provide
		 * a file name I cannot access
		 */
		if (!access(name, R_OK)) {
			dra_alternate_menu_file(event->display, name);
			XChangeProperty(event->display,
						cli->scrInfo->rootid,
						AtomMenuFileName, AtomMenuFileName, 8, PropModeReplace,
						(unsigned char *)name,
						strlen(name)+1);
		}
	}
...
static void dra_new_menu_default(cli, event)
	Client		*cli;
	XPropertyEvent	*event;
{
	unsigned long nitems;
	unsigned long bytes_after;
	long *def = (long *)GetWindowProperty(event->display, event->window, 
									event->atom, 0, 1, XA_INTEGER, 32,
									&nitems, &bytes_after);
	if (def) {
		cli->wmDecors->def_item = *def;
...
static Atom AtomWMName = XA_WM_NAME;
static Atom AtomWMIconName = XA_WM_ICON_NAME;
static Atom AtomWMHints = XA_WM_HINTS;
static Atom AtomWMNormalHints = XA_WM_NORMAL_HINTS;

static ClientPropUpdate propUpdateTable[] =  {
	{ &AtomMenuFileName,		dra_new_menu_filename },
	{ &AtomSetWinMenuDefault,	dra_new_menu_default },
