...
/*
 * Send Quit to the window under the pointer.
 */
void KeyQuitPointer(Display *dpy, XKeyEvent *ke)
{
    Window child;
    WinGeneric *childinfo;
    Client *cli;

    if (ke->type != KeyPress) return;
 ...
 */
void KeyProperties(dpy, ke)
    Display *dpy;
    XKeyEvent *ke;
{
    if (CurrentClient->framewin != NULL) ClientShowProps(CurrentClient);
}

static void move_pointer(ev, dx, dy)
	XKeyEvent *ev;
	int dx;
	int dy;
{
	...
	if (ev->time > lasttime && ev->time - lasttime < 400) ++accel_cnt;
	else accel_cnt = 0;
	lasttime = ev->time;
	if (accel_cnt > 10) { dx <<= 3; dy <<= 3; }
	newx = ev->x_root + dx;
	newy = ev->y_root + dy;
	if (newx < 0 || newy < 0) return;
	XWarpPointer(ev->display, None, ev->root, 0, 0, 0, 0, newx, newy);
}

void dra_pointer_up(dpy, ev)
	Display *dpy;
	XKeyEvent *ev;
{
	move_pointer(ev, 0, -1);
