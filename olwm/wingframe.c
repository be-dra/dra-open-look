...
char wingframe_c_sccsid[] = "@(#) %M% V%I% %E% %U% $Id: wingframe.c,v 2.4 2025/01/19 14:30:58 dra Exp $";
 ...
 *
 */
int GFrameEventButtonRelease(Display *dpy, XEvent *event,
							WinGenericFrame *frameInfo)
{
	WinGenericPane *winPane = (WinGenericPane *) frameInfo->fcore.panewin;
	Client *cli = frameInfo->core.client;

	if (!AllButtonsUp(event)) return 0;

#define bevent  (event->xbutton)

	/* very careful not to be captured in a mouse grab, see
	 * Ref (lkhrgdbtvybdrtvge)
	 */
	XUngrabPointer(dpy, event->xbutton.time);

	switch (currentAction) {
		case ACTION_SELECT:
			if (dra_quick_duplicate_select(dpy, event, frameInfo)) return 0;
			...
			if ((WinFunc(frameInfo, fcore.selectDoubleClick) != NULL) &&
								((winPane == NULL)
													|| (bevent.subwindow !=
																		 ...
																		 core.
																		 self))
								&& (bevent.time - lastSelectRelease.time) <=
								GRV.DoubleClickTime
								&& (bevent.time - lastDoubleClickTime) >
								GRV.DoubleClickTime
								&& bevent.window == lastSelectRelease.window
								&& ABS(bevent.x - lastSelectRelease.x) <
								GRV.MoveThreshold
								&& ABS(bevent.y - lastSelectRelease.y) <
								GRV.MoveThreshold) {
		...
		case ACTION_ADJUST:
			if (dra_quick_duplicate_adjust(dpy, event, frameInfo)) return 0;
 ...
 */
int GFrameEventMotionNotify(Display *dpy, XEvent *event, WinGenericFrame *frameInfo)
{
	if (dra_quick_duplicate_update(dpy, event, frameInfo)) return 0;
	...
	if ((ABS(event->xmotion.x - buttonPressX) < GRV.MoveThreshold) &&
						(ABS(event->xmotion.y - buttonPressY) <
											GRV.MoveThreshold)) return 0;
 ...
 */
int GFrameEventButtonPress(Display *dpy, XEvent *event,
								 WinGenericFrame *frameInfo)
{
	...
	ignoremask = ModMaskMap[MOD_CONSTRAIN] |
						ModMaskMap[MOD_QUICKDUPL] |
						ModMaskMap[MOD_INVERT] | ModMaskMap[MOD_REDUCE];
	...
	switch (a) {
		case ACTION_SELECT:
				...
				(WinFunc(frameInfo, fcore.selectPress) != NULL))
			{
				 ...
				 */
				XAllowEvents(dpy, ReplayPointer, LastEventTime);
