...
char winpane_c_sccsid[] = "@(#) %M% V%I% %E% %U% $Id: winpane.c,v 2.3 2025/01/04 16:05:22 dra Exp $";
...
static int eventClientMessage(Display *dpy, XEvent *event, WinPane *winInfo)
{
	...
	else if (event->xclient.message_type == AtomRereadMenuFile) {
		dra_reread_user_menu(dpy);
	}
	else if (event->xclient.message_type == AtomDraTrace) {
		dra_olwm_set_traces(event->xclient.data.l);
	}
	else if (event->xclient.message_type == Atom_NET_WM_STATE) {
		/* ignore */
	}
	else if (event->xclient.message_type ==
					XInternAtom(dpy, "_XWAYLAND_MAY_GRAB_KEYBOARD", False))
	{
		/* wayland? - not with me: ignore */
		/* kommt anscheinend von vncviewer:
		 * data.l[0] = 1, alle anderen 0
		 */
	}
	else if (event->xclient.message_type == 0)
	{
		/* libreoffice calc sendet diesen Bloedsinn.
		 * Mein Gott, sind da draussen Idioten...
		 *
		 * data.l[0] = 0x1 (1)
		 * data.l[1] = vermutlich ein timestamp
		 * die anderen sind 0
		 */
		fprintf(stderr, "received idiotic ClientMessage on a pane window:\n");
		fprintf(stderr, "\tmessage_type = 0\n");
	}
	else {
		char *atnam;
		int i;
		...
		fprintf(stderr, "received ClientMessage on a pane window:\n");
		...
		if (event->xclient.format == 8) {
			...
			for (i = 0; i < 5; i++) {
				fprintf(stderr, "\t\tdata.l[%d] = 0x%lx (%;d)\n",i, p[i], p[i]);
			}
		}
		fprintf(stderr, "\n");
	}
 ...
 */
static int eventExtension(dpy, event, winInfo)
    Display	*dpy;
    XEvent	*event;
    WinPane	*winInfo;
{
 ...
 * event.
 */
static int newconfigPane(win, pxcre)
	WinPane *win;
	XConfigureRequestEvent *pxcre;
{
	...
	if (pxcre->value_mask & (CWStackMode | CWSibling)) {
		GFrameSetStack(winFrame, pxcre->value_mask, pxcre->detail,
				pxcre->above);
	}
...
XWindowAttributes *paneattrs;
{
	...
	/* Reparent the pane */
	XReparentWindow(cli->dpy, win, par->core.self, w->core.x, w->core.y);
