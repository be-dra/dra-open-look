...
char slots_c_sccsid[] = "@(#) %M% V%I% %E% %U% $Id: slots.c,v 2.1 2024/09/20 19:59:01 dra Exp $";
...
#define ICON_GRID_WIDTH GRV.GridSpacing
#define ICON_GRID_HEIGHT GRV.GridSpacing
	...
	IconGrid *g;
{
	...
	for (ima = 0; ima<g->maslots; ima++)
	    for (imi = 0; imi<g->mislots; imi++)
		if (OCCUPANCY(g,ima,imi)!=0)
		    fprintf(stderr,"%x: occ[%d,%d]/[%d] = %d\n",AOCCUPANCY(g,ima,imi),ima,imi,SLOTOFFSET(g,ima,imi),OCCUPANCY(g,ima,imi));
...
int ma, mi, nmajor, nminor, *pminrgn, *pmaxrgn;
{
	...
	for (ii=ma; ii<ma+nmajor; ii++) {
		for (jj = mi; jj<mi+nminor; jj++) {
			...
			if (occ > max) {
				max = occ;
				/* strange: without this trace call, on linux
				 * the icon positioning has a slight (but very strange)
				 * misbehaviour: empty icon places are not found, if
				 * the next place to the right is taken !!
				 */
 				dra_olwm_trace(456, "\tmax %d at (ma = %d, mi = %d)\n",
 									occ, ii, jj);
			}
		}
	}
	...
	for (;;)	/* will actually only iterate twice */
	{
	    ...
	    for (ma = 0; (ma <= iconGrid->maslots-nmajor); ma++)
	    {
	        for (mi = 0; (mi <= iconGrid->mislots-nminor); mi++)
			{
				...
				if (minrgn < minfound) minfound = minrgn;
				if (maxrgn < minfoundrgn) minfoundrgn = maxrgn;
				if (maxrgn <= minoccrgn) {
					slot->ma = ma;
					slot->mi = mi;
					slot->maw = nmajor;
...
Bool snaptogrid;
{
	...
	if (positioned) {
		...
		if (snaptogrid) setIconPos(winicon,slot);
		else (WinFunc(winicon,core.newposfunc))(winicon,
										winicon->core.x,winicon->core.y);
	}
    else
    {
