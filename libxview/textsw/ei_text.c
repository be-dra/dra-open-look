...
static void ei_plain_text_set_tab_width(Ei_handle eih, register int tab_width)
{
...
*/
	...
	if (private->x_font_info->per_char) {
		...
		private->tab_pixel = private->x_font_info->min_bounds.width * tab_width;
	}
	if (private->tab_pixel == 0)
		private->tab_pixel = 1;
}

static int ei_plain_text_set_tab_widths(Ei_handle eih, Attr_attribute *widths, int adjust_for_font)
/*
...
static caddr_t ei_plain_text_get(Ei_handle eih, int attr)
{
	Ei_attribute attribute = (Ei_attribute) attr;
	register Eipt_handle private = ABS_TO_REP(eih);
 ...
 */
{
	...
	result.break_reason = EI_PR_BUF_EMPTIED;
	result.considered = 0;
	result.bounds.r_height = 0;
	result.bounds.r_width = 0;
	result.bounds.r_top = 0;
	result.bounds.r_left = 0;
	result.pos.y = 0;
	result.pos.x = 0;
	if (!in_buf || !out_buf)
		return result;
	...
	for (cp = in_buf, op = out_buf;
			...
			result.last_plus_one < out_buf_len; cp++, esbuf->first++) {
...
/*	    XFontStruct		*x_font_info;*/
			...
			process_result =
					ei_plain_text_process(eih, (int)EI_OP_MEASURE, &process_esbuf, x,
					rect->r_top, PIX_SRC, (Xv_Window) 0, rect, tab_origin);
			switch (process_result.break_reason) {
				...
				default:
					...
					if (result.last_plus_one >= (out_buf_len - spaces_in_tab)) {
