#ifndef lint
char     ev_attr_c_sccsid[] = "@(#)ev_attr.c 20.27 93/06/28 DRA: $Id: ev_attr.c,v 4.2 2024/09/15 16:18:16 dra Exp $";
#endif
/*
...
static Ev_status ev_set_internal(Ev_handle view, Ev_chain chain, Attr_avlist attrs);
						...
						Xv_opaque args1, Xv_opaque args2, Xv_opaque args3)
{
	...
	switch (attribute) {
		...
		case EV_NO_REPAINT_TIL_EVENT:{
				return ((Xv_opaque) (private->
								state & EV_VS_NO_REPAINT_TIL_EVENT));
			}
		case EV_RECT:{
...
static void ev_set_rect(Ev_handle view, struct rect *rect, struct rect *intersect_rect);
...
static Ev_status ev_set_internal(Ev_handle view, Ev_chain chain, Attr_avlist attrs)
{
	...
	switch ((Ev_attribute) (*attrs)) {
	  ...
	  case EV_CHAIN_NOTIFY_PROC:{
		chain_private->notify_proc = (int (*) (Xv_opaque, Attr_attribute *)) attrs[1];
...
Pkg_private void ev_notify(Ev_handle view, ...)
{
	...
	return;
    VA_START(args, view);
    copy_va_to_av( args, attr_argv + 2, 0L );
    va_end(args);
