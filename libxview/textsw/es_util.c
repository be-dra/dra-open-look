#ifndef lint
char es_util_c_sccsid[] = "@(#)es_util.c 20.22 93/06/28 DRA: $Id: es_util.c,v 4.1 2024/03/28 19:06:00 dra Exp $";
#endif
 ...
 * stream.
 */
Pkg_private int es_make_buf_include_index(Es_buf_handle esbuf, Es_index indx, int desired_prior_count)
{
	register Es_index last_plus_one, next;
	int read;

	last_plus_one = (desired_prior_count > indx) ? 0
										: indx - desired_prior_count;
...
Pkg_private     Es_status es_copy(Es_handle from, Es_handle to, int newline_must_terminate)
{
