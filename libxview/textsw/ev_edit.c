...
Pkg_private void ev_update_lt_after_edit(Ev_line_table *table, Es_index before_edit, long int delta)
{
	...
	if (delta > 0) {
		...
		if (before_edit < line_seq[0].pos && table->last_plus_one > 0) {
			...
			/* in ft_add_delta, they say that the second param
     		 * is assumed to be < finger_table.last_plus_one
			 * we get an "invalid read error" from valgrind.
			 * I think that here lt_index + 1 == table->last_plus_one...
			 *
			 * Now valgrind no longer complains...
			 */
		 ...
		 */
		ft_set_esi_span(*table, (int)(before_edit + delta + 1), before_edit,
				(int)(before_edit + delta), 0);
...
Pkg_private int ev_delete_span(Ev_chain views, Es_index first, Es_index last_plus_one, Es_index *delta)
{
    ...
    *delta = first - last_plus_one;
    ...
    ev_update_after_edit(views, last_plus_one, (int)*delta, old_length, first);
 ...
 */
{
    ...
    if (delta != 0) ev_update_lt_after_edit(ft, insert, (long)delta);
    if (delta > 0) {
...
Pkg_private void ev_update_view_display(Ev_handle view)
{
    ...
    if ((int) ev_get(view, EV_NO_REPAINT_TIL_EVENT, XV_NULL, XV_NULL, XV_NULL) == 0)
	ev_lt_paint(view, &view->line_table, &view->tmp_line_table);
    ASSERT(allock());
}

Pkg_private void ev_update_after_edit(Ev_chain views, Es_index last_plus_one,
					int delta, Es_index old_length, Es_index min_insert_pos)
/*
...
Pkg_private void ev_make_visible(Ev_handle view, Es_index position, int lower_context, int auto_scroll_by, int delta)
{
	...
	if (delta < 50 * view->line_table.last_plus_one
	    ...
	    */
		...
		old_top = 0; /* very INCOMPLETE , only guessing */
	}
	/*
...
Pkg_private int ev_input_partial(Ev_chain views, CHAR *buf, long int buf_len)
{
    ...
    new_insert_pos = es_replace(views->esh, old_insert_pos, (int)buf_len, buf,
				&used);
...
{
    Ev_chain_pd_handle private = EV_CHAIN_PRIVATE(views);
    int        delta = private->insert_pos - old_insert_pos;
