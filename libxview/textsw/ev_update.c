...
Pkg_private int ev_lt_delta(Ev_handle view, Es_index before_edit, int delta)
{
    ...
    if (delta != 0) {
	ev_update_lt_after_edit(&view->line_table, before_edit, (long)delta);
	result = TRUE;
    }
    return result;
}
...
Pkg_private void ev_lt_format (Ev_handle view, Ev_line_table *new_lt, Ev_line_table *old_lt)
{
	...
	while (new_ix + 1 < lpo) {
		...
		if (new->pos == ES_INFINITY) {
			...
			if (old->pos < ES_INFINITY && old->pos + old->considered > length)
				new->damaged = 0;
			break;
		}
		if (old_ix + 1 < lpo && new->pos == old->pos) {
			 ...
			 */
			while (old_ix + 1 < lpo && (old + 1)->pos == old->pos) {
				++old;
				++old_ix;
			}
			...
			if (old->damaged == -1) {
				 ...
				 */
				if ((old + 1)->damaged > -1	/* Memcheck:Cond */
						&& (old + 1)->damaged + (old + 1)->pos <=	/* Memcheck:Cond */
		...
		++new;
		++new_ix;
		if (line_lpo.last_plus_one == length && line_lpo.considered == length) {
			new->pos = ES_INFINITY;
		}
		else {
			new->pos = line_lpo.last_plus_one;
		...
		if (old_ix == new_ix && old->pos == new->pos) {
			...
			ev_lt_fmt_find_damage(new, old, new_ix, old_ix, lpo);	/* Memcheck:Cond */
#else
			save_old_ix = old_ix;
			/* I changed this "old_ix + 1 < lpo" to "old_ix + 2 < lpo":
			 * valgrind did no longer complain about (tmp + 1)->damaged
			 */
			for (tmp = old; old_ix + 2 < lpo; ++old_ix, ++new_ix, ++tmp) {
				if (tmp->damaged > -1
							|| (tmp + 1)->damaged > -1
							|| tmp->pos == ES_INFINITY)
					break;
			...
			if (tmp > old) {
				XV_BCOPY(old, new, (old_ix - save_old_ix) *
										sizeof(struct ev_impl_line_seq));
				old = tmp;
				new += (old_ix - save_old_ix);
			}
			new->pos = old->pos;
...
Pkg_private void ev_display_line(Ev_handle view, int width_before,
						int lt_index, Es_index first, Es_index last_plus_one)
{
#ifndef BUFSIZE
#define BUFSIZE			 2048
#endif /* BUFSIZE */
...
#define ESBUF_SET_POSITION(pos)						\
	...
	if (buf == (CHAR *) 0) {
		buf = MALLOC((size_t)BUFSIZE_SMALL + 1);
	}
	 ...
	 */
	if (first == es_get_length(chain->esh) || first == last_plus_one) {
	...
	if (esbuf.sizeof_buf >= BUFSIZE_SMALL) {
		bp = MALLOC((size_t)(esbuf.sizeof_buf + 1));
	}
	else {
		bp = buf;
	}
	...
	if (rc == 0) {
		do {
			...
			result = ei_process(eih, range.ei_op | EI_OP_CLEAR_BACK,
...
static void ev_copy_and_fix(Ev_handle view, Rect *new_rect, Rect *old_rect)
{
    ...
    textsw_update_scrollbars(NULL, (Textsw_view_private)view);
