...
static int EV_GC_KEY;

static char     normal_bits[] = {0x08, 0x1c, 0x1c, 0x3e, 0x3e, 0x7f, 0x7f};
static char     ghost_bits[] = {0x08, 0x14, 0x2a, 0x55, 0x2a, 0x14, 0x08};

static void free_gc_list(Xv_object obj, int key, char *data)
{
	/* Now it is too late (obj is no longer a window) to call XFreeGC.
	 * However - a GC is also something 'allocated on the client side'.
	 */
	if (key == EV_GC_KEY) {
		GC *gc = (GC *)data;
...
static	void ev_init_X_carets(Xv_opaque window)
{
	...
	gcvaluemask = GCFunction | GCPlaneMask | GCForeground | GCBackground;

	gc = (GC *) calloc(1L, sizeof(GC));
	gc[0] = XCreateGC(display, drawable, gcvaluemask, &gcvalues);
	xv_set(window,
			XV_KEY_DATA, EV_GC_KEY, gc,
			XV_KEY_DATA_REMOVE_PROC, EV_GC_KEY, free_gc_list,
			NULL);
}

static void ev_put_caret(Xv_opaque window, int which, int x, int y)
{
	Xv_window caret_window;
	Display *display;
	Window drawable;
	Xv_Drawable_info *info;
	Pixmap pixmap;
	...
	caret_window = xv_get(window, OPENWIN_VIEW_PAINT_WINDOW);
	if (! caret_window) caret_window = window;

	DRAWABLE_INFO_MACRO(caret_window, info);
	display = xv_display(info);
	drawable = xv_xid(info);
	...
	gc_list = (GC *) xv_get(caret_window, XV_KEY_DATA, EV_GC_KEY);
	if (!gc_list) {
		ev_init_X_carets(caret_window);
		gc_list = (GC *) xv_get(caret_window, XV_KEY_DATA, EV_GC_KEY);
	}
	gc = gc_list[0];
	...
	new_fg = xv_get(xv_screen(info), SCREEN_INPUT_PIXEL);
	new_bg = xv_bg(info);
	...
	gcvalues.clip_mask = pixmap;
	gcvalues.clip_x_origin = x;
	gcvalues.clip_y_origin = y;
	gcvaluemask |= (GCClipMask | GCClipXOrigin | GCClipYOrigin);
	XChangeGC(display, gc, gcvaluemask, &gcvalues);
	XFillRectangle(display, drawable, gc, x, y, CARET_WIDTH, CARET_HEIGHT);
}

Pkg_private int ev_check_cached_pos_info(Ev_handle view, Es_index pos,
										Ev_pos_info *cache)
{
	struct rect rect;

	if (!EV_CACHED_POS_INFO_IS_VALID(view, pos, cache)) {
		Ev_chain_pd_handle chain_private = EV_CHAIN_PRIVATE(view->view_chain);
...
Pkg_private	Ev_line_table ev_ft_for_rect(Ei_handle	eih, struct rect *rect)
{
	...
	ft_set(result, 0, result.last_plus_one, ES_INFINITY, &line_data);
    result.seq[0] = 0;
    return (result);
}
...
{
	register Ev_impl_line_seq seq = (Ev_impl_line_seq)view->line_table.seq;
...
Pkg_private	int ev_get_selection(Ev_chain chain, Es_index *first, Es_index *last_plus_one, unsigned type)
{
    ...
    *last_plus_one = selection.last_plus_one;
	...
	/* orig: return result,
	 * BUT: since moving away from the sel svc, I have a problem
	 * with "pending delete" after multi-click.
	 *
	 * My thoughts: do I **EVER** want a primary selection be
	 * NOT pending delete??? I don't think so...
	 */
    return ((type == EV_SEL_PRIMARY) ? result | EV_SEL_PD_PRIMARY : result);
}
...
Pkg_private	void ev_display_fixup(Ev_handle view)
{
    ...
    int scroll_by_display_lines)
{
	...
	if (line_count >= 0 && line_count < view->line_table.last_plus_one) {
		...
		span_result.first = span_result.last_plus_one = 0;
		esbuf.esh = chain->esh;
		esbuf.buf = buf;
		...
		if (line_count < 0) {
			...
			pos_to_remember = pos;
			while (i > 0 && pos > 0) {
				span_result = ei_span_of_group(chain->eih, &esbuf,
						EI_SPAN_LINE | EI_SPAN_LEFT_ONLY, (int)(pos - 1));
...
#define MAX_SUB_LINES 128	/* max sublines per a line */
					...
					tmp_pos[0] = pos;
					for (;;) {
						lpo_res = ev_line_lpo(view, (int)tmp_pos[count]);
						count++;
						tmp_pos[count] = lpo_res.last_plus_one;
						if (count == MAX_SUB_LINES ||
								lpo_res.last_plus_one >
			...
			while (i--) {
				span_result =
						ei_span_of_group(chain->eih, &esbuf,
						EI_SPAN_LINE | EI_SPAN_RIGHT_ONLY, (int)pos);
				if (span_result.first == ES_CANNOT_SET)
					break;
		...
		if (span_result.first != ES_CANNOT_SET && line_count < 0 &&
			...
			if (scroll_by_display_lines) {
				while (line_seq[-line_count].pos != pos_to_remember) {
					...
					ev_lt_format(view, &view->tmp_line_table,
							&view->line_table);
				...
				(void)ev_swap_line_table(&view->line_table,
						&view->tmp_line_table);
				if (ev_get(view, EV_NO_REPAINT_TIL_EVENT, XV_NULL, XV_NULL,
								XV_NULL) == 0)
...
ev_ei_process(Ev_handle view, Es_index start, /* Entity to start measuring at */
 ...
 */
{
	...
	while (!ev_process_update_buf(ph)) {
		...
		(void)ev_process(ph, (long)EV_PROCESS_NEXT_LINE,
				(int)EI_OP_MEASURE, PIX_SRC, 0L);
	}
	return (ph->result);
}
...
Pkg_private	struct ei_process_result ev_line_lpo(Ev_handle view, Es_index line_start)
{
	...
	ev_process(ph, 0L, (int)EI_OP_MEASURE, PIX_SRC, 0L);
	if (ph->result.break_reason != EI_PR_BUF_EMPTIED)
	    break;
    }
    if (ph->result.break_reason == EI_PR_NEWLINE)
 ...
 */
{
	...
	ph = ev_process_init(&process_object, view, line_start, pos + 1,
			     &rect, buf, EV_BUFSIZE);
	while (!ev_process_update_buf(ph)) {
	    ev_process(ph, (long)EV_PROCESS_NEXT_LINE,
			      (int)EI_OP_MEASURE, PIX_SRC, 0L);
 ...
 */
{
    ...
    *total_chars = 0;
    ...
    esbuf.first = esbuf.last_plus_one = 0;
    span_result = ei_span_of_group(
	       chain->eih, &esbuf, EI_SPAN_LINE | EI_SPAN_LEFT_ONLY, (int)start);
	...
	expand_result = ei_expand(
				  ...
				  (long)(out_buf_len - expand_result.last_plus_one),
				  view->rect.r_left);
 ...
 */
void ev_paint_view(Ev_handle e_view, Xv_window xv_win, XEvent *xevent)
{
	...
	Textsw_private	priv = TSWPRIV_FOR_VIEWPRIV(VIEW_PRIVATE(xv_win));
	Ev_pd_handle	ev_pd_handle = EV_PRIVATE(e_view);
	int		repaint_caret;

	/* figure out the lines that have been damaged */
	exposed = tty_calc_exposed_lines(xv_win, xevent, ev_pd_handle->caret_pr_pos.y);

	repaint_caret = 
		((exposed->caret_line_exposed) &&
		(priv->caret_state & TXTSW_CARET_ON) && 
		(priv->focus_view == xv_win) &&
	...
	if(repaint_caret)
		ev_put_caret(e_view->pw, 
			(EV_CHAIN_PRIVATE(priv->views)->caret_is_ghost?
