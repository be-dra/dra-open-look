/*      @(#)ps_impl.h 20.13 93/06/28 SMI  DRA: $Id: ps_impl.h,v 4.1 2024/03/28 19:06:00 dra Exp $      */
...
/* wenn der Kommentar da oben etwas mit der folgenden Datenstruktur
 * zu tun haben sollte, ist der Begriff 'four_bytes' in der 64-Bit-Welt
 * ganz falsch ! Wir versuchen es mal so:
 */
struct piece_record_header {
	unsigned pos_prev_rec, flags, start, stop_plus_one, dp_count;
};
struct deleted_piece {
	unsigned source_and_pos, length;
};
...
EXTERN_FUNCTION( caddr_t ps_get, (Es_handle esh, Es_attribute attribute, DOTDOTDOT )) _X_SENTINEL(0);
