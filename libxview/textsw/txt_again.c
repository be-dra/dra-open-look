...
/* Returns FALSE iff it needed to malloc and the malloc failed */
{
	...
	return (FALSE);
    BCOPY(buffer, ptr_to_string->free, (size_t)buffer_length);
...
#define		    STR_INC_SIZE	1024	/* string increment size */
	register int desired_max =
			((((used + min_free_desired) / STR_INC_SIZE) + 1) * STR_INC_SIZE);
#undef	STR_INC_SIZE
	/* BEAR TRAP */
	if (ptr_to_string->max_length < (ptr_to_string->free-ptr_to_string->base)) {
		while (!textsw_again_debug) {
			/* DRA: this looks like an endless loop... */
		}
	}
	if (ptr_to_string->max_length < desired_max) {
		...
		if (old_string == NULL)
			ptr_to_string->base = MALLOC((size_t)desired_max + 1);
		else
...
#endif
{
    ...
    VA_START(args, fmt);
    vsprintf(TXTSW_STRING_FREE(ptr_to_string), fmt, args);
    va_end(args);
    TXTSW_STRING_FREE(ptr_to_string) +=strlen(TXTSW_STRING_FREE(ptr_to_string));

    return 1; /* IGNORED everywhere */
#else /* __linux */
    FILE            _strbuf;
    int             result;
    va_list         args;
...
static	void textsw_record_buf(string_t *again, CHAR *buffer, int buffer_length)
{
...
Pkg_private void textsw_record_caret_motion(Textsw_private priv,
...
Pkg_private void textsw_record_delete(Textsw_private priv)
{
    register string_t *again = &priv->again[0];
...
Pkg_private void textsw_record_edit(Textsw_private priv, unsigned unit, unsigned direction)
{
    register string_t *again = &priv->again[0];
...
Pkg_private void textsw_record_find(Textsw_private priv, CHAR *pattern,
								int pattern_length, int direction)
{
	register string_t *again = &priv->again[0];
	...
	if ((priv->func_state & TXTSW_FUNC_AGAIN) ||
			(priv->state & TXTSW_NO_AGAIN_RECORDING))
		return;
	if (priv->state & (TXTSW_AGAIN_HAS_FIND | TXTSW_AGAIN_HAS_MATCH)) {
		textsw_checkpoint_again(XV_PUBLIC(priv));
	}
	else {
		priv->again_insert_length = 0;
	}
	...
	textsw_printf(again, "%s %s ", cmd_tokens[ord(FIND_TOKEN)],
			direction_tokens[(direction == 0)
					? ord(FORWARD_DIR)
					: ord(BACKWARD_DIR)]);
	textsw_record_buf(again, pattern, pattern_length);
	priv->state |= TXTSW_AGAIN_HAS_FIND;
}
...
Pkg_private void textsw_record_match(Textsw_private priv, unsigned flag, CHAR *start_marker)
{
    ...
    if ((priv->func_state & TXTSW_FUNC_AGAIN) ||
                (priv->state & TXTSW_NO_AGAIN_RECORDING))
	return;
    if (priv->state & TXTSW_AGAIN_HAS_MATCH) {
		textsw_checkpoint_again(XV_PUBLIC(priv));
    } else {
	priv->again_insert_length = 0;
    }
...
static Es_handle textsw_pieces_for_replay(string_t *again)
{
...
static int textsw_text_for_replay(string_t *again, CHAR **ptr_to_buffer)
{
#define CHECK_ERROR(test)	if AN_ERROR(test) goto Again_Error;
    int             count, length = -1;
	...
	*ptr_to_buffer = MALLOC(((unsigned) length + 1) *
	...
	(void) STRNCPY(*ptr_to_buffer,
		       TXTSW_STRING_BASE(again), (size_t)length);
...
#define CHECK_ERROR(test)	if AN_ERROR(test) goto Again_Error;
	...
	switch ((Token) count) {
	  ...
	  case EXTRAS_TOKEN:{
		...
		buffer[buffer_length] = '\0';	/* In case garbage follow */
	  ...
	  case FIELD_TOKEN:{
		...
		if ((field_flag == TEXTSW_FIELD_ENCLOSE) ||
		    ...
		    matched = textsw_match_field_and_normalize(view, &first,
							&last_plus_one, start_marker,
							(unsigned)strlen(start_marker), field_flag, TRUE);
		} else {
	  ...
	  case FILTER_TOKEN:{
		...
		buffer_length = textsw_text_for_replay(again, &buffer);
		if (buffer_length > 0) {
		    BCOPY(buffer, textsw->to_insert, (size_t)buffer_length);
