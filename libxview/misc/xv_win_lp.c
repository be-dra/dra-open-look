...
Xv_public Xv_opaque xv_window_loop(Frame   frame)
{
	...
	Notify_scheduler_func save_sched_func;

	if (!frame) {
		xv_error(XV_NULL,
				ERROR_STRING, XV_MSG("xv_window_loop() : NULL frame passed."),
				NULL);
		return (XV_ERROR);
	}
	 ...
	 */
	if (save_loop_frame) {
		if (window_set_tree_flag(save_loop_frame, XV_NULL, FALSE, FALSE) != XV_OK)
		{
			xv_error(save_loop_frame,
					ERROR_STRING, XV_MSG("xv_window_loop : Attempt to unblock input to windows failed"),
					NULL);
			return (XV_ERROR);
		}
	}
	 ...
	 * non-deaf windows
	 */
	if (window_set_tree_flag(frame, XV_NULL, FALSE, TRUE) != XV_OK) {
		xv_error(frame,
				ERROR_STRING, XV_MSG("xv_window_loop : Attempt to block input to windows failed"),
				NULL);
		return (XV_ERROR);
	}
	...
	if (!display) {
		...
		if (!display) {
			xv_error(XV_NULL,
					ERROR_STRING, XV_MSG("xv_window_loop() : Cannot get handle to display"),
					NULL);
			return (XV_ERROR);
		}
	}
	...
	notify_set_scheduler_func(save_sched_func);
	 ...
	 */
	if (window_set_tree_flag(frame, XV_NULL, FALSE, FALSE) != XV_OK) {
		xv_error(frame,
				ERROR_STRING, XV_MSG("xv_window_loop : Attempt to unblock input to windows failed"),
				NULL);
		return (XV_ERROR);
	 ...
	 */
	if (save_loop_frame) {
		if (window_set_tree_flag(save_loop_frame, XV_NULL, FALSE, TRUE) != XV_OK) {
			xv_error(save_loop_frame,
					ERROR_STRING, XV_MSG("xv_window_loop : Attempt to block input to windows failed"),
					NULL);
			return (XV_ERROR);
