				...
				*bindtmptr = '\0';
				...
				if ((ret = searchmmaplist(pathtmp)) == MAX_MSG) {
					...
					if (stat(pathtmp, &statbuf)) {
						...
						xv_bindtextdomain(domain_name,(unsigned char *)bindtmp);
					}
				}
				else if (ret == -1) {
					bindtmptr = bindtmp;
				...
				if ((newlen = strlen(bindtmp)) > bindinglen) {
					...
					binding = malloc((size_t)newlen + 1);
...
static char *unused_xview_textdomain(char *domain_name)
{
...
char *xv_dgettext(const char *domain_name, const char *msg_id)
{
	...
	if (domain_name == NULL) {
		current_domain = unused_xview_textdomain(NULL);
	}
		...
		current_domain = (char *)domain_name;
	}
	...
	while (msg_inc < first_free) {
		if (!strcmp(msgfile, messages_so[msg_inc].message_so_path)) {
			if (messages_so[msg_inc].fd != -1 &&
				...
				return (char *)_xview_gettext(cur_mess_so, (char *)msg_id);
			}
			else {
				return (char *)msg_id;
			}
	...
	if (msg_inc == MAX_MSG) {
		return (char *)msg_id;	/* not found and no more space */
	}
	if (first_free == MAX_MSG) {
		return (char *)msg_id;	/* no more space        */
	}
...
*/
	 ...
	 */
	addr = mmap(0, (size_t)statbuf.st_size, PROT_READ, MAP_SHARED, fd, (off_t)0);
#else
	/*
	...
	messages_so[first_free].msg_ids =
			(char *)&messages_so[first_free].
			message_list[messages_so[first_free].mess_file_info->message_count];
	messages_so[first_free].msgs =
			(char *)messages_so[first_free].msg_ids +
			messages_so[first_free].mess_file_info->string_count_msgid;
	...
	return (_xview_gettext(cur_mess_so, (char *)msg_id));
}


static char *_xview_gettext(struct message_so messages, char *key_string)
{
	int check;
	int val;
	...
	for (;;) {
		if ((val = strcmp(key_string, messages.msg_ids +
						messages.message_list[check].msgid_offset)) < 0)
		{
