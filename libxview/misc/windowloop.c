...
Xv_private Xv_opaque _xv_block_loop(Frame frame);
...
Xv_private Xv_opaque _xv_block_loop(Frame frame)
{
	 ...
	 */
	if (no_return || xv_get(frame, XV_SHOW) || xv_get(frame, FRAME_CLOSED))
		return (Xv_opaque) 0;
	 ...
	 */
	(void)xv_set(frame, XV_SHOW, TRUE, NULL);

	/*  screen_rect = *(Rect *) xv_get(frame, WIN_SCREEN_RECT); */
	first_window = xv_get(frame, FRAME_NTH_SUBWINDOW, 0);

#ifdef SUNVIEW1
	shadow = xv_get(frame, FRAME_SHADOW, 0);
#endif
...
#ifdef SUNVIEW1
	(void)pw_exposed(xv_get(frame, WIN_PIXWIN));
	if (shadow != NULL)
		(void)pw_exposed(xv_get(shadow, WIN_PIXWIN));
	(void)pw_exposed(xv_get(first_window, WIN_PIXWIN));
#endif
	...
	if (shadow != NULL) {
		 ...
		 */
		shadow_rect = *((Rect *) xv_get(shadow, WIN_RECT));
...
#ifdef SUNVIEW1
	if (shadow) xv_set(shadow, XV_SHOW, FALSE, NULL);
#endif
