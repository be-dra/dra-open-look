#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)om_compat.c 20.16 90/06/21 DRA: $Id: om_compat.c,v 4.1 2024/03/28 17:36:33 dra Exp $";
#endif
#endif
    ...
    Xv_opaque       attr, v1;
{
    ...
    return xv_get(menu_public, (Attr32_attribute)attr, v1);
    ...
    void            (*destroy_proc) (void);
{
	...
	if (xv_get(m_public, XV_IS_SUBTYPE_OF, MENUITEM)) {
		...
		item->extra_destroy_proc =
				(void (*)(Menu_item, Attr_attribute))destroy_proc;
	}
	else if (xv_get(m_public, XV_IS_SUBTYPE_OF, MENU)) {
		menu = MENU_PRIVATE(m_public);
		menu->extra_destroy_proc = (void (*)(struct menu *, Attr_attribute))destroy_proc;
	}
	xv_destroy(m_public);
}
