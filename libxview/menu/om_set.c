...
static int lookup(Xv_menu_item_info *il[], int len, Xv_menu_item_info *mi);
static void menu_add_pin(Xv_menu_info *m);
static void menu_create_title(Xv_menu_info *m, int typ, Xv_opaque arg1);
...
static void destroy_panel_item_handles(Xv_menu_info *m);
	...
	for (; *attrs; attrs = attr_next(attrs)) {
		...
		switch (attrs[0]) {
			...
			case MENU_FRAME_ADD:
				{
					...
					if (!found) {
						if (m->nframes + 1 > m->max_frames) {
							 ...
							 */
							new_list =
									(Frame *) xv_calloc((unsigned)m->
									max_frames + MENU_MAX_FRAMES,
			...
			case MENU_SELECTED:
				{
					int sel =  (int)attrs[1];
					int start = (m->item_list[0]->title ? 2 : 1);

					if (sel < start) sel = start;
					if (sel > m->nitems+ 1) sel = m->nitems;

					m->selected_position = sel;
					m->curitem = sel;

					/* for the moment I'm only interested in MENU_CHOICE */
					if (m->class == MENU_CHOICE) {
						int i;
						Xv_menu_item_info *mi;

						for (i = 0; i < m->nitems; i++) {
							m->item_list[i]->selected = (i == sel-1);
						}
						mi = m->item_list[sel - 1];
			...
			case MENU_ITEM:
				if (m->nitems < m->max_nitems || extend_item_list(m)) {
					m->item_list[m->nitems] =
							MENU_ITEM_PRIVATE(xv_create_avlist(XV_NULL,
									MENUITEM, &attrs[1]));
				}
			...
			case MENU_NCOLS:
				if (m->class == MENU_MIXED) {
					if ((int)attrs[1] > 1) {
						xv_error(XV_NULL,
							ERROR_STRING, XV_MSG("Several columns in mixed menus not supported"),
							ERROR_PKG, MENU,
							NULL);
					}
					m->ncols_fixed = m->ncols = 1;
				}
				else {
			...
			case MENU_NOTIFY_PROC:
				m->notify_proc = (Menu_notify_proc_t) attrs[1];
				if (!m->notify_proc)
			...
			case MENU_REMOVE:
				{
					...
					for (k = 0; k < m->nframes; k++)
						menu_set_acc_on_frame(m->frame_list[k], menu_public,
								(Menu_item) xv_get(MENU_PUBLIC(m),
										MENU_NTH_ITEM, (int)attrs[1]), FALSE);
					/* ACC_XVIEW */
					...
					if (m->pin_window && m->ginfo) {
						...
						menu_compute_max_item_size(m, std_image, &status, TRUE,
													NULL);
						menu_create_pin_panel_items(xv_get(m->pin_window,
										FRAME_CMD_PANEL), m);
			...
			case MENU_STRINGS_AND_ACCELERATORS:
				{
					...
					fprintf(stderr,
							"somebody uses dangerous MENU_STRINGS_AND_ACCELERATORS\n");
				...
				switch ((int)attrs[1]) {
					...
					default:{
							...
							(void)sprintf(dummy,
									...
									(int)attrs[1]);
							xv_error(XV_NULL, ERROR_STRING, dummy, ERROR_PKG,
									MENU, NULL);
						}
				}
				break;
			...
			case MENU_CLASS:
				xv_error(XV_NULL,
						ERROR_STRING,XV_MSG("MENU_CLASS attribute is get-only"),
						ERROR_PKG, MENU,
						NULL);
				break;

			case XV_SET_MENU:
				server_set_menu(menu_public, (Xv_opaque)attrs[1]);
				break;
...
Pkg_private Xv_opaque menu_item_sets(Menu_item menu_item_public, Attr_attribute *attrs)
{
	...
	for (; *attrs; attrs = attr_next(attrs)) {
		...
		switch (attrs[0]) {
			...
			case MENU_NOTIFY_PROC:	/* == MENU_ACTION_PROC == MENU_ACTION */
				mi->notify_proc = (Menu_notify_proc_t) attrs[1];
				if (mi->parent && mi->parent->pin_window
						&& mi->panel_item_handle) {
					xv_set(mi->panel_item_handle, XV_KEY_DATA, MENU_NOTIFY_PROC,
							mi->notify_proc, NULL);
				}
				break;
			...
			case MENU_ACTION_ITEM:
				...
				mi->image.width = mi->image.height = 0;
				mi->notify_proc = (Menu_notify_proc_t) attrs[2];
				if (mi->parent && mi->parent->pin_window
						&& mi->panel_item_handle) {
					xv_set(mi->panel_item_handle, PANEL_LABEL_STRING,
							mi->image.string, XV_KEY_DATA, MENU_NOTIFY_PROC,
							mi->notify_proc, NULL);
				}
				break;
			...
			case MENU_ACTION_IMAGE:
				...
				if (mi->parent && mi->parent->pin_window
						&& mi->panel_item_handle) {
					xv_set(mi->panel_item_handle, PANEL_LABEL_IMAGE,
							mi->image.svr_im, XV_KEY_DATA, MENU_NOTIFY_PROC,
							mi->notify_proc, NULL);
				}
				break;
			...
			case MENU_GEN_PROC:
				mi->gen_proc = (Menu_item_gen_proc_t) attrs[1];
				break;

			case MENU_PREVIEW_PROC:
				mi->preview_proc = (Menu_item_preview_proc_t) attrs[1];
				break;
			...
			case MENU_GEN_PROC_IMAGE:
				...
				mi->image.svr_im = (Server_image) attrs[1];
				mi->image.width = mi->image.height = 0;
				mi->gen_proc = (Menu_item_gen_proc_t) attrs[2];
				if (mi->parent && mi->parent->pin_window
						&& mi->panel_item_handle) {
					xv_set(mi->panel_item_handle, PANEL_LABEL_IMAGE,
							mi->image.svr_im, NULL);
			...
			case MENU_GEN_PULLRIGHT_IMAGE:
				...
				mi->image.svr_im = (Server_image) attrs[1];
				mi->image.width = mi->image.height = 0;
				mi->gen_pullright = (Menu_item_gen_proc_t) attrs[2];
				mi->pullright = mi->gen_pullright != NULL;
				mi->value = 0;
				if (mi->parent && mi->parent->pin_window
						&& mi->panel_item_handle) {
			...
			case MENU_INACTIVE:
				if (((int)attrs[1] ? TRUE : FALSE) != mi->inactive) {
					...
					if (mi->parent && mi->parent->pin_window &&
							mi->panel_item_handle) {
						xv_set(mi->panel_item_handle,
								PANEL_INACTIVE, mi->inactive, NULL);
					}
				}
				break;
			...
			case MENU_PULLRIGHT:
				...
				if (mi->parent && mi->parent->pin_window
						&& mi->panel_item_handle) {
					xv_set(mi->panel_item_handle, PANEL_ITEM_MENU, mi->value,
							NULL);
				}
				mi->mark_type |= OLGX_HORIZ_MENU_MARK;
				break;
			...
			case MENU_PULLRIGHT_IMAGE:
				...
				if (mi->parent && mi->parent->pin_window
						&& mi->panel_item_handle) {
					xv_set(mi->panel_item_handle, PANEL_LABEL_IMAGE,
							mi->image.svr_im, PANEL_ITEM_MENU, mi->value, NULL);
				}
				mi->mark_type |= OLGX_HORIZ_MENU_MARK;
				break;
			...
			case MENU_PULLRIGHT_ITEM:
				...
				if (mi->parent && mi->parent->pin_window
						&& mi->panel_item_handle) {
					xv_set(mi->panel_item_handle, PANEL_LABEL_STRING,
							mi->image.string, PANEL_ITEM_MENU, mi->value, NULL);
				}
				mi->mark_type |= OLGX_HORIZ_MENU_MARK;
				break;
#endif /* OW_I18N */
			...
			case MENU_VALUE:
				...
				if (mi->parent && mi->parent->pin_window
						&& mi->panel_item_handle) {
					xv_set(mi->panel_item_handle, PANEL_ITEM_MENU, XV_NULL,
							NULL);
				}
				break;
...
Pkg_private void menu_destroys(Xv_menu_info *m, void (*destroy_proc) (Xv_menu_info *, Attr_attribute))
{
	...
	if (m->shadow_window) {
		...
		screen_set_cached_window_busy(xv_screen(info), m->shadow_window, FALSE);
	}
	if (destroy_proc)
		destroy_proc(m, (Attr_attribute) MENU_MENU);
	free(m);
}
...
Pkg_private void menu_set_pin_window(Xv_menu_info *m, Xv_opaque pin_window)
{
	...
	xv_set(m->pin_window, XV_KEY_DATA, MENU_MENU, m, NULL);
       ...
       if (WIN_IS_IN_LOOP)
            window_set_tree_flag(m->pin_window, XV_NULL, FALSE, TRUE);
        else
            window_set_tree_flag(m->pin_window, XV_NULL, FALSE, FALSE);
...
static int extend_item_list(register Xv_menu_info *m)
{
	...
	m->item_list = (Xv_menu_item_info **) xv_realloc((char *)m->item_list,
			(m->max_nitems * sizeof(Xv_menu_item_info)));
	if (!m->item_list) {
...
static void replace_item(Xv_menu_item_info *il[],	/* item list ptr */ int len,	/* nbr of menu items */ int pos,	/* position: 1= first item */ Xv_menu_item_info *mi)
{
    if (pos < 1 || pos > len)
	return; /* invalid position */
    il[pos - 1] = mi;
...
/* typ: MENU_STRING, MENU_IMAGE or 0 (= no title) */
/* the string or pixrect */
static void menu_create_title(Xv_menu_info *m, int typ, Xv_opaque arg1)
{
    register int    i;
    Menu_item       menu_item;
...
static void destroy_panel_item_handles(Xv_menu_info *m)
{
	...
	if (panel_item_destroy_flag == 1) panel_item_destroy_flag = 2;
		...
		for (i = 0; i < m->nitems; i++) {
			...
			if (mi->panel_item_handle) {
				if (mi->class == MENU_COMMAND) {
					xv_set(mi->panel_item_handle, PANEL_ITEM_MENU, NULL, NULL);
					xv_destroy(mi->panel_item_handle);
				}
				else if (!panel_item_destroyed) {
					/* assuming: only ONE non-MENU_COMMAND */
					xv_destroy(mi->panel_item_handle);
					panel_item_destroyed = TRUE;
				}
				mi->panel_item_handle = XV_NULL;
			}
		}
	}
	else {
...
Xv_private void menu_set_acc_on_frame(Frame frame, Menu menu, Menu_item	item, int set)
{
	...
	if (xv_instance_app_name) {
		...
		if (tmp) {
			...
			acc_resource_name = xv_malloc(strlen(xv_instance_app_name) +
					1 +
					strlen(item_instance_name) + 1 + strlen("accelerator") + 1);
	...
	if (!acc_string) {
		...
		menu_set_key_qual(menu, item, FALSE, (KeySym) NULL,
				(unsigned int)XV_NULL, (unsigned int)XV_NULL, (char *)NULL);
		return;
	}
	...
	if (set) {
		...
		parse_result = server_parse_keystr(server_public,
				acc_string, &keysym, &code, &modifiers, meta_modmask, qual_str);
		...
		if (parse_result == XV_OK) {
			...
			if (result == XV_OK) {
				...
				Frame_accel_data *dup_acc = (Frame_accel_data *) NULL;
				...
				menu_accelerator = (Frame_menu_accelerator *) xv_get(frame,
						FRAME_MENU_X_ACCELERATOR, code, modifiers, keysym);
		 ...
		 * Unset diamond mark flag
		 */
		menu_set_key_qual(menu, item, FALSE, (KeySym) 0, (unsigned int)0,
				(unsigned int)0, (char *)NULL);
	}
}
/* ACC_XVIEW */
