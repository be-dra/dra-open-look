...
Xv_private char *fullscreen_translate_report_code(int);

static void fullscreen_allow_events(Display *display, int event_mode);

Pkg_private Xv_opaque fullscreen_set_avlist(Xv_opaque self,
										Attr_avlist avlist)
{
	Xv_fullscreen *fullscreen_public = (Xv_fullscreen *)self;
	Attr_avlist attrs;
	...
	int new_im = FALSE;
	int saw_allow_events_attr = FALSE, allow_mode = TRUE;
	...
	int new_pointer_grab = -1, new_kbd_grab = -1, new_server_grab = -1;
	...
	if (new_pointer_grab == TRUE || new_kbd_grab == TRUE
			|| new_server_grab == TRUE || grab_change == TRUE
			|| grab_mode_change == TRUE) {
		...
		grab_result = xv_win_grab(fullscreen->input_window,
				...
				fullscreen->owner_events, &grab_successful);
		if (!grab_successful) {
			(void)xv_error((Xv_opaque) fullscreen,
					ERROR_STRING,
					fullscreen_translate_report_code(grab_result),
					ERROR_PKG, FULLSCREEN, NULL);
			return (Xv_opaque) XV_ERROR;
		}
	}
	if (saw_sync_now_attr && sync_now && (fullscreen->sync_mode_now == FALSE)) {
		int grab_successful, grab_result;
		...
		grab_result = xv_win_grab(fullscreen->input_window,
				...
				fullscreen->grab_kbd,
				fullscreen->grab_server,
				(int)(fullscreen->pointer_ptr_mode=(Fullscreen_grab_mode)FALSE),
				(int)(fullscreen->pointer_kbd_mode=(Fullscreen_grab_mode)TRUE),
				(int)(fullscreen->keyboard_ptr_mode=(Fullscreen_grab_mode)TRUE),
				(int)(fullscreen->keyboard_kbd_mode=(Fullscreen_grab_mode)FALSE),
				fullscreen->owner_events, &grab_successful);
		...
		grab_result = xv_win_grab(fullscreen->input_window,
				...
				fullscreen->grab_kbd,
				fullscreen->grab_server,
				(int)(fullscreen->pointer_ptr_mode=(Fullscreen_grab_mode)TRUE),
				(int)(fullscreen->pointer_kbd_mode=(Fullscreen_grab_mode)TRUE),
				(int)(fullscreen->keyboard_ptr_mode=(Fullscreen_grab_mode)TRUE),
				(int)(fullscreen->keyboard_kbd_mode=(Fullscreen_grab_mode)TRUE),
				fullscreen->owner_events, &grab_successful);
	...
	if (allow_sync_event) {
		if (fullscreen->sync_mode_now == TRUE) {
			...
			xv_error(XV_NULL,
					ERROR_STRING,
					XV_MSG
					("Attempt to allow synchronous event processing without first being in synchronous mode. Attribute ignored!"),
					ERROR_PKG, FULLSCREEN, NULL);
		}
	}
...
static void fullscreen_allow_events(Display *display, int event_mode)
	/* Xlib allow events event_mode argument */
{
