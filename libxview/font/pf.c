#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)pf.c 20.17 93/06/28 SMI DRA: RCS $Id: pf.c,v 4.1 2024/03/28 17:55:25 dra Exp $ ";
#endif
#endif
...
/* public functions */
Xv_public Pixfont * xv_pf_default(void); /* sei still */

Xv_public Pixfont * xv_pf_default(void)
{
    return (xv_pf_open((char *) 0, XV_NULL));
}

Xv_public Pixfont *xv_pf_open(char *fontname, Xv_server srv)
{
	if (srv) {
		return ((Pixfont *) xv_get(srv, SERVER_FONT_WITH_NAME, fontname));
	}
	else {
		if (!xv_default_server)
			xv_create(XV_NULL, SERVER, NULL);
...
Xv_public Pixfont *xv_pf_open_private(char *fontname); /* sei still */
...
Xv_public int xv_pf_close(Pixfont *pf); /* sei still */
