...
Xv_public struct pr_size xv_pf_textbatch(struct pr_prpos *dst, int *lenp, Pixfont *pf, char *str);
...
Xv_public int xv_pf_text(struct pr_prpos prpos, int op, Pixfont *pf, char *str)
{
	...
	Window root_window = RootWindow(xv_default_display,
	...
	if (glyph_width < needed_width || glyph_height < needed_height) {
		...
		glyph_pixmap = XCreatePixmap(xv_default_display, root_window,
				(unsigned)glyph_width, (unsigned)glyph_height, 1);
	}
	if (!glyph_gc) {
		XGCValues v;
		...
		glyph_gc =
				XCreateGC(xv_default_display, glyph_pixmap,
				GCForeground | GCBackground, &v);
	}
	XSetFont(xv_default_display, glyph_gc, (Font) xv_get((Xv_opaque) pf,
					XV_XID));
	XDrawImageString(xv_default_display, glyph_pixmap, glyph_gc,
			pf->pf_char[(int)*str].pc_home.x,
			-pf->pf_char[(int)*str].pc_home.y,
			str, rmdr);
	errors = xv_read_internal(prpos.pr,
			prpos.pos.x - pf->pf_char[(int)*str].pc_home.x,
			prpos.pos.y + pf->pf_char[(int)*str].pc_home.y, needed_width,
...
Xv_public void xv_pf_textbound(struct pr_subregion *bound, int len, Pixfont *pf, char *str)
/*
