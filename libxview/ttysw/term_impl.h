...
#  define TERMSW_VIEW_FROM_TERMSW_FOLIO(_termsw_priv) \
	termsw_first_view_private(_termsw_priv)
 ...
 * in termsw_init_internal, it cannot become a termsw.  The
...
#define TTY_PRIVATE_TERMSW(_termsw_public)	\
		((Ttysw_private)((Xv_termsw *)(_termsw_public))->private_tty)
...
#define TTY_VIEW_PRIVATE_FROM_TTY(_ttysw_folio_public)		\
		((Ttysw_view_handle)((Ttysw_private) TTY_PRIVATE(_ttysw_folio_public))->view)
...
typedef struct _Termsw_folio_object {
	...
	Menu saved_topmenu; /* to avoid MLKs */
    /* For Textedit */
    Textsw		textedit;
