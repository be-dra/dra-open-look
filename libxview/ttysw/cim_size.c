#ifndef lint
char     cim_size_c_sccsid[] = "@(#)cim_size.c 20.32 93/06/28 DRA: $Id: cim_size.c,v 4.2 2024/12/16 22:52:50 dra Exp $";
#endif
    ...
    int             for_temp;	/* decide which data structure to go into */
{
    ...
    newimage = (CHAR **) calloc(1L, (size_t) (ttysw_bottom * sizeof(CHAR *)));
    newmode = (char **) calloc(1L, ttysw_bottom * sizeof(char *));
    bold = (char *) calloc(1L, (size_t)(nchars + 2 * ttysw_bottom));
#ifdef OW_I18N
    line = (CHAR *) calloc(1, (unsigned) ((nchars + 2 * ttysw_bottom) *
						sizeof(CHAR)));
#else
    line = (char *) calloc(1L, (size_t) (nchars + 2 * ttysw_bottom));
#endif
...
#else
	XV_BCOPY(image[oldrow], temp_image[row], (size_t)sl);
#endif
	XV_BCOPY(screenmode[oldrow], temp_mode[row], (size_t)sl);
