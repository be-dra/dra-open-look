...
/* DRA: be very careful concerning the mixture of view and Textsw/Termsw/Ttysw:
 * If you have split views in a Termsw and start (e.g.) vi there,
 * only this view switches its mode!
 */
 ...
 */
{
	...
	xv_set(textsw_view, TEXTSW_READ_ONLY, TRUE, NULL);
	xv_set(xv_get(textsw_view, WIN_VERTICAL_SCROLLBAR), XV_SHOW, FALSE, NULL);
	...
	fd = (int)xv_get(textsw_view, TTY_TTY_FD);
...
#  endif /* XV_USE_TERMIOS */
	...
	if (!ttysw_waiting_for_pty_input) {
		notify_set_input_func(TTY_PUBLIC(ttysw), ttysw_pty_input_pending,
											ttysw->ttysw_pty);
...
Pkg_private int ttysw_be_termsw(Ttysw_view_handle ttysw_view)
{
	...
	textsw_view = ttysw->current_view_public;	/* Textsw really need the
												 * public view handle
												 */
	xv_set(textsw_view, TEXTSW_READ_ONLY, FALSE, NULL);
...
#define	TTYSW_USEC_DELAY 100000
	...
	xv_set(xv_get(textsw_view, WIN_VERTICAL_SCROLLBAR), XV_SHOW, TRUE, NULL);
...
#      ifdef __linux
	/* I'm just trying out: this seems to fix the problems after vi end */
	ttysw->termios.c_lflag &= (~(ICANON | ECHO | ECHOE));
	tcsetattr(ttysw->ttysw_tty, TCSANOW, &ttysw->termios);
#      endif /* __linux */
#  else /* XV_USE_TERMIOS */
...
#  endif /* XV_USE_TERMIOS */
	...
	if (xv_get(textsw, WIN_KBD_FOCUS)) {
		win_post_id(textsw_view, KBD_USE, NOTIFY_IMMEDIATE);
	}
	else {
	...
	/* when switching back to termsw-mode, we need a repaint... */
	/* das funktioniert zwar, aber nach der Eingabe des ersten Kbd-Events
	 * ist das Fenster 'dreckig'
	 * VIEW_PRIVATE(textsw_view)->obscured hilft nichts
	 */
	{
		Display *dpy = (Display *)xv_get(textsw_view, XV_DISPLAY);
		Window w;
		XSetWindowAttributes xswa;

		w = XCreateWindow(dpy, xv_get(textsw_view, XV_XID), 0, 0,
					(unsigned)xv_get(textsw_view, XV_WIDTH),
					(unsigned)xv_get(textsw_view, XV_HEIGHT),
