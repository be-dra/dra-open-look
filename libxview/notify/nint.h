/*	@(#)nint.h 20.11 93/06/28 SMI   DRA: $Id: nint.h,v 4.1 2024/03/28 18:09:08 dra Exp $	*/
...
extern	Notify_error nint_alloc_stack(void);/* (Pre)allocate enough stack space */

extern	Notify_func nint_set_func(NTFY_CONDITION *cond, Notify_func new_func);
					/* Call while in critical section. */
extern	Notify_func nint_get_func(NTFY_CONDITION *cond); /* Call while in critical section. */
...
extern	Notify_func nint_next_callout(Notify_client nclient,
						    NTFY_TYPE type);
extern	void nint_pop_callout(void);		/* Enters critical section
						   to do work. */
extern	void nint_unprotected_pop_callout(void);	/* Like
