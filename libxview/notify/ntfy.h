...
typedef struct	ntfy_client {
	...
	Notify_prioritizer_func prioritizer;	/* Function to call to do notification
...
/* the old value of NTFY_FUNCS_MAX was 6 - and I hit that border.
 * I don't understand the reason for the original definition - 
 * I will just try:
 */
/* didn't work, SIGSEGV: #  define NTFY_FUNCS_MAX 10
...
#else
/* DRA_CHANGED: #define sigisempty(s)   (!(*(s))) */
...
typedef	NTFY_ENUM (*NTFY_ENUM_FUNC)(NTFY_CLIENT *, NTFY_CONDITION *, NTFY_ENUM_DATA);
 ...
 * Node pool routines
 */
extern	NTFY_NODE *ntfy_alloc_node(void);
extern	void ntfy_free_node(NTFY_NODE *node);
extern	void ntfy_replenish_nodes(void);

extern	char *ntfy_malloc(u_int size);
extern	void ntfy_free_malloc(char *ptr);
extern	void ntfy_flush_tb_freed(void);	/* called from non-interrupt level */
/*
 * Ntfy_list_* routines need to be executed while data protected
 */
extern	void ntfy_append_node(NTFY_NODE **node_list, NTFY_NODE *new_node);
extern	void ntfy_remove_node(NTFY_NODE **node_list, NTFY_NODE *node_axe);
...
extern	NTFY_CLIENT *ntfy_new_nclient(NTFY_CLIENT **client_list,
					    Notify_client nclient,
					    NTFY_CLIENT **client_latest);
extern	void ntfy_remove_client(NTFY_CLIENT **client_list,
					    NTFY_CLIENT *client,
					    NTFY_CLIENT **condition_latest,
					    NTFY_WHO who);	/* Removes client from client_list
					   and makes sure that all conditions
					   all removed first */

extern void ntfy_add_to_table(NTFY_CLIENT *, NTFY_CONDITION *, int );
 ...
 */
extern	NTFY_CONDITION *ntfy_find_condition(NTFY_CONDITION *condition_list,
					    NTFY_TYPE type,
...
extern	NTFY_CONDITION *ntfy_new_condition(NTFY_CONDITION **condition_list,
					    NTFY_TYPE type,
					    NTFY_CONDITION **condition_latest,
					    NTFY_DATA data, int use_data);/* Creates condition if can't find.
					   Inits to data if use_data else 0.
					    */
#define	NTFY_USE_DATA		1	/* Use_data flag value */
#define	NTFY_IGNORE_DATA	0	/* Use_data flag value */

extern	void ntfy_unset_condition(NTFY_CLIENT **client_list,
					    NTFY_CLIENT *client,
					    NTFY_CONDITION *condition,
					    NTFY_CLIENT **client_latest,
					    NTFY_WHO who);	/* Removes condition from client if
					   condition->function is null &
					   client from client_list if no
					   conditions in it.
					    */
extern	void ntfy_remove_condition(NTFY_CLIENT *client,
					    NTFY_CONDITION *condition,
					    NTFY_WHO who);	/* Removes condition from client
					    */

extern void ntfy_remove_from_table(NTFY_CLIENT *client, NTFY_CONDITION *condition);
...
extern	NTFY_ENUM ntfy_paranoid_enum_conditions(NTFY_CLIENT *clients,
						NTFY_ENUM_FUNC enum_func,
						NTFY_ENUM_DATA  context); /* May remove any client or
					   condition during call.  But may not
					   be call recursively. */
extern	NTFY_ENUM ntfy_enum_conditions(NTFY_CLIENT *clients, NTFY_ENUM_FUNC enum_func, NTFY_ENUM_DATA context); /* May remove condition during call.
...
void	ntfy_set_errno_debug(Notify_error  error);
#else	 /* NTFY_DEBUG */
...
void	ntfy_set_warning_debug(Notify_error error);
#else	 /* NTFY_DEBUG */
...
void	ntfy_assert_debug(int code);
#else	 /* NTFY_DEBUG */
...
#elif defined(DRA_LINUX_10)
#  define OVERLOAD_SYSCALLS 0
#else
#  define OVERLOAD_SYSCALLS 1
#endif
