...
static int pipe_started;
static int pipefds[2];

static void ndet_signal_catcher(int sig)
{

#if defined(SVR4) || defined(__linux)
	void (*old_handler)(int sig) = ndet_prev_sigvec[sig].sa_handler;
#else
	void (*old_handler) (int) = ndet_prev_sigvec[sig].sv_handler;
#endif /* SVR4 */
...
#else
	{ { ndet_signal_catcher }, {{ 0 }}, SA_RESTART, 0};
#endif
	static ucontext_t *ndet_signal_context;
#endif  /* SVR4 */
...
static void ndet_send_async_sigs(sigset_t *sigs);
static void     ndet_fig_fd_change(void), ndet_fig_wait3_change(void),
                ndet_fig_destroy_change(void), ndet_fig_sig_change(void);
static NTFY_ENUM ndet_poll_send(NTFY_CLIENT *client,
    NTFY_CONDITION *condition, NTFY_ENUM_DATA  context);
static NTFY_ENUM ndet_sig_send(NTFY_CLIENT *client,
    NTFY_CONDITION *condition,
    NTFY_ENUM_DATA  context);

static NTFY_ENUM ndet_itimer_change( NTFY_CLIENT    *client,
    NTFY_CONDITION *condition, NTFY_ENUM_DATA  context);
static NTFY_ENUM ndet_destroy_change(NTFY_CLIENT    *client,
    NTFY_CONDITION *condition, NTFY_ENUM_DATA  context);
static NTFY_ENUM ndet_sig_change(NTFY_CLIENT    *client,
    NTFY_CONDITION *condition, NTFY_ENUM_DATA  context);
static NTFY_ENUM ndet_fd_change(NTFY_CLIENT *client, NTFY_CONDITION *condition,
    							NTFY_ENUM_DATA  context);
static NTFY_ENUM ndet_wait3_change(NTFY_CLIENT *client,
    NTFY_CONDITION *condition,
    NTFY_ENUM_DATA  context);
static NTFY_ENUM ndet_virtual_set_tv_update(NTFY_CLIENT *client,
							NTFY_CONDITION *condition, NTFY_ENUM_DATA  context);
static NTFY_ENUM ndet_async_sig_send(NTFY_CLIENT *client,
    NTFY_CONDITION *condition,
    NTFY_ENUM_DATA  context);
...
Notify_error notify_start()
{
	...
	do {
		 ...
		 */
		timer = ((ndet_flags & (NDET_POLL |NDET_ITIMER_ENQ | NDET_NO_DELAY))
		...
		if (FD_ISSET(pipefds[0], &ibits)) {
			do {
				readval = read(pipefds[0], pipebuf, (size_t)10);
			}
...
/* ARGSUSED */
static NTFY_ENUM ndet_fd_change(NTFY_CLIENT    *client,
    NTFY_CONDITION *condition,
    NTFY_ENUM_DATA  context)
{
...
pkg_private void ndet_update_real_itimer(void)
{
	...
	n = gettimeofday(&enum_itimer.current_tv, (struct timezone *)0);	/* SYSTEM CALL */
...
/* ARGSUSED */
static NTFY_ENUM
ndet_virtual_set_tv_update(NTFY_CLIENT    *client, NTFY_CONDITION *condition,
    NTFY_ENUM_DATA  context)
{
...
/* ARGSUSED */
static          NTFY_ENUM
ndet_destroy_change(NTFY_CLIENT    *client,
    NTFY_CONDITION *condition,
    NTFY_ENUM_DATA  context)
{
...
ndet_fd_send(NTFY_CLIENT *client, register NTFY_CONDITION *condition,
    NTFY_ENUM_DATA  context)
{
...
static          NTFY_ENUM
ndet_sig_send(NTFY_CLIENT *client,
    NTFY_CONDITION *condition,
    NTFY_ENUM_DATA  context)
{
    register NDET_ENUM_SEND *enum_send = (NDET_ENUM_SEND *) context;
...
#ifdef SEEMS_UNUSED
extern void notify_set_signal_check(struct timeval tv)
{
