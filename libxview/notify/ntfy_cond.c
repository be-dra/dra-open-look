#ifndef	lint
#ifdef sccs
static char     sccsid[] = "@(#)ntfy_cond.c 20.19 93/06/28 Copyr 1985 Sun Micro  DRA: $Id: ntfy_cond.c,v 4.1 2024/03/28 18:09:08 dra Exp $ ";
#endif
#endif
	...
	if (condition->data.an_u_int != 0) {
		switch (condition->type) {
			...
			case NTFY_VIRTUAL_ITIMER:
				if (who == NTFY_NDET)
					ntfy_free_node((NTFY_NODE *) condition->data.an_u_int);
				break;
			case NTFY_WAIT3:
				if (who == NTFY_NDIS)
					ntfy_free_malloc((NTFY_DATA) condition->data.an_u_int);
				break;
			default:
				break;
		}
