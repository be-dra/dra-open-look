#ifndef	lint
#ifdef sccs
static char     sccsid[] = "@(#)ndet_auto.c 20.21 93/06/28 Copyr 1985 Sun Micro DRA: $Id: ndet_auto.c,v 4.2 2024/11/30 13:02:13 dra Exp $ ";
#endif
#endif
...
static Notify_value ndet_auto_sig_func(Notify_client nclient, int sig, Notify_signal_mode mode)
{
    return (NOTIFY_DONE);
}

pkg_private void ndet_toggle_auto(sigset_t *old_bits, int sig)
{
	...
	if (old_bit && !new_bit)
		...
		(void)notify_set_signal_func(ndet_auto_nclient, NOTIFY_SIGNAL_FUNC_NULL,
				sig, NOTIFY_SYNC);
 ...
 */
static NTFY_ENUM ndet_auto_sigchld(NTFY_CLIENT *client, NTFY_CONDITION *condition, NTFY_ENUM_DATA  context)
{
...
/* ARGSUSED */
pkg_private NTFY_ENUM ndet_auto_sig_send(NTFY_CLIENT *client, NTFY_CONDITION *condition, NTFY_ENUM_DATA context)
{
    register NDET_ENUM_SEND *enum_send = (NDET_ENUM_SEND *) context;
...
#ifdef DRA_IRIX
		nfds = select(FD_SETSIZE, &ibits, &obits, &ebits, &ndet_polling_tv);
#else
