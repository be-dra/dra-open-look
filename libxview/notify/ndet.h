...
extern void ndet_toggle_auto(sigset_t *old_bits, int sig);	/* When some condition that the notifier
...
extern	Notify_client ndet_auto_nclient;/* Private notifier client for auto
					   signal handling */
extern	NTFY_ENUM ndet_auto_sig_send(NTFY_CLIENT *client,
					    NTFY_CONDITION *condition,
					    NTFY_ENUM_DATA context);	/* Tell auto sig manager that one of
...
extern void ndet_enable_sig(int sig);	/* Call this routine (other than from
...
extern void ndet_send_delayed_sigs(void);/* Process any async signal conditions
...
extern int ndet_check_pid(int pid);
extern int ndet_check_tv(struct timeval *tv);
extern int ndet_itimer_expired(NTFY_CLIENT *client, NTFY_CONDITION *condition);

extern	int ndet_check_sig(int sig);	/* Returns 0 if all OK else -1 and sets
					   notify_errno to NOTIFY_BAD_SIGNAL */
extern	int ndet_check_mode(Notify_signal_mode mode,
					    NTFY_TYPE *type_ptr);	/* Returns 0 if all OK else -1 and sets
					   notify_errno to NOTIFY_INVAL.  Sets
					   type_ptr if not null.  */

extern	int ndet_check_which(int which, NTFY_TYPE *type_ptr);	/* Turn itimer which into type.
...
extern	struct timeval ndet_tv_subt(struct timeval atv, struct timeval btv);
						/* Subtracts b from a.  Will round down
...
extern	struct timeval ndet_tv_min(struct timeval atv, struct timeval btv);	/* Find min of b and a.  */


extern	struct timeval ndet_real_min(NTFY_ITIMER *ntfy_itimer, struct timeval  current_tv);	/* Figure the interval that has
					    ...
					    */
extern	struct timeval ndet_virtual_min(NTFY_ITIMER *ntfy_itimer, struct timeval  current_tv); /* Update the interval until
					    ...
					    ndet_virtual_min.  */
extern	void ndet_reset_itimer_set_tv(NTFY_CONDITION *condition);/* Reset ntfy_itimer->set_tv based on type  */

extern int ndet_check_when(Notify_event_type when, NTFY_TYPE *type_ptr);
extern int ndet_check_status(Destroy_status  status);
...
typedef	struct  ndet_enum_itimer {
	...
	struct timeval	(*min_func)(NTFY_ITIMER *ntfy_itimer, struct timeval  current_tv);	/* Returns the interval that a given
					   itimer needs to wait until expiration
					   (virtual itimer resets set_time) */
	Notify_value	(*expire_func)(const char *unused, int **UNUSED);/* Called when itimer expiration
					   detected */
...
extern	void ndet_update_virtual_itimer(void);/* Some virtual itimer related
					   ...
					   catching of SIGVTALRM. */
extern	void ndet_update_real_itimer(void);	/* Some real itimer related condition
					   has changed.  Determine minimum wait
					   and set real itimer.  Enable(disable)
					   notifier auto signal catching of
					   SIGALRM. */
extern	NTFY_ENUM ndet_fd_send(NTFY_CLIENT *client, register NTFY_CONDITION *condition, NTFY_ENUM_DATA  context);	/* Enqueue notifications for any fds
					   in (NDET_ENUM_SEND *) context.  */


extern	void ndet_set_event_processing(Notify_client nclient, int on);/* Called from dispatcher to tell
