...
static int server_image_create_internal(Xv_opaque parent,
		Server_image srvim_public, Attr_avlist avlist, int *offset_ptr)
{

	register Server_image_info *server_image;
	register Attr_avlist attrs;
	register Pixrect *rpr = &((Xv_server_image *) srvim_public)->pixrect;
	Xv_Drawable_info *info;
	Xv_Screen screen;

	DRAWABLE_INFO_MACRO(srvim_public, info);
	((Xv_server_image *) srvim_public)->private_data =
			(Xv_opaque) xv_alloc(Server_image_info);

	if (!(server_image = SERVER_IMAGE_PRIVATE(srvim_public))) {
		xv_error(XV_NULL,
				ERROR_STRING,
	...
	*offset_ptr = xv_set_embedding_data((Xv_opaque)rpr, srvim_public);
			...
			case SERVER_IMAGE_BITMAP_FILE:{
					...
					if (status != BitmapSuccess) {
						xv_error(XV_NULL,
								ERROR_STRING,
								XV_MSG
								("SERVER_IMAGE_BITMAP_FILE: Server image creation failed"),
								ERROR_PKG, SERVER_IMAGE, NULL);
						return (XV_ERROR);
					}
										...
										Attr_attribute *avlist)
{
	...
	if (!(xv_xid(info) = 
		XCreatePixmap(display,
		    xv_get(xv_get(screen, XV_ROOT), XV_XID),
		    (unsigned)rpr->pr_size.x, (unsigned)rpr->pr_size.y, (unsigned)rpr->pr_depth))) {
	    		xv_error(XV_NULL,
		     	    ERROR_STRING,
	...
	xv_set_gc_op(xv_display(info), info,
	    	xv_gc(srvim_public, info), PIX_SRC,
		XV_USE_CMS_FG, XV_DEFAULT_FG_BG);

	XSetPlaneMask(xv_display(info), xv_gc(srvim_public, info),
			  (unsigned long)((0x1 << rpr->pr_depth) - 1));

	xv_rop_mpr_internal(xv_display(info), xv_xid(info),
	    xv_gc(srvim_public, info), 0, 0, rpr->pr_width,
...
/*ARGSUSED*/
static Xv_opaque server_image_get_internal(Server_image srvim_public,
			int *status, Attr_attribute attr, va_list args)
{
	...
	break;
      ...
      case SERVER_IMAGE_PIXMAP:
	rv = (Xv_opaque)xv_get(srvim_public, XV_XID);
	break;
...
static int server_image_destroy_internal(Server_image srvim_public, Destroy_status status)
{

    register Server_image_info *image = SERVER_IMAGE_PRIVATE(srvim_public);
    Xv_Drawable_info *info;
