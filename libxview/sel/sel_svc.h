/*      @(#)sel_svc.h 20.35 93/06/28 SMI      DRA: $Id: sel_svc.h,v 4.5 2024/12/21 21:24:43 dra Exp $ */
...
#define SELN_CLIPBOARD SELN_SHELF
...
typedef enum	{
    ...
    SELN_FN_FRONT = 5, SELN_FN_COPY   = 6,
    SELN_FN_OPEN  = 7,  SELN_FN_PASTE  = 8,
    SELN_FN_FIND  = 9,  SELN_FN_CUT = 10
}	Seln_function;

/* for historic reason: */
#define SELN_FN_PUT SELN_FN_COPY
#define SELN_FN_GET SELN_FN_PASTE
#define SELN_FN_DELETE SELN_FN_CUT
...
typedef struct {
    int                 pid;
    int                 unused_program;
    struct sockaddr_in  unused_tcp_address;
    struct sockaddr_in  unused_udp_address;
    char               *client;
}	Seln_access;

typedef struct __seln_holder {
    Seln_rank           rank;
    Seln_state          state;
    Seln_access         access;
}	Seln_holder;
...
typedef struct __seln_func_buffer {
...
struct __seln_request;

typedef struct	{
    Seln_result	       (*consume)(struct __seln_request *);
    char		*context;
}	Seln_requester;

typedef struct __seln_request {
...
EXTERN_FUNCTION (Seln_request *	selection_ask, (Xv_Server server, Seln_holder *holder, DOTDOTDOT)) _X_SENTINEL(0);
...
EXTERN_FUNCTION (void selection_init_request, (Xv_Server server, Seln_request *buffer, Seln_holder *holder, DOTDOTDOT)) _X_SENTINEL(0);
...
EXTERN_FUNCTION (Seln_result selection_query, (Xv_Server server, Seln_holder *holder, Seln_result (*reader)(Seln_request *), char *context, DOTDOTDOT)) _X_SENTINEL(0);
...
EXTERN_FUNCTION (Seln_client seln_create, (void (*func_proc)(Xv_Server, Seln_function_buffer *), Seln_result (*request_proc)( Seln_attribute, Seln_replier_data *, int), char *client_data));
...
EXTERN_FUNCTION (Seln_request *seln_ask, (Seln_holder *holder, DOTDOTDOT)) _X_SENTINEL(0);
...
EXTERN_FUNCTION (Seln_result seln_query, (Seln_holder *holder, Seln_result (*reader)(Seln_request *), char *context, DOTDOTDOT)) _X_SENTINEL(0);
...
EXTERN_FUNCTION (void seln_init_request, (Seln_request *buffer, Seln_holder *holder, DOTDOTDOT)) _X_SENTINEL(0);
