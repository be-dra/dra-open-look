/*      @(#)io_stream.h 20.14 93/06/28 SMI  DRA: $Id: io_stream.h,v 4.1 2024/03/28 18:02:44 dra Exp $     */
...
*** NOT IMPLEMENTED YET ***
				 ...
				 */
	char	*(*str_fgets)(char *, int, struct Stream *in);
	/*
...
int stream_peekc(struct Stream	*);
/* int stream_peekc(in)
 *	struct Stream	*in;
...
char *stream_fgets(char *, int, struct Stream *);
...
int stream_chars_avail(struct Stream *);
/* int chars_avail(in)
 *	struct Stream	*in;
 */

void stream_ungetstr(char *, struct Stream *);
...
void append_streams(struct Stream *, struct Stream *);
...
char *stream_get_token (struct Stream *, char *, enum CharClass (*charproc)(char));
...
void xv_skip_over(struct Stream	*, enum CharClass (*charproc) (char));
...
#ifndef WHITESPACE
#define WHITESPACE
extern enum CharClass xv_white_space(char c);
...
char * stream_get_sequence (struct Stream *, char *, struct CharAction (*charproc) (char));
...
#ifndef EVERYTHING
#define EVERYTHING
struct CharAction xv_everything(char);
...
int stream_putc(char, struct Stream *);
/* stream_putc(c, out)
 *	char		c;
 *	struct Stream	*out;
...
void stream_puts(char *, struct Stream *);
/* stream_puts(s, out)
 *	char		*s;
 *	struct Stream	*out;
 * analagous to puts:
...
void stream_fputs(char *, struct Stream *);
/* stream_fputs(s, out)
 *	char		*s;
 *	struct Stream	*out;
...
struct posrec stream_get_pos(STREAM *);
...
void stream_flush(struct Stream	*);
...
struct Stream *string_input_stream(char *, struct Stream *);
...
struct Stream *string_output_stream(char *, struct Stream *);
...
struct Stream *xv_file_input_stream(char *, FILE *);
...
struct Stream *xv_file_output_stream(char *, FILE *, Bool);
...
struct Stream *xv_filter_comments_stream(struct Stream *);
