...
no server defined (Defaults package)"),
	...
	xdefaults = XResourceManagerString(xv_default_display);
#else
	xdefaults = xv_default_display->xdefaults;
#endif

	if (xdefaults)
	    new_db = XrmGetStringDatabase(xdefaults);
    }
    if (new_db)
...
no server defined (Defaults package)"), NULL);
		return;
	}
