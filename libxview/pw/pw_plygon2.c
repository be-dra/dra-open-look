#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)pw_plygon2.c 20.28 89/07/31 DRA: RCS $Id: pw_plygon2.c,v 2.2 2023/12/14 19:42:47 dra Exp $";
#endif
#endif
...
Xv_public int
pw_polygon_2(struct pixwin *pw, int dx, int dy, int nbds, int npts[],
    struct pr_pos  *vlist, int op, struct pixrect *spr, int sx, int sy)
{
    ...
    GC              fill_gc;
    XGCValues       gc_val;
    unsigned long   gc_mask = GCTileStipXOrigin | GCTileStipYOrigin;
    short           i;
    short           depth;
    int             temp_total = 0;
    Server_image    tile;
