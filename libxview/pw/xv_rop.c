...
Xv_private void xv_set_gc_op(Display *display, Xv_Drawable_info *info, GC gc,
							int op, short fg_mode, int fg_bg)
{
...
Xv_private int xv_rop_internal(Display *display, Drawable d, GC gc,
			int x, int y, int width, int height, Xv_opaque src,
			int xr, int yr, Xv_Drawable_info *dest_info)
{
	...
	if (PR_NOT_MPR(((Pixrect *) src))) {
		...
		if (PR_IS_SERVER_IMAGE((Pixrect *) src)) {
			...
			XFillRectangle(display, d, gc, x, y,
								(unsigned)width, (unsigned)height);
		}
		else {
			/* src is a window */
			if (xv_depth(dest_info) == xv_depth(src_info)) {
				XCopyArea(display, src_d, d, gc, xr, yr,
...
Xv_private int xv_rop_mpr_internal(Display *display, Drawable d, GC gc, int x, int y, int width, int height, Xv_opaque src, int xr, int yr, Xv_Drawable_info *dest_info, int mpr_bits)
{
	...
	if (src_depth == 1) {
		if (!(ximage = xv_image_bitmap(dest_info))) {
			...
			visual = (Screen_visual *) xv_get(xv_screen(dest_info),
					SCREEN_DEFAULT_VISUAL);
			xv_image_bitmap(dest_info) = ximage =
					(XImage *) XCreateImage(display, visual->vinfo->visual, 1,
					XYBitmap, 0, (char *)mpr_d(((Pixrect *) src))->md_image, 0,
					0, MPR_LINEBITPAD, mpr_d(((Pixrect *) src))->md_linebytes);
			if (!ximage) {
				return (XV_ERROR);
			}
		}
		...
		if (!(ximage = xv_image_pixmap(dest_info))) {
			...
			xv_image_pixmap(dest_info) = ximage =
					(XImage *) XCreateImage(display, visual->vinfo->visual, 8,
					ZPixmap, 0, (char *)mpr_d(((Pixrect *) src))->md_image, 0,
					0, MPR_LINEBITPAD, mpr_d(((Pixrect *) src))->md_linebytes);
			if (!ximage) {
				return (XV_ERROR);
			}
	...
	if (src_depth == 1) {
		XPutImage(display, d, gc, ximage, xr, yr, x, y,
				(unsigned)MIN(width, ximage->width),
				(unsigned)MIN(height, ximage->height));
	}
	else {
		register int i, j;
		unsigned long index;
		...
		if (size > last_size) {
			if (data)
				xv_free(data);
			data = (unsigned char *)xv_malloc((size_t)size);
			last_size = size;
		}
		...
		for (i = 0; i < ximage->height; i++) {
			for (j = 0; j < ximage->bytes_per_line; j++) {
				...
				data[index] =
						cms->index_table[(unsigned char)image_data[index]];
			}
		}

		ximage->data = (char *)data;
		XPutImage(display, d, gc, ximage, xr, yr, x, y,
				(unsigned)MIN(width, ximage->width), (unsigned)MIN(height,
						ximage->height));
...
Pkg_private GC xv_find_proper_gc(Display *display, Xv_Drawable_info *info, int op)
{
	...
	if (!GC_CHAIN_KEY) GC_CHAIN_KEY = xv_unique_key();
	ops_private_gcs =
			(struct gc_chain **)xv_get(screen, XV_KEY_DATA, GC_CHAIN_KEY);
	if (!ops_private_gcs) {
		ops_private_gcs = (struct gc_chain **)xv_calloc((PW_NUM_OPS + 1),
											(unsigned)sizeof(struct gc_chain));
		xv_set(screen, XV_KEY_DATA, GC_CHAIN_KEY, ops_private_gcs, NULL);
	}
	...
	if (!gc_list) {
		gc_list = ops_private_gcs[op] =
				(struct gc_chain *)xv_calloc(1, (unsigned)sizeof(struct gc_chain));
		if (xv_in_fullscreen) {
			gv.subwindow_mode = IncludeInferiors;
			gc_list->gc = XCreateGC(display, xid, GCSubwindowMode, &gv);
		}
		else {
			gc_list->gc = XCreateGC(display, xid, 0L, 0);
		}
		...
		while (gcs) {
			if (gcs->depth == depth) {
				...
				if (gcs->clipping_set && !clip_xrects->count) {
					...
					gv.clip_mask = None;
					XChangeGC(display, gcs->gc, GCSubwindowMode | GCClipMask,
							&gv);
				}
				else {
				...
				if (clip_xrects->count && (gcs->xid != xid)) {
					XSetClipRectangles(display, gcs->gc, 0, 0,
							clip_xrects->rect_array, clip_xrects->count,
							Unsorted);
				...
				if (gcs->next) {
					 ...
					 */
					gcs->next = new =
							(struct gc_chain *)xv_malloc(sizeof(struct
									gc_chain));
					if (xv_in_fullscreen) {
						gv.subwindow_mode = IncludeInferiors;
						...
						new->gc = XCreateGC(display, xid, 0L, 0);
					}
					new->depth = depth;
					new->next = NULL;
...
Xv_public int xv_rop(Xv_opaque window, int x, int y, int width, int height,
				int op, Pixrect *pr, int xr, int yr)
{
	...
	if (pr == NULL) {
		...
		XFillRectangle(display, d, gc, x, y, (unsigned)width, (unsigned)height);
	}
	else {
...
#ifdef APPEARS_UNUSED
Xv_private void xv_invert_polygon(Xv_opaque pw,		/* pixwin */
    int npts,	/* number of points */
    struct pr_pos  *outline,	/* array of points */
    int x, int y,	/* top left corner of enclosing rectangle */
    int w, int h,	/* width and height of enclosing rectangle */
...
int xv_replrop(Xv_opaque window, int xw, int yw, int width, int height, int op,
							Pixrect *pr, int xr, int yr)
{
	...
	if (pr) {
		...
		if (xv_replrop_internal(display, info, d, gc, xw, yw, width,
			...
			xv_error(XV_NULL,
					ERROR_STRING,
					XV_MSG("xv_replrop: xv_replrop_internal failed"), NULL);
		}
	}
	else {
				...
				Pixrect *src, int xr, int yr, Xv_Drawable_info *dest_info)
{
	...
	if (!PR_NOT_MPR(src)) {
		 ...
		 */
		if (!(tile = XCreatePixmap(display, d, (unsigned)src->pr_width,
								(unsigned)src->pr_height,
								(unsigned)src->pr_depth))) {
			return (XV_ERROR);
		}
		if (!(tile_gc = XCreateGC(display, tile, 0L, 0))) {
	...
	XFillRectangle(display, d, gc, xw, yw, (unsigned)width, (unsigned)height);
	if (!PR_NOT_MPR(src)) {
		XFreePixmap(display, tile);
	}
	return (XV_OK);
...
#ifdef APPEARS_UNUSED
Xv_private GC xv_get_temp_gc(Display *display, unsigned int xid, int depth)
{
#ifndef SVR4
