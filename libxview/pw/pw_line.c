#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)pw_line.c 20.36 93/06/28 DRA: RCS $Id: pw_line.c,v 2.2 2023/12/14 19:42:52 dra Exp $";
#endif
#endif
...
Xv_public int pw_line(struct pixwin *pw, int x0, int y0, int x1, int y1,
    struct pr_brush *brush, Pr_texture *tex, int op)
{
    XGCValues       gc_val;
...
Pkg_private char *pw_short_to_char(short *pattern, int *len)
{
	...
	while (*p) {
		...
		if (*p > 255) {
			printf(XV_MSG
					("line texture pattern element %d is greater than 255! Shorten to 255\n"),
					l);
			*p = 255;
		}
		p++;
	...
	*len = l;
	c = c1 = xv_malloc((unsigned long)l);
	p = pattern;
	while (l--) {
		*c1++ = *p++;
	}
...
Xv_private void xv_draw_rectangle(
    Xv_opaque pw,
    ...
    int op)
{
    ...
    XDrawRectangle(display, xid, rect_gc, x, y, (unsigned)w, (unsigned)h);
}
