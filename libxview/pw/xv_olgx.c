...
Xv_private Cms xv_set_control_cms(Xv_Window window_public,
							Xv_Drawable_info *info, int cms_status)
{
	Cms cms;
	char cms_name[25];
	...
	CMS_CONTROL_CMS_NAME(cms_name, (unsigned long)visual, color);
									...
									Xv_Font text_font)
{
	...
	if ((xv_depth(info) > 1 && *three_d) || ui_style == SCREEN_UIS_2D_COLOR) {
	...
	if (control_cms) {
		...
		if (ui_style == SCREEN_UIS_2D_COLOR) {
			index_table = (unsigned long *)xv_get(cms, CMS_INDEX_TABLE);
			pixvals[OLGX_WHITE] = index_table[CMS_CONTROL_BG1];
	...
	if (three_d_state == OLGX_3D_MONO) {
		...
		if (stipple_pixmaps[0] == (Pixmap) NULL) {
			...
			xv_set(screen,
					XV_KEY_DATA, SCREEN_BG1_PIXMAP, stipple_pixmaps[0], 0);
		}
		if (stipple_pixmaps[1] == (Pixmap) NULL) {
			...
			xv_set(screen,
					XV_KEY_DATA, SCREEN_BG2_PIXMAP, stipple_pixmaps[1], 0);
		}
		if (stipple_pixmaps[2] == (Pixmap) NULL) {
			...
			xv_set(screen,
					XV_KEY_DATA, SCREEN_BG3_PIXMAP, stipple_pixmaps[2], 0);
		}
