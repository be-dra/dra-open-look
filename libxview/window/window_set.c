...
static Xv_opaque window_set_avlist_tier2(Xv_Window win_public, Attr_avlist attrs, int error, Rect *new_rect, Rect *old_rect, XSetWindowAttributes	*win_attrs, unsigned long *win_attrs_mask);
static     Xv_opaque window_set_avlist_tier3(Xv_Window win_public, Attr_avlist attrs, int error, Rect *new_rect, Rect *old_rect, XSetWindowAttributes	*win_attrs, unsigned long *win_attrs_mask);
			...
			switch (attrs[0]) {
					...
					if ((int)attrs[1]) {
						...
						if (win->dropSites) {
							...
							while ((node = (Win_drop_site_list *)
								...
								win_add_drop_interest(frame_private,
										node->drop_item);
							}
							/*
						...
						if (win->dropSites) {
							...
							while ((node = (Win_drop_site_list *)
									(XV_SL_SAFE_NEXT(node)))) {
								win_delete_drop_item(frame_private,
										node->drop_item, Win_Drop_Interest);
							}
							/*
							 * win_update_dnd_property() used to be called
				...
				case WIN_IGNORE_EVENT:{
						...
						if (attrs[0] == WIN_CONSUME_EVENT) {
							set_mask_bit(&im, (Window_input_event)attrs[1]);
						}
						else {
							unset_mask_bit(&im, (Window_input_event)attrs[1], win_public);
						}
				...
				case WIN_IGNORE_EVENTS:{
						...
						for (i = 1; attrs[i]; i++) {
							if (attrs[0] == WIN_CONSUME_EVENTS)
								set_mask_bit(&im, (Window_input_event)attrs[i]);
							else
								unset_mask_bit(&im, (Window_input_event)attrs[i], win_public);
						}
						window_set_event_mask(win_public, &im, &win_attrs,
								&win_attrs_mask);
						ADONE;
				...
				case WIN_PARENT:{
						...
						(void)update_rect(win, &old_rect, &new_rect);
						win_appeal_to_owner(FALSE, win,
								WIN_GET_RECT, (Xv_opaque) & rect,
								XV_NULL);
				...
				default:
					error = window_set_avlist_tier2(win_public, attrs, error,
							&new_rect, &old_rect, &win_attrs, &win_attrs_mask);
					break;
		...
		case WIN_TOP_LEVEL_NO_DECOR:
			if (!win->top_level && ! IS_FRAME(win_public)) {
				xv_error(win_public,
						ERROR_STRING,
						XV_MSG
						("WIN_TOP_LEVEL_NO_DECOR specified for non-top level non-frame window"),
						NULL);
			}
		...
		case WIN_FIT_HEIGHT:
			{
				...
				if (y <= 0) {
					window_sync_rect(win, old_rect, new_rect);
					y = new_rect->r_height;
				}
...
static Xv_opaque window_set_avlist_tier3(Xv_Window win_public,
		Attr_avlist attrs, int error, Rect *new_rect, Rect *old_rect,
		XSetWindowAttributes *win_attrs, unsigned long *win_attrs_mask)
{
	register Window_info *win = WIN_PRIVATE(win_public);
		...
		case WIN_RIGHT_OF:
			{
				...
				win_appeal_to_owner(TRUE, win, WIN_GET_RIGHT_OF,
						(Xv_opaque) (attrs[1]), (Xv_opaque) & vp);
				new_rect->r_left = vp;
				win->rect_info |= WIN_X_SET;
			}
		...
		case WIN_PERCENT_HEIGHT:
			...
			attrs[1] = (Window_attribute)
					((((int)xv_get(WIN_PUBLIC(win->owner), WIN_HEIGHT) -
									...
									win->owner->bottom_margin) * (int)attrs[1] /
		...
		case WIN_PERCENT_WIDTH:
			...
			attrs[1] = (Window_attribute)
					((((int)xv_get(WIN_PUBLIC(win->owner), WIN_WIDTH) -
									...
									win->owner->right_margin) * (int)attrs[1] /
...
static void update_rect(Window_info *win, Rect *old_rect, Rect *new_rect)
{
	...
	if (!rect_equal(old_rect, new_rect)) {
		win_appeal_to_owner(TRUE, win, WIN_ADJUST_RECT,
...
static void win_map(Window_info *win, int flag)
{
	...
	(void) win_appeal_to_owner(FALSE, win, win->map ? WIN_INSERT : WIN_REMOVE,
...
static void window_set_event_mask(Xv_Window window, Inputmask *im, XSetWindowAttributes *win_attrs, unsigned long *win_attrs_mask)
{
    ...
    if (xevent_mask != win->xmask) {
        window_grab_selectbutton(window, (unsigned long)win->xmask,
									(unsigned long)xevent_mask, im);
...
Xv_private void window_get_grab_flag(void)
{
	...
	if (!get_grab_resource) {
		...
		if (!follow_mouse) {
			 ...
			 * in my .Xdefault file.
			 * Then I changed this default value to False and remove the whole
			 * thing from the .Xdefaults.
			 *
			 * I have never encountered any problems with this ....
			 */
 ...
 */
static void window_grab_selectbutton(Xv_Window window, unsigned long oldmask,
						unsigned long newmask, Inputmask *newim)
{
	Xv_Drawable_info *info;
	Bool kbd_use = False;
	...
	if (xv_get(window, WIN_IS_IN_FULLSCREEN_MODE)) return;
	...
	if ((newmask & FocusChangeMask) && (kbd_use) && (!fullscreendebug)) {
		...
		grab_button_mask = ButtonPressMask | ButtonReleaseMask |
				(newxmask & EnterWindowMask) | (newxmask & LeaveWindowMask);
		if (newxmask & ButtonMotionMask)
			grab_button_mask |= Button1MotionMask;

		XGrabButton(xv_display(info), Button1, (int)None, xv_xid(info), FALSE,
				(unsigned)grab_button_mask, GrabModeSync, GrabModeSync, None,
				None);
	}
	else if (oldmask & FocusChangeMask)
		XUngrabButton(xv_display(info), Button1, (int)None, xv_xid(info));
}
...
Xv_private void window_release_selectbutton(Xv_Window window, Event *event)
{
	...
	if (event_action(event) == ACTION_SELECT &&
			event_is_down(event) &&
			(win->xmask & FocusChangeMask) && (!fullscreendebug))
	{
		Xv_Drawable_info *info;
