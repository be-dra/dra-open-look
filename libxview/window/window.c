 ...
 */
static int window_init(Xv_Window parent_public, Xv_Window win_public, Attr_avlist avlist, int *u)
{
	Window_info *parent;
	...
	int border = FALSE, no_decor = FALSE,
			convert_cu = FALSE, inherit_colors = WIN_NOT_DEFINED;
	Display *display;
	...
	if (is_root) {
		...
		if (!parent->layout_proc) {
			eexit(XV_MSG("Parent window does not support children"), error_msg);
		}
	...
	if (!is_root) {
		...
		window_sync_rect(win, &default_rect, &win->cache_rect);
		...
		vinfo_template.visualid =
				XVisualIDFromVisual(DefaultVisual(display, xv_get(screen,
								SCREEN_NUMBER)));
		vinfo_mask = VisualIDMask;
	}
	...
	if (vinfo_mask) {
		visual = (Screen_visual *) xv_get(screen, SCREEN_VISUAL, vinfo_mask,
				&vinfo_template);
		if (visual) {
			xv_visual(info) = visual;
			if (!is_root &&
					(xv_visual(info)->vinfo->visualid !=
							xv_visual(parent_info)->vinfo->visualid))
				win->inherit_colors = FALSE;
		}
	}
	...
	if (!xv_visual(info)) {
		eexit(XV_MSG("Window creation failed to get new visual"), error_msg);
	}
	...
	if (win->inherit_colors) {
		...
		sprintf(cms_name, "xv_default_cms_for_0x%lx",
				xv_visual(info)->vinfo->visualid);
		xv_cms(info) =
				(Cms) xv_find(screen, CMS, CMS_NAME, cms_name, XV_VISUAL,
				xv_visual(info)->vinfo->visual, CMS_DEFAULT_CMS, TRUE, CMS_SIZE,
				2, CMS_NAMED_COLORS, "white", "black", NULL, NULL);
		xv_cms_bg(info) = 0;
	...
	if (notify_set_event_func(win_public,(Notify_func)window_default_event_func,
					NOTIFY_SAFE) == NOTIFY_FUNC_NULL) {
		eexit(XV_MSG("notify_set_event_func failed in window creation"),
				error_msg);
		/*
		 * BUG: do we need this here? (void)notify_remove(win_public);
		 */
	}
	win->notify_safe_event_proc = (void (*)(Xv_window, Event *))window_default_event_func;
	...
	win->notify_immediate_event_proc = (void (*)(Xv_window,Event *))window_default_event_func;
...
Pkg_private XID window_new(Display *display, Xv_opaque screen, Window_info *win, Colormap cmap_id, Xv_Drawable_info *parent_info)
{
	...
	if (win->cursor) {
        ...
        new_window = XCreateWindow(display, xv_xid(parent_info),
		        win->cache_rect.r_left, win->cache_rect.r_top,
		        (unsigned)win->cache_rect.r_width,
				(unsigned)win->cache_rect.r_height,
		        (unsigned)(win->has_border ? WIN_DEFAULT_BORDER_WIDTH : 0),
		        (int)(win->input_only ? 0 : xv_depth(info)),
		        (unsigned)(win->input_only ? InputOnly : CopyFromParent),
...
void xv_main_loop(Xv_Window win_public)
{
	...
	if (xv_get(win_public, XV_IS_SUBTYPE_OF, FRAME_BASE)) {
		xv_set(xv_server(info), SERVER_BASE_FRAME, win_public, NULL);
	}
	/*
	...
	/* das hier habe ich eingebaut, weil es auf der Mehrprozessorkiste
	 * ab und zu zu dem Effekt kam, dass zwar beim Prozessstart das
	 * Fenster sichtbar wird, aber offenbar keine Expose-Events
	 * verarbeitet werden, d.h. das Fenster 'malt sich nicht'. Erst wenn man
	 * irgendeinen Event ausloest (Mausclick, Fenster verschieben o.ae.)
	 * malt sich das Fenster. Der Verdacht war, dass bereits Expose-Events
	 * in der Xlib-Queue stecken, aber der notify_start sich einfach auf
	 * 'select' stellt...
	 */
	xv_set(xv_server(info), SERVER_SYNC_AND_PROCESS_EVENTS, NULL);
    ...
    Xv_Window       win;
{
     ...
     * the root window.
     */
    while ((grand_owner = xv_get(owner, WIN_OWNER))) {
	/*
...
static int window_destroy_win_struct(Xv_Window win_public, Destroy_status status)
{
		...
		case DESTROY_CLEANUP:
			{
				 ...
				 */
				notify_remove(win_public);
				if (win->owner && win->owner->layout_proc)
					(win->owner->layout_proc) (WIN_PUBLIC(win->owner),
							win_public, WIN_DESTROY, XV_NULL, XV_NULL, XV_NULL,
							XV_NULL, XV_NULL);
				...
				if (win->dropSites) {
					...
					while ((dropSite = (Win_drop_site_list *)dropSite_save.next)) {
