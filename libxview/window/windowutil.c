...
/* instead of including seln_impl */
Xv_private void selection_agent_clear(Xv_Server, XSelectionClearEvent *);
...
Pkg_private int win_appeal_to_owner(int adjust, Window_info *win, Window_layout_op op, Xv_opaque d1, Xv_opaque d2)
{
	...
	if (!win->owner) {
		...
		window_layout(WIN_PUBLIC(win), WIN_PUBLIC(win), op, d1, d2, XV_NULL,
				XV_NULL, XV_NULL);
		return adjust;
	}
	if (win->owner && win->owner->layout_proc) {
		(win->owner->layout_proc) (WIN_PUBLIC(win->owner), WIN_PUBLIC(win),
				op, d1, d2, XV_NULL, XV_NULL, XV_NULL);
	...
	switch (event_action(event)) {
		...
		case KBD_DONE:{
				...
				if (win->softkey_flag) {
					...
					/* send a _OL_SOFTKEY_LABELS message asking the
					 * soft key process to update its labels
					 */
					...
					if (sfk_win != None)
						xv_send_message(win_public, sfk_win,
								"_OL_SOFTKEY_LABELS", 32, ldata,
								(unsigned)sizeof(ldata));
				}
			}
			break;
		...
		case SEL_CLEAR:{
				...
				xv_sel_handle_selection_clear(clear_event);
				break;
			}
...
Xv_private void window_set_client_message(Xv_Window window, XClientMessageEvent *msg)
{
    ...
    XV_BCOPY (msg->data.b, win->client_message.data.b, (size_t)WIN_MESSAGE_DATA_SIZE);
}
...
Xv_private void window_set_cache_rect(Xv_Window window, Rect *rect)
{
	...
	if ((win->rect_info & (int)WIN_X_SET) && (rect->r_left != EMPTY_VALUE)) {
	...
	if ((win->rect_info & (int)WIN_Y_SET) && (rect->r_top != EMPTY_VALUE)) {
		if (WIN_DEFAULT_RECT(&win->cache_rect)) {
			change_mask |= CWY;
	...
	if ((win->rect_info & (int)WIN_WIDTH_SET) && (rect->r_width != EMPTY_VALUE)) {
		if (WIN_DEFAULT_RECT(&win->cache_rect)) {
			change_mask |= CWWidth;
	...
	XConfigureWindow(xv_display(info), xv_xid(info), (unsigned)change_mask,
								&changes);
	...
	if ((change_mask & (CWWidth | CWHeight | CWY | CWX)) &&
		...
		/* habt ihr wohl vergessen, Freunde */
    	cevent.border_width = win->has_border;
		...
		if (win->being_rescaled) {
			...
			win_post_event_arg(window, &event, NOTIFY_IMMEDIATE,
					(unsigned long)win->scale, NOTIFY_COPY_NULL,
					NOTIFY_RELEASE_NULL);
		}
		else
...
Xv_private void window_set_border(Xv_object window, int width)
{
    ...
    DRAWABLE_INFO_MACRO(window, info);
    XSetWindowBorderWidth(xv_display(info), xv_xid(info), (unsigned)width);
}
...
Xv_private void window_outer_to_innerrect(Xv_Window window, Rect *rect)
{
    ...
    if (win->has_border) rect_borderadjust(rect, WIN_DEFAULT_BORDER_WIDTH);
    ...
    int *new_width, int *new_height)
{
    ...
    /*
     * vp = old_rect.r_height;
	 * vp -= win->top_margin + win->bottom_margin;
     * rows = vp / (actual_row_height(win) + win->row_gap));
	 * vp = * old_rect.r_width;
	 * vp -= win->left_margin + win->right_margin;
	 * cols = * (vp / (actual_column_width(win) + win->column_gap));
     */
    ...
    *new_height = rows * (win->row_height + win->row_gap)
    ...
    win->column_width = xv_get(par->font, FONT_DEFAULT_CHAR_WIDTH);
    *new_width = cols * (win->column_width + win->column_gap)
...
Xv_private void window_add_to_rect_list(Window_rescale_rect_obj *rect_obj_list,
								Xv_Window window, Rect *rect, int i)
{
    rect_obj_list[i].sw = window;
    rect_obj_list[i].old_rect = *rect;
}

Xv_private int window_rect_equal_ith_obj(Window_rescale_rect_obj *rect_obj_list,
											Rect *rect, int i)
{
...
Xv_private void window_adjust_rects(Window_rescale_rect_obj *rect_obj_list,
					Xv_Window parent_public, int num_elems,
					int parent_width, int parent_height)
{
	...
	for (i = 0; i < num_elems; i++) {
		window_calculate_new_size(parent_public, rect_obj_list[i].sw,
		...
		window_inner_to_outerrect(rect_obj_list[i].sw,
				&(rect_obj_list[i].new_rect));
	...
	for (this_sw = 0; this_sw < num_elems; this_sw++) {
		...
		adjust_rect_obj(num_elems, this_sw, rect_obj_list, parent_width,
				parent_height);
...
static void adjust_rect_obj(int num_elems, int this_sw, Window_rescale_rect_obj *rect_obj_list, int parent_width, int parent_height)
{
	int sw_left_of = 0, left_coord = 0, sw_above = 0, top_coord = 0;
		...
		if (this_sw != another_sw) {
			...
			if (rect_right_of(&(temp_sw->old_rect), &(sw_to_adjust->old_rect))) {
				if (left_coord < temp_sw->old_rect.r_left) {
					 ...
					 */
					adjust_rect_obj(num_elems, another_sw, rect_obj_list,
							parent_width, parent_height);
					left_coord = temp_sw->old_rect.r_left;
					sw_left_of = another_sw;
				}
			}
			else if (rect_right_of(&(sw_to_adjust->old_rect),
							&(temp_sw->old_rect)))
				nothing_right_of = FALSE;

			if (rect_below(&(temp_sw->old_rect), &(sw_to_adjust->old_rect))) {
			...
			else if (rect_below(&(sw_to_adjust->old_rect),
							&(temp_sw->old_rect)))
				nothing_below = FALSE;
			...
			if (sw_above) {
				sw_to_adjust->new_rect.r_top +=
						rect_obj_list[sw_above].y_change +
						rect_obj_list[sw_above].height_change;
			...
			if (sw_left_of) {
				sw_to_adjust->new_rect.r_left +=
						rect_obj_list[sw_left_of].x_change +
						rect_obj_list[sw_left_of].width_change;
...
Xv_private int xv_get_external_data(Xv_object window, char *key, Xv_opaque *data, int *len, int *format)
	/* Number of bytes in the external data */
{
	...
	if ((prop_name = xv_get(srv, SERVER_ATOM, key)) != None) {
		if (XGetWindowProperty(display, DefaultRootWindow(display),
						prop_name, 0L, 1L, FALSE, AnyPropertyType, &actual_type,
			...
			*len = nitems * (*format) / 8;
			if (sizeof(key) != sizeof(int)) {
				...
				if (*format == 32) {
					fprintf(stderr,
							"%s-%d: wrong length calculation, aborting\n",
							__FILE__, __LINE__);
					abort();
				}
			}
			XV_BCOPY(prop, (char *)data, (size_t)*len);
			return (*len);
		}
	}
	return (0);
...
Xv_private int xv_write_external_data(Xv_object window, char *key, int format,
							Xv_opaque *data, int len)
	/* lenght of data in the format */
{
	Xv_server srv = XV_SERVER_FROM_WINDOW(window);
	Display *display = XV_DISPLAY_FROM_WINDOW(window);
	Atom atom;

	if ((atom = xv_get(srv, SERVER_ATOM, key)) != None) {
	...
	/* len: Number of bytes in data */
{
	Xv_server srv = XV_SERVER_FROM_WINDOW(window);
	Display *display = XV_DISPLAY_FROM_WINDOW(window);
	Atom registration = xv_get(srv, SERVER_ATOM, msg_type);
	XClientMessageEvent client_event;

	client_event.type = ClientMessage;
	client_event.message_type = registration;
	client_event.format = format;
	memset(client_event.data.b, 0, sizeof(client_event.data.b));
	XV_BCOPY((char *)data, client_event.data.b, (size_t)len);
...
/* window that wants to grab quick selection keys */
{
    Xv_Drawable_info *info;
    int		    keycode;
