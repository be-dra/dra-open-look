...
Xv_private void window_set_cms_name(Xv_Window win_public, char *new_name)
{
	...
	cms = xv_find(xv_screen(info), CMS,
			...
			XV_AUTO_CREATE, FALSE, CMS_NAME, new_name, NULL);
	if (cms != XV_NULL) {
		if (cur_name) {
			free(cur_name);
		...
		if (xv_cms(info) != cms)
			window_set_cms(win_public, cms, 0, (int)xv_get(cms, CMS_SIZE,
							0) - 1);
	}
	else {
		if (cur_name) {
...
with a static visual"), ERROR_PKG, WINDOW,
				NULL);
		return;
	}
	if (!xv_cms_name_key) {
	...
	if (cur_name != NULL) {
		cms = (Cms) xv_find(xv_screen(info), CMS,
				XV_VISUAL, visual,
				XV_AUTO_CREATE, FALSE, CMS_NAME, cur_name, NULL);
		if (cms == XV_NULL) {
			cms = xv_create(xv_screen(info), CMS,
					...
					CMS_COLOR_COUNT,
					cms_data->rgb_count, CMS_COLORS, cms_colors, NULL);
		}
		else {
			xv_set(cms,
					CMS_INDEX, cms_data->index,
					CMS_COLOR_COUNT, cms_data->rgb_count,
					CMS_COLORS, cms_colors, NULL);
		}
	}
	else {
		cms = xv_cms(info);
	...
	window_set_cms(win_public, cms, 0, (int)xv_get(cms, CMS_SIZE, 0) - 1);
	if (cur_name) {
		xv_free(cur_name);
		xv_set(win_public, XV_KEY_DATA, xv_cms_name_key, NULL, NULL);
	}
