...
/* -------------------- XView Functions  -------------------- */
static int panel_list_init(Xv_Window parent, Panel_item panel_list_public,
										Attr_avlist avlist, int *u)
{
	...
	/* da muss einer den Font verdreht haben... */
	DRAWABLE_INFO_MACRO(parent, info);
  	gc_list = (GC *)xv_get(xv_screen(info), SCREEN_OLGC_LIST, parent);
	XSetFont(xv_display(info), gc_list[SCREEN_TEXT_GC],
						xv_get(dp->font, XV_XID));
	...
	strcpy(textname, "PanelListText");
	for (attrs = avlist; *attrs; attrs = attr_next(attrs)) {
		if ((int)attrs[0] == XV_INSTANCE_NAME) {
			char *listname = (char *)attrs[1];
			sprintf(textname, "%sText", listname);
		}
		else if ((int)attrs[0] == PANEL_LIST_SHOW_PROPS) {
			dp->show_props = (int)attrs[1] ? TRUE : FALSE;
    ...
    dp->text_item = xv_create(dp->parent_panel, PANEL_TEXT,
					XV_INSTANCE_NAME, textname,
					PANEL_ITEM_OWNER, PANEL_LIST_PUBLIC(dp),
					PANEL_NOTIFY_STRING, "\r",   /* RETURN only */
					PANEL_VALUE_DISPLAY_LENGTH, 1, /* some artificial value 
								* that doesn'taffect the width of the panel
								* (see shrink_to_fit) */
					XV_KEY_DATA, PANEL_LIST_EXTENSION_DATA, dp,
					XV_HELP_DATA, "xview:plist_textfield",
					XV_SHOW, FALSE,
					NULL);
...
static Xv_opaque panel_list_set_avlist(Panel_item panel_list_public, Attr_avlist     avlist)
{
	...
	Item_info *ip = ITEM_PRIVATE(panel_list_public);
	int max_glyph_width;
	Row_info *next = NULL;	/* next node in list */
		...
		switch ((int)avlist[0]) {
			...
			case PANEL_LIST_ROW_VALUES:{
					...
					for (ii = 0; ii < count; ++ii) {
						 ...
						 */
						if (entry->mask_glyph)
							set_row_mask_glyph(dp, node,
									(Pixrect *) entry->mask_glyph);
						else
			...
			case PANEL_CHOOSE_NONE:
				dp->choose_none = (int)avlist[1] ? TRUE : FALSE;
				break;
			case PANEL_CHOOSE_ONE:
				dp->choose_one = (int)avlist[1] ? TRUE : FALSE;
				break;
			...
			case PANEL_ITEM_COLOR:
				if (dp->list_sb)
					xv_set(dp->list_sb,
							WIN_FOREGROUND_COLOR, ip->color_index, NULL);
				break;
			...
			case PANEL_ITEM_MENU:
				if (dp->edit_mode) {
					if (dp->edit_menu != (Menu) avlist[1]) {
						...
						if (dp->edit_menu)
							xv_set(dp->edit_menu, XV_KEY_DATA,
									PANEL_LIST_EXTENSION_DATA, dp, NULL);
					...
					if (dp->read_menu != (Menu) avlist[1]) {
						...
						if (dp->read_menu)
							xv_set(dp->read_menu, XV_KEY_DATA,
									PANEL_LIST_EXTENSION_DATA, dp, NULL);
			...
			case PANEL_LIST_MODE:
				...
				if ((Panel_list_mode) avlist[1] == PANEL_LIST_READ &&
						dp->edit_mode)
					enter_read_mode(dp->read_menu, XV_NULL);
				else if ((Panel_list_mode) avlist[1] == PANEL_LIST_EDIT &&
						!dp->edit_mode)
					enter_edit_mode(dp->edit_menu, XV_NULL);
				break;
			...
			case PANEL_LIST_INSERT:
				if (!dp->insert_delete_enabled) {
					xv_error(panel_list_public,
							ERROR_STRING,
							XV_MSG
							("PANEL_LIST insertions are currently disabled"),
			...
			case PANEL_LIST_INSERT_STRINGS_WCS:
				...
				if (dp->list_sb)
					xv_set(dp->list_sb,
							SCROLLBAR_OBJECT_LENGTH, dp->nrows, NULL);
				break;
			...
			case PANEL_LIST_DELETE:
				if (!dp->insert_delete_enabled) {
					xv_error(panel_list_public,
							ERROR_STRING,
							XV_MSG
							("PANEL_LIST deletions are currently disabled"),
							ERROR_PKG, PANEL, NULL);
					break;
				}
				which_row = (int)avlist[1];
			...
			case PANEL_INACTIVE:
				if ((int)avlist[1] && ip->panel->kbd_focus_item == ip) {
					if (dp->initialized)
						panel_list_paint(panel_list_public, PANEL_NONE);
					ip->panel->kbd_focus_item =
							panel_next_kbd_focus(ip->panel, TRUE);
					panel_accept_kbd_focus(ip->panel);
				}
				if (dp->list_sb)
					xv_set(dp->list_sb, SCROLLBAR_INACTIVE, (int)avlist[1], NULL);
				break;
			...
			case PANEL_READ_ONLY:
				dp->read_only = (int)avlist[1] ? TRUE : FALSE;
				break;
			...
			case PANEL_VALUE_STORED_LENGTH:
				...
				xv_set(dp->text_item,
						PANEL_VALUE_STORED_LENGTH, (int)avlist[1], NULL);
				break;
			...
			case PANEL_VALUE_STORED_LENGTH_WCS:
				...
				xv_set(dp->text_item,
						PANEL_VALUE_STORED_LENGTH_WCS, avlist[1], NULL);
				break;
#endif

			case XV_SHOW:
				if (dp->text_item_row)
					xv_set(dp->text_item, XV_SHOW, (int)avlist[1], NULL);
				if (dp->list_sb)
					xv_set(dp->list_sb, XV_SHOW, (int)avlist[1], NULL);
				break;
	...
	if (reset_rows && dp->initialized) {
		...
		xv_set(dp->list_sb,
				...
				XV_HEIGHT, dp->list_box.r_height, NULL);
	...
	if (dp->initialized) {
		switch (ip->layout) {
			...
			case PANEL_VERTICAL:
				...
				ip->value_rect.r_top = rect_bottom(&ip->label_rect) + 1 +
						(ip->label_rect.r_height
									? panel_label_y_gap(ip->panel)
									: 0);
				break;
			default:
				break;
		...
		if (dp->TITLE) {
			dp->title_rect.r_height =
					(int)xv_get(panel_list_public, PANEL_LEVEL_DOT_HEIGHT) +
					(int)xv_get(ip->panel->bold_font,
					FONT_DEFAULT_CHAR_HEIGHT) + PANEL_LIST_ROW_GAP;
			size = XV_PF_TEXTWIDTH((int)strlen(dp->TITLE),
					ip->panel->bold_font,
					dp->TITLE);
			dp->title_rect.r_width =
					LIST_BOX_BORDER_WIDTH * 2 + ROW_MARGIN * 2 + size.x;
		}
		else
		...
		if (xv_end_create) {
			Panel pan = xv_get(panel_list_public, XV_OWNER);
			/*
			...
			if (!dp->read_menu) {
				char *listname, edmenu[200], selmenu[200];

				listname = (char *)xv_get(panel_list_public, XV_INSTANCE_NAME);
				sprintf(edmenu, "%sEditMenu", listname ? listname:"PanelList");
				sprintf(selmenu, "%sSelMenu", listname ? listname:"PanelList");
				...
				dp->read_menu = xv_create(server, MENU,
						...
						MENU_ITEM,
							...
							XV_HELP_DATA, dp->choose_one ?
										"xview:plist_locate_choice" :
										"xview:plist_locate_next_choice",
				...
				if (!dp->choose_one)
					xv_set(dp->read_menu,
							MENU_APPEND_ITEM, xv_create(XV_NULL, MENUITEM,
								MENU_STRING, XV_MSG("Clear All Choices"),
								MENU_ACTION, clear_all_choices,
								XV_HELP_DATA, "xview:plist_clear_all_choices",
								MENU_RELEASE,
								NULL),
							NULL);
				if (!dp->read_only) {
					xv_set(dp->read_menu,
							MENU_APPEND_ITEM, xv_create(XV_NULL, MENUITEM,
									MENU_STRING, XV_MSG("Edit List"),
									MENU_ACTION, enter_edit_mode,
									XV_HELP_DATA, "xview:plist_edit_list",
									MENU_RELEASE,
									NULL),
							NULL);
					if (!dp->edit_menu) {
						char *change_label, *change_help;


						if (dp->show_props) {
							change_label = XV_MSG("Properties...");
							change_help = "xview:plist_props";
						}
						else {
							change_label = XV_MSG("Change");
							change_help = "xview:plist_change";
						}
						/*
						 * Set destroy flag
						 */
						...
						dp->edit_menu = xv_create(server, MENU,
							XV_INSTANCE_NAME, edmenu,
							XV_KEY_DATA, PANEL_LIST_EXTENSION_DATA, dp,
							MENU_TITLE_ITEM, XV_MSG("Scrolling List"),
							XV_KEY_DATA, MENU_DONE_PROC, NULL,
							MENU_ITEM,
								MENU_STRING, change_label,
								MENU_ACTION, change_proc,
								XV_HELP_DATA, change_help,
								NULL,
							MENU_ITEM,
								MENU_STRING, XV_MSG("Insert"),
								XV_HELP_DATA, "xview:plist_insert",
								MENU_PULLRIGHT, xv_create(server, MENU,
									XV_KEY_DATA, PANEL_LIST_EXTENSION_DATA, dp,
									MENU_ITEM,
										MENU_STRING, XV_MSG("Before"),
										MENU_ACTION, insert_proc,
										XV_KEY_DATA,PANEL_INSERT,INSERT_BEFORE,
										XV_HELP_DATA,"xview:plist_insert_before",
										NULL,
									MENU_ITEM,
										MENU_STRING, XV_MSG("After"),
										MENU_ACTION, insert_proc,
										XV_KEY_DATA, PANEL_INSERT, INSERT_AFTER,
										XV_HELP_DATA,"xview:plist_insert_after",
										NULL,
									NULL),
								NULL,
							MENU_ITEM,
								...
								XV_HELP_DATA, "xview:plist_delete",
								NULL,
							MENU_ITEM,
								MENU_STRING, "",
								MENU_FEEDBACK, FALSE,
								NULL,
							MENU_ITEM,
								MENU_STRING, XV_MSG("End Editing"),
								MENU_ACTION, enter_read_mode,
								XV_HELP_DATA, "xview:plist_end_edit",
								NULL,
							XV_SET_MENU, pan,
							NULL);
					}	/* if (!dp->edit_menu) */
			...
			xv_set(dp->list_sb, SCROLLBAR_OBJECT_LENGTH, dp->nrows, NULL);
		}
	}  /* if (dp->initialized) */
...
static Xv_opaque panel_list_get_attr(Panel_item panel_list_public,
						int *status, Attr_attribute which_attr, va_list valist)
{
	Panel_list_info *dp = PANEL_LIST_PRIVATE(panel_list_public);
	Row_info *node;
	int row;
	...
	switch (which_attr) {
		...
		case PANEL_ITEM_NTH_WINDOW:
			if (va_arg(valist, int) == 0) return (Xv_opaque) dp->list_sb;
			else return (Xv_opaque) NULL;
		...
		case PANEL_LIST_FIRST_SELECTED:
			for (node = dp->rows; node; node = node->next) {
				if (node->f.selected) return (node->row);
			}
		...
		case PANEL_LIST_CLIENT_DATA:
			row = va_arg(valist, int);
			node = find_or_create_nth_row(dp, row, FALSE);
			return (node ? (Xv_opaque) node->
					client_data : (Xv_opaque) XV_ERROR);
		...
		case PANEL_LIST_MASK_GLYPH:
			...
			node = find_or_create_nth_row(dp, row, FALSE);
			return (node ? (Xv_opaque) node->mask_glyph : (Xv_opaque) XV_ERROR);
		...
		case PANEL_LEVEL_DOT_HEIGHT:
			return (Xv_opaque) LEVEL_DOT_HEIGHT;
		...
		case PANEL_ITEM_MENU:
			return ((Xv_opaque) dp->edit_mode ? dp->edit_menu : dp->read_menu);
		...
		case PANEL_LIST_INACTIVE:
			row = va_arg(valist, int);
			node = find_or_create_nth_row(dp, row, FALSE);
			return (node ? (Xv_opaque) node->f.
					row_inactive : (Xv_opaque) XV_ERROR);
...

			{
				...
				for (ii = 0; ii < count; ++ii) {
					...
					if (which_attr == PANEL_LIST_ROW_VALUES_WCS)
						entry->string =
								(char *)_xv_get_wcs_attr_dup(&node->string);
					else
    				...
    				entry->inactive = node->f.row_inactive;
...
static int panel_list_destroy(Panel_item item_public, Destroy_status  status)
{
