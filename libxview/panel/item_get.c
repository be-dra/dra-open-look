...
Pkg_private Xv_opaque item_get_attr(Panel_item item_public, int *status, Attr_attribute which_attr, va_list valist)
{
	...
	switch (which_attr) {
		...
		case PANEL_ITEM_LAYOUT_ROLE:
			return (Xv_opaque) ip->layout_role;

		case PANEL_ITEM_LAYOUT_PROC:
			return (Xv_opaque) ip->layout_proc;

		case PANEL_ITEM_LABEL_BASELINE:
			/* compare panel_paint_label / panel_paint_image */
			{
				Panel_image *image = &ip->label;

				if (image->im_type == PIT_STRING) {
	    			int baseline = ip->label_rect.r_top
										+ panel_fonthome(image_font(image));

					return (Xv_opaque)baseline;
				}
			}
			return (Xv_opaque)0;
