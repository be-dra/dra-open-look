#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)item.c 20.56 93/06/28 DRA: $Id: item.c,v 4.5 2025/11/01 14:55:10 dra Exp $";
#endif
#endif
...
static int item_init(Xv_Window parent, Panel_item item_public, Attr_avlist avlist, int *u)
{
	...
	for (attrs = avlist; *attrs; attrs = attr_next(attrs)) {
		switch (attrs[0]) {
			...
			case PANEL_ITEM_Y_GAP:
				...
				break;
			case PANEL_ITEM_LAYOUT_ROLE:
				ip->layout_role = (Panel_item_role)attrs[1];
...
static int item_destroy(Panel_item item_public, Destroy_status  status)
{
