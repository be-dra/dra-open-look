...
#define	orig_TEXT_VALUE_GAP	6	/* padding on either side of text value */
#define	TEXT_VALUE_GAP	(ip->panel->item_x_offset-1)
...
static void slider_handle_event(Panel_item item_public, Event *event);
...
static void slider_paint(Panel_item item_public, Panel_setting);
...
static Panel_ops ops = {
    slider_handle_event,		/* handle_event() */
...
/* -------------------- XView Functions  -------------------- */
static int slider_init(Panel panel_public, Panel_item item_public,
...
static Xv_opaque slider_set_avlist(Panel_item item_public, Attr_avlist avlist)
{
	Attr_avlist attrs;
	register Item_info *ip = ITEM_PRIVATE(item_public);
			...
			case PANEL_ITEM_COLOR:
				if (dp->value_textitem)
					xv_set(dp->value_textitem,
							PANEL_ITEM_COLOR, (char *)*avlist++, NULL);
				else
					avlist++;
				break;
...
#else
		size = xv_pf_textwidth((int)strlen(dp->min_tick_string),
						ip->panel->std_font, dp->min_tick_string);
#endif /* OW_I18N */
...
#else
		size = xv_pf_textwidth((int)strlen(dp->max_tick_string),
				ip->panel->std_font, dp->max_tick_string);
#endif /* OW_I18N */
		...
		if (dp->flags & SHOWVALUE) {
			...
			if (dp->value_textitem) {
				xv_set(dp->value_textitem,
						...
						PANEL_VALUE_DISPLAY_LENGTH, dp->valuesize, NULL);
			}
			else {
				 ...
				 */
				char textname[200], *slname;

				slname = (char *)xv_get(item_public, XV_INSTANCE_NAME);
				sprintf(textname, "%sText", slname ? slname : "slider");
				dp->value_textitem =
			...
			/* Hide value */
			xv_set(dp->value_textitem, XV_SHOW, FALSE, NULL);
		}
	}
...
static Xv_opaque slider_get_attr(Panel_item item_public, int *status,
									Attr_attribute which_attr, va_list valist)
{
    register Slider_info *dp = SLIDER_PRIVATE(item_public);
#ifdef OW_I18N
...
static int slider_destroy(Panel_item item_public, Destroy_status  status)
{
...
static void apply_delta(Panel_item item_public, Event *event, int delta)
{
	Slider_info *dp = SLIDER_PRIVATE(item_public);
	...
	xv_set(PANEL_PUBLIC(ip->panel),
			WIN_MOUSE_XY, mouse_left, mouse_top, NULL);
}
...
static void slider_handle_event(Panel_item item_public, Event *event)
{
	if (event_action(event) == ACTION_WHEEL_FORWARD) {
		if (event_is_down(event)) {
			apply_delta(item_public, event, 1);
		}
		return;
	}
...
static void slider_update_preview(Panel_item item_public, Event *event)
{
    ...
    if (dp->flags & CONTINUOUS)
		(*mynotify) (ITEM_PUBLIC(ip), itoe(dp, dp->apparent), event);
}
...
static void slider_cancel_preview(Panel_item item_public, Event *event)
{
	...
	mynotify = (mynotify_t)ip->notify;
	if (dp->flags & CONTINUOUS)
	    (*mynotify) (ITEM_PUBLIC(ip), itoe(dp, dp->actual), event);
	paint_slider(ip, OLGX_UPDATE);
	if (dp->flags & SHOWVALUE) {
...
static void slider_accept_preview(Panel_item item_public, Event *event)
{
	...
	if (delta) {
		apply_delta(item_public, event, delta);
	}
	else
		paint_slider(ip, OLGX_UPDATE);
...
static void slider_paint(Panel_item item_public, Panel_setting u)
{
    CHAR            buf[10];
    Slider_info	   *dp = SLIDER_PRIVATE(item_public);
	...
	if (!hidden(tp))
	    (*tp->ops.panel_op_paint)(dp->value_textitem, PANEL_NO_CLEAR);
    }
	...
	PANEL_EACH_PAINT_WINDOW(ip->panel, pw)
		...
		XFillRectangle(xv_display(info), xv_xid(info),
			       ...
			       r->r_left, r->r_top,
				   (unsigned)r->r_width, (unsigned)r->r_height);
	    }
	PANEL_END_EACH_PAINT_WINDOW
	...
	PANEL_EACH_PAINT_WINDOW(ip->panel, pw)
		...
		XFillRectangle(xv_display(info), xv_xid(info),
			       gc_list[SCREEN_INACTIVE_GC],
			       r->r_left, r->r_top, (unsigned)r->r_width, (unsigned)r->r_height);
	    }
	PANEL_END_EACH_PAINT_WINDOW
    }
...
static void adjust_slider(Item_info	*ip, Event *event, Slider_adjust adjustment)
{
	...
	mynotify = (mynotify_t)ip->notify;
   (*mynotify) (ITEM_PUBLIC(ip), dp->print_value, event);
}
		...
		if ((olgx_state & OLGX_UPDATE) == 0) {
			...
			if (dp->nticks) {
				...
				if (inactive(ip)) {
					...
					XFillRectangle(xv_display(info), xv_xid(info),
							gc_list[SCREEN_INACTIVE_GC],
							dp->tickrect.r_left, dp->tickrect.r_top,
							(unsigned)dp->tickrect.r_width,
							(unsigned)dp->tickrect.r_height);
					XFillRectangle(xv_display(info), xv_xid(info),
							gc_list[SCREEN_INACTIVE_GC],
							dp->tickstringrect.r_left,
							dp->tickstringrect.r_top,
							(unsigned)dp->tickstringrect.r_width,
							(unsigned)dp->tickstringrect.r_height);
				}
			}
		}
	PANEL_END_EACH_PAINT_WINDOW
...
#else
	    {
		size = xv_pf_textwidth((int)strlen(dp->max_value_string),
				       ip->panel->std_font,
				       dp->max_value_string);
		dp->max_range_rect.r_width = size.x;
	    } else
...
#else
	    {
		size = xv_pf_textwidth((int)strlen(dp->min_value_string),
				       ip->panel->std_font,
				       dp->min_value_string);
		dp->min_range_rect.r_width = size.x;
	    } else
