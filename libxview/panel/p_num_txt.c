...
static void num_txt_paint( Panel_item item_public, Panel_setting u);
...
/* Local functions */
static int check_dimming(Num_text_info *dp);
static int get_value(Num_text_info *dp);
...
static Notify_value num_textitem_scroll_itimer_func(Panel_item item, int which);
...
/* -------------------- XView Functions  -------------------- */
/*ARGSUSED*/
static int panel_num_text_init(Panel panel_public, Panel_item item_public, Attr_avlist avlist, int *u)
{
	...
	strcpy(textname, "NumText");
	for (attrs = avlist; *attrs; attrs = attr_next(attrs)) {
		if ((int)attrs[0] == XV_INSTANCE_NAME) {
			char *numname = (char *)attrs[1];

			sprintf(textname, "%sText", numname);
			break;
		}
	}
	...
	dp->text_field = xv_create(panel_public, PANEL_TEXT,
			...
			PANEL_VALUE_DISPLAY_LENGTH, 12, PANEL_VALUE_STORED_LENGTH, 12,
...
static Xv_opaque panel_num_text_set_avlist(Panel_item	item_public, Attr_avlist avlist)
{
	...
	switch (avlist[0]) {
	  ...
	  case PANEL_VALUE_Y:
	  case XV_SHOW:
	    xv_set(dp->text_field, avlist[0], (int)avlist[1], NULL);
	    pos_change = TRUE;
	    break;
    ...
    if (value_change) set_value(dp, new_value);
    else if (range_change) set_value(dp, get_value(dp));

    if (check_for_dimming) check_dimming(dp);
...
static Xv_opaque panel_num_text_get_attr(Panel_item item_public,
					int *status, Attr_attribute which_attr, va_list valist)
{
	...
	switch (which_attr) {
		...
		case PANEL_MASK_CHAR_WC:
			...
			return xv_get(dp->text_field, (Attr32_attribute)which_attr);
...
static int panel_num_text_destroy(Panel_item item_public, Destroy_status status)
{
    Num_text_info *dp = NUM_TEXT_PRIVATE(item_public);
...
/* returns TRUE: button state changed; FALSE: button state not changed */
static int check_dimming(Num_text_info *dp)
{
...
static Bool notify_user(Num_text_info *dp, Event *event)
{
...
static void num_txt_paint_btn(register Item_info *ip, register Num_text_info *dp)
{
	...
	PANEL_EACH_PAINT_WINDOW(ip->panel, pw)
		...
		olgx_draw_numscroll_button(ginfo, xv_xid(info),
				dp->btn_rect.r_left, dp->btn_rect.r_top, dp->btn_state);
...
static Panel_setting text_notify_proc(Panel_item text_field, Event *event)
{
    Panel_item num_text = xv_get(text_field, XV_KEY_DATA, item_key);
