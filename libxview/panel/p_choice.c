#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_choice.c 20.140 93/06/28 DRA: $Id: p_choice.c,v 4.3 2024/10/06 20:07:04 dra Exp $";
#endif
#endif
 ...
 */
#define	CREATE_SET(n) ((unsigned int *) xv_calloc((unsigned int) (WORD(n) + 1), (unsigned)sizeof(unsigned int)))

#define	CLEAR_SET(set, n)	\
    (XV_BZERO((char *) (set), (int) (WORD(n) + 1) * sizeof(unsigned int)))

#define	COPY_SET(from_set, to_set, n)	\
    (XV_BCOPY((char *) (from_set), (char *) (to_set), (size_t) ((WORD(n) + 1) * sizeof(unsigned int))))
...
static void choice_paint(Panel_item item_public, Panel_setting u);
...
static void choice_images_to_menu_items(Item_info *ip, Panel_image images[],
							Menu_item mitems[], int last, int cyc);
...
static void layout_choices(register Item_info *ip)	/* the item */;
static void preview_choice(Item_info *ip, int new, Event *event, int paint);
static void update_display(register Item_info *ip, register int which_choice,
										int on, int preview);
static void update_item_rect(Item_info *ip);
static void update_value_rect(Item_info *ip);

static int menu_to_item_key = 0,
			done_proc_key = 0,
			value_key = 0,
			data_handle_key = 0;
...
static struct pr_size image_size(register Panel_image *image, register int *above_baseline, int max_wisth);
...
typedef struct {		/* data for a choice item */
	...
	int cyclic; /* DRA_CHANGED: only for display_level CURRENT */
	...
	int         orientation; /* DRA_CHANGED: only for display_level CURRENT */
...
static void paint_choice(Panel_info *panel, Item_info *ip, Choice_info *dp, int which_choice, int selected);

/* ========================================================================= */

/* -------------------- XView Functions  -------------------- */
/*ARGSUSED*/
static int choice_init(Panel panel_public, Panel_item item_public,
									Attr_avlist avlist, int *u)
{
	...
	if (data_handle_key == 0) {
		data_handle_key = xv_unique_key();
		done_proc_key = xv_unique_key();
		value_key = xv_unique_key();
	}
	...
	panel_make_image(ip->value_font, &dp->choices[0],
			PIT_STRING, panel_strsave_wc(_xv_null_string_wc), FALSE, FALSE);
#else
	panel_make_image(ip->value_font, &dp->choices[0],
	...
	{
		int cyclic = FALSE, ncols = 1;
	    Panel_setting display_level = PANEL_ALL;

		/* create-only attrs here! */
		for (; *avlist; avlist = attr_next(avlist)) {
			switch (avlist[0]) {
				case PANEL_ITEM_CYCLIC:
					cyclic = (int)avlist[1];
					ATTR_CONSUME(*avlist);
					break;
				case PANEL_DISPLAY_LEVEL:
	    			display_level = (Panel_setting)avlist[1];
					break;
		...
		if (cyclic) {
			if (display_level == PANEL_CURRENT && ncols == 1) {
				/* only then we accept PANEL_ITEM_CYCLIC ! */
				dp->cyclic = TRUE;
			}
		}
	}
...
static Xv_opaque choice_set_avlist(Panel_item item_public, Attr_avlist avlist)
{
	register Item_info *ip = ITEM_PRIVATE(item_public);
	...
	while ((attr = *avlist++)) {
		switch (attr) {
			...
			case PANEL_CHOICE_STRINGS:
				 ...
				 */
				temp_choices =
						(Xv_opaque *) xv_alloc_n(Xv_opaque, (num_choices + 1));
				for (count = 0; count < num_choices; count++) {
					temp_choices[count] =
							(Xv_opaque) _xv_mbstowcsdup((char *)choices[count]);
				}
			...
			case PANEL_CHOICE_STRINGS_WCS:
				...
				for (count = 0; count < num_choices; count++) {
					temp_choices[count] =
							(Xv_opaque) panel_strsave_wc((char *)
							choices[count]);
				}
				temp_choices[num_choices] = 0;
				choices = temp_choices;
			...
			case PANEL_CHOICE_IMAGE:
				...
				if (!modify_choice(ip, attr == PANEL_CHOICE_STRING ?
								PIT_STRING : PIT_SVRIM, which_choice,
								*avlist++))
					return (Xv_opaque) attr;
				choices_changed = TRUE;
				break;
			...
			case PANEL_CHOICES_BOLD:
				...
				for (which_choice = 0; which_choice <= dp->last; which_choice++)
					if (is_string(&dp->choices[which_choice]))
						image_bold(&dp->choices[which_choice]) =
								dp->choices_bold;
				choices_changed = TRUE;
				break;
			...
			case PANEL_DIRECTION:
				if ((Panel_setting) * avlist++ == PANEL_HORIZONTAL)
	...
	if ((dp->display_level ==PANEL_NONE || dp->display_level ==PANEL_CURRENT)
			&& !created(ip)
			&& !value_set)
	{
...
static Xv_opaque choice_get_attr( Panel_item      item_public, int            *status, Attr_attribute which_attr, va_list	    valist)
{
	register Choice_info *dp = CHOICE_PRIVATE(item_public);
	...
	switch (which_attr) {
		...
		case PANEL_VALUE:	/* ordinal value or set of values */
			return (Xv_opaque) choice_value(dp->choose_one, dp->value,
					dp->last);
		...
		case PANEL_ITEM_CYCLIC:
			/* this might be FALSE even though the application requested TRUE */
			return (Xv_opaque)dp->cyclic;

		case PANEL_ITEM_LABEL_BASELINE:
			/* compare rect.r_top in choice_paint */
			{
				int top;
				Panel_image *image = &ip->label;

				if (ip->layout == PANEL_HORIZONTAL)
					top = ip->value_rect.r_top;
				else top = ip->label_rect.r_top;

				/* I have not found out why I needed this: */
				top += 5;

				return (Xv_opaque)(top + panel_fonthome(image_font(image)));
			}
...
static int choice_destroy(Panel_item item_public, Destroy_status  status)
{
    register Choice_info *dp = CHOICE_PRIVATE(item_public);
...
static void choice_begin_preview(Panel_item self, Event *event)
{
	Item_info *ip = ITEM_PRIVATE(self);
    Choice_info *dp = CHOICE_PRIVATE(self);
	...
	if (dp->cyclic) {
		...
		/* mouse on abbrev button?  copied from find_choice */
		rect = ip->value_rect;
		rect.r_width = Abbrev_MenuButton_Width(ip->value_ginfo);
		if (rect_includespoint(&rect, x, y)) {
			int value, def = (int)xv_get(ip->menu, MENU_DEFAULT);

			if (def == 1) { /* next */
				value = (int)xv_get(self, PANEL_VALUE);

				value = ((value + 1) % (dp->last + 1));
			}
			else {
				value = def - 2;
			}
			preview_choice(ip, value, event, TRUE);
			return;
		}
	}
...
static void choice_accept_preview(Panel_item self, Event *event)
{
	int choice_nbr;
	Choice_info *dp = CHOICE_PRIVATE(self);
	Item_info *ip = ITEM_PRIVATE(self);
	int on;
	int value;
	typedef void (*mynotify_t)(Panel_item, int, Event *);
	mynotify_t mynotify;
	...
	/* same special handling as in choice_begin_preview */
	if (dp->cyclic) {
		Rect rect;
		register int x = event_x(event);	/* locator x */
		register int y = event_y(event);	/* locator y */
	...
	(*mynotify) (ITEM_PUBLIC(ip), value, event);
	dp->current = NULL_CHOICE;
}
...
static void choice_accept_menu(Panel_item item_public, Event *event)
{
	 ...
	 */
	xv_set(ip->menu,
			XV_KEY_DATA, data_handle_key, dp,
	...
	if (dp->orientation & OLGX_HORIZONTAL) {
		...
		int scal = (int)xv_get(ip->value_font, FONT_SCALE);
		int itemheight = Ascent_of_TextFont(ip->value_ginfo)
						+ Descent_of_TextFont(ip->value_ginfo)
						+ scalesizes[scal].heightadd;
		int defa = (int)xv_get(ip->menu, MENU_DEFAULT);

		position_rect.r_left += Abbrev_MenuButton_Width(ip->value_ginfo);
		position_rect.r_top -= scalesizes[scal].base + (defa*itemheight);
		...
		position_rect.r_top += 1;
	}

	ip->panel->status.current_item_active = TRUE;

	menu_show(ip->menu, paint_window, event,
			MENU_POSITION_RECT, &position_rect, MENU_PULLDOWN, TRUE, NULL);
}
...
static void choice_paint(Panel_item item_public, Panel_setting u)
{
	...
	switch (dp->display_level) {
		case PANEL_CURRENT:	/* draw the current choice */
			rect = ip->value_rect;
			rect.r_left +=
					Abbrev_MenuButton_Width(ip->value_ginfo) + ABSTK_X_GAP;
			rect.r_width -=
					Abbrev_MenuButton_Width(ip->value_ginfo) + ABSTK_X_GAP;
 ...
 */
static void note_next(Menu menu, Menu_item menu_item)
{
	Panel_item self = xv_get(menu, XV_KEY_DATA, menu_to_item_key);
	int value = (int)xv_get(self, PANEL_VALUE);
    Choice_info	*dp = CHOICE_PRIVATE(self);
	Event *event = (Event *) xv_get(menu, MENU_LAST_EVENT);
	Item_info *ip;

	ip = ITEM_PRIVATE(self);

	value = ((value + 1) % (dp->last + 1));

	preview_choice(ip, value, event, TRUE);
	choice_accept_preview(self, event);
}

static void note_mixed_menu(Menu menu, Menu_item menu_item)
{
	Panel_item item_public = xv_get(menu, XV_KEY_DATA, menu_to_item_key);
	int menu_value = xv_get(menu_item, XV_KEY_DATA, value_key);
	Event *event = (Event *) xv_get(menu, MENU_LAST_EVENT);
	Item_info *ip;
	...
	preview_choice(ip, menu_value, event, TRUE);
...
static void choice_create_menu(Item_info *ip)
{
	...
	if (ip->menu) {
		Menu_item mi;
		int mini = dp->cyclic ? 2 : 1; /* don't destroy Next */
		...
		Xv_server srv = XV_SERVER_FROM_WINDOW(PANEL_PUBLIC(ip->panel));
		if (menu_to_item_key == 0) menu_to_item_key = xv_unique_key();

		/* Create choice menu */
		if (dp->cyclic) {
			ip->menu = xv_create(srv, MENU_MIXED_MENU,
					MENU_NOTIFY_PROC, note_mixed_menu,
					XV_KEY_DATA, menu_to_item_key, ITEM_PUBLIC(ip),
					XV_INCREMENT_REF_COUNT,
					MENU_ITEM,
						MENU_CLASS, MENU_COMMAND,
						MENU_STRING, XV_MSG("Next"),
						MENU_NOTIFY_PROC, note_next,
						NULL,
					NULL);
		}
		else {
			ip->menu = xv_create(srv, MENU_CHOICE_MENU,
	...
	choice_images_to_menu_items(ip, dp->choices, dp->mitems, dp->last,
												dp->cyclic);
	if (dp->cyclic) {
		xv_set(ip->menu, MENU_DEFAULT, 1, NULL);
	}
	else {
		xv_set(ip->menu,
	...
	for (i = 0; i <= last; i++) {
		...
		switch (image_type(image)) {
			case PIT_STRING:
				...
				xv_set(ip->menu, MENU_APPEND_ITEM, mitems[i], NULL);
				break;
...
static void choice_menu_busy_proc( Menu	    menu)
{
    Choice_info *dp = (Choice_info *)xv_get(menu,XV_KEY_DATA,data_handle_key);
    Xv_Drawable_info *info;
...
static void choice_menu_done_proc(Menu menu, Xv_opaque result)
{
	...
	if ((dp->display_level == PANEL_NONE ||
		...
		PANEL_EACH_PAINT_WINDOW(ip->panel, pw)
			...
			olgx_draw_abbrev_button(ip->value_ginfo, xv_xid(info),
				...
				(dp->choice_rects[0].r_height -
						Abbrev_MenuButton_Height(ip->value_ginfo)) / 2,
				dp->orientation | (ip->panel->status.three_d ? OLGX_NORMAL :
	...
	orig_done_proc =
			(void (*)(Menu, Xv_opaque))xv_get(menu, XV_KEY_DATA, done_proc_key);
...
static int choice_number(register unsigned int *value_set, register int last_element)
{
    ...
    EACH_CHOICE(value_set, last_element, i) return i;
    return 0;
}
...
static struct pr_size image_size(register Panel_image *image, register int *above_baseline, int max_width)
{
	...
	size = xv_pf_textwidth((int)strlen(sp), font, sp);
...
static void layout_choices(register Item_info *ip)	/* the item */
{
    ...
    Rect           *rect = NULL;	/* each choice rect */
...
#else
		size = xv_pf_textwidth((int)strlen(image->im_value.t.text),
...
static int modify_choice(Item_info *ip, int type, int which_choice, Xv_opaque choice_info)
{
	...
	int num_choices, i = 0, old_slot;	/* counters */
	struct pr_size size;
		...
		for (i = 0; i <= dp->last; i++) {
			...
			if (i <= old_last) {
				...
				size = panel_make_image(is_string(&old_choices[old_slot]) ?
						image_font(&old_choices[old_slot]) : ip->panel->
						std_font, &dp->choices[i], PIT_STRING,
						panel_strsave_wc(_xv_null_string_wc), choice_is_bold,
						FALSE);
#else
				size = panel_make_image(ip->value_font, &dp->choices[i],
						PIT_STRING, (Xv_opaque) "", choice_is_bold, FALSE);
#endif /* OW_I18N */
	...
	if (dp->display_level == PANEL_NONE) {
		dp->choice_rects[i].r_width = Abbrev_MenuButton_Width(ip->value_ginfo);
		dp->choice_rects[i].r_height =
				Abbrev_MenuButton_Height(ip->value_ginfo);
	}
	else {
 ...
 * PANEL_MARKED.
 */
static void paint_choice(Panel_info *panel, Item_info *ip, Choice_info *dp,
							int which_choice, int selected)
{
	int color_index;
	Display *display;
	Panel_image *image;
	...
	if (dp->feedback == PANEL_MARKED) {
		...
		PANEL_EACH_PAINT_WINDOW(panel, pw)
			...
			olgx_draw_check_box(ip->value_ginfo, xv_xid(info), left,top,state);
		PANEL_END_EACH_PAINT_WINDOW
	...
	switch (image->im_type) {
		...
		case PIT_SVRIM:
					...
					dp->display_level == PANEL_CURRENT)
			{
				...
				PANEL_EACH_PAINT_WINDOW(panel, pw)
					panel_paint_svrim(pw, (Pixrect *) image_svrim(image),
						left, top, color_index, (Pixrect *) NULL);
				PANEL_END_EACH_PAINT_WINDOW
			}
			break;
	}
	...
	if (paint) {
		switch (dp->choose_one) {
			case TRUE:
				...
				/* mark the new choice */
				update_display(ip, new, dp->choose_none ? !new_is_on : TRUE,
						TRUE);
				break;
...
static int re_alloc_choices(Item_info *ip,int type,	Xv_opaque choices[])
{
    ...
    /* allocate the panel_image[] storage for the choices */
    if ((dp->choices = (Panel_image *)
	 xv_calloc((unsigned int) num_choices, (unsigned)sizeof(Panel_image))) == NULL)
	return 0;

    /* Allocate the Menu_item storage for each choice */
    if ((dp->mitems = (Menu_item *)
	 xv_calloc((unsigned int) num_choices, (unsigned)sizeof(Menu_item))) == NULL)
	return 0;

    /* allocate the enclosing rectangles for each choice */
    if ((dp->choice_rects = (Rect *)
	 xv_calloc((unsigned int) num_choices, (unsigned)sizeof(Rect))) == NULL)
	return 0;
 ...
 */
static void update_display(register Item_info *ip, register int which_choice, int on, int preview)
{
	register Choice_info *dp = CHOICE_FROM_ITEM(ip);
	Xv_Drawable_info *info;
	Xv_Window pw;
		...
		if (which_choice == NULL_CHOICE) {
			PANEL_EACH_PAINT_WINDOW(ip->panel, pw)
				...
				olgx_draw_abbrev_button(ip->value_ginfo, xv_xid(info),
					...
					(dp->choice_rects[0].r_height -
							Abbrev_MenuButton_Height(ip->value_ginfo)) / 2,
					dp->orientation | (preview ? OLGX_INVOKED :
			...
			case PANEL_MARKED:
				...
				olgx_draw_check_box(ip->value_ginfo, xv_xid(info),
						...
						(dp->choice_rects[which_choice].r_height -
								CheckBox_Height(ip->value_ginfo)) / 2, state);
				PANEL_END_EACH_PAINT_WINDOW if (color_index >= 0)
					olgx_set_single_color(ip->value_ginfo, OLGX_BLACK,
