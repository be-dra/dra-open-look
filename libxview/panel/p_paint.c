char p_paint_sccsid[] = "@(#)p_paint.c 20.26 89/11/30 Copyr 1984 Sun Micro DRA: $Id: p_paint.c,v 4.2 2025/04/03 06:22:56 dra Exp $";
...
static void panel_repaint_background(register Panel_info *, Xv_Window , Rect *);
static void panel_paint_item(Item_info *ip);
...
Pkg_private void panel_redisplay(Panel panel_public, Xv_Window pw, Rectlist *repaint_area)
{
	...
	if ( panel->show_border )
		...
		olgx_draw_box(panel->ginfo, xv_xid(info), 0, 0, 
			      (int)xv_get(panel_public, XV_WIDTH) - 1, 2 /* height */,
			      OLGX_INVOKED, FALSE
			      );
	} else { 	/* 2D */
	    if ( (int) xv_get(panel_public, XV_Y)  == 0 ) {
		...
		XDrawLine(xv_display(info), xv_xid(info),
			  ...
			  0, 0, (int)xv_get(panel_public, XV_WIDTH) - 1, 0);
	    }
	}
    }
...
Xv_public int panel_paint(Panel client_object, Panel_setting flag)
{
	...
	if (is_panel(object)) {
		(*object->ops.panel_op_paint)(client_object, flag);
	}
	else {
		/*
...
static void panel_paint_item(Item_info *ip)
{
	 ...
	 *   bug 1089473
	 */
	if (panel->paint_window->view || rect_intersectsrect(&rect, view_rect)) {
		/* paint */
		(*ip->ops.panel_op_paint) (ITEM_PUBLIC(ip), PANEL_NO_CLEAR);
...
Pkg_private void panel_paint_border(Panel panel_public, Panel_info *panel, Xv_Window pw)
{
	...
	XDrawRectangle(xv_display(info), xv_xid(info),
		       gc_list[SCREEN_SET_GC], 
		       0, 0, (unsigned)(width-1), (unsigned)(height-1));
    }
} /* panel_paint_border() */
