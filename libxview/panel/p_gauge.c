...
static void gauge_paint(Panel_item item_public, Panel_setting u);
...
/* -------------------- XView Functions  -------------------- */
/*ARGSUSED*/
static int gauge_init(Panel panel_public, Panel_item item_public,
...
static Xv_opaque gauge_set_avlist(Panel_item item_public, Attr_avlist avlist)
{
    register Item_info *ip = ITEM_PRIVATE(item_public);
...
static Xv_opaque gauge_get_attr(Panel_item item_public, int *status,
...
static int gauge_destroy(Panel_item item_public, Destroy_status  status)
{
...
#else
	size = xv_pf_textwidth((int)strlen(min_buf), ip->panel->std_font, min_buf);
#endif /* OW_I18N */
...
#else
	size = xv_pf_textwidth((int)strlen(max_buf), ip->panel->std_font, max_buf);
#endif /* OW_I18N */
