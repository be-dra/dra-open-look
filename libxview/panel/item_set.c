#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)item_set.c 20.104 93/06/28 DRA: $Id: item_set.c,v 4.5 2024/11/30 16:20:53 dra Exp $";
#endif
#endif
	...
	for (attrs = avlist; *attrs; attrs = attr_next(attrs)) {
		...
		switch (attrs[0]) {
			...
			case PANEL_EVENT_PROC:
				ip->ops.panel_op_handle_event = (void (*)(Xv_opaque, Event *))attrs[1];
				break;
			...
			case PANEL_LABEL_BOLD:
				if (ip->item_type == PANEL_MESSAGE_ITEM) {
					if ((int)attrs[1] != image_bold(&ip->label)) {
						if ((int)attrs[1]) {
			...
			case PANEL_NOTIFY_PROC:
				ip->notify = (int (*)(Panel_item, Event *))attrs[1];
			...
			case PANEL_ITEM_MENU:
				...
				if ((ip->menu = (Xv_opaque) attrs[1]))
					xv_set(ip->menu, XV_INCREMENT_REF_COUNT, NULL);
				...
				if (label_type == -1) {
...
#ifdef OW_I18N
					{
						...
						label_data =
								(Xv_opaque)
								panel_strsave_wc(image_string_wc(label));
					}
#else
				...
				/* WHY do we "force label to be remade?
				 * Does a PANEL_ITEM_MENU affect the label????
				 */
				
				/* in rare cases PANEL_BUTTONs with menus were not wide enough
				 * Yes: in case of PANEL_BUTTONs a PANEL_ITEM_MENU will
				 * affect at least the width of the whole item
				 */

				/* this modification might also solve the problem in
				 * p_utl.c: REF (hklesbrfhklbserf)
				 */

				/* BUT: do we really need to set a "new" label? Isn't it enough
				 * to "recalculate the size" ?
				 * I made an attempt to set ONLY: label_width_changed = TRUE;
				 * But then, a few menu buttons were to narrow - and especially
				 * labels of PANEL_TEXTs appeared truncated...
				 */
				if (xv_get(item_public, XV_IS_SUBTYPE_OF, PANEL_BUTTON)
					&& label_type == -1)
				{
					label_type = label->im_type;/* force label (image) to be
													 * remade */
					if (label_type == PIT_STRING) {
						...
						label_data = (Xv_opaque)label_to_be_freed;
					}
					else
						label_data = (Xv_opaque)image_svrim(label);
				}
			...
			case PANEL_OPS_VECTOR:
				...
				if (panel->event_proc)
					ip->ops.panel_op_handle_event =
							(void (*)(Panel, Event *))panel->event_proc;
				break;
 ...
 * PANEL_ITEM_WANTS_WC					     */
			...
			case PANEL_ITEM_LAYOUT_PROC:
				ip->layout_proc = (item_layout_t)attrs[1];
				break;
			...
			case PANEL_POST_EVENTS:
				if ((int)attrs[1]) {
					...
					notify_set_event_func(item_public,
							(Notify_func)panel_base_event_handler,
							NOTIFY_IMMEDIATE);
				}
				else
					ip->flags &= ~POST_EVENTS;
...
#else
		label_size = xv_pf_textwidth((int)strlen(image_string(label)),
	...
	if (set(label_type)) {
		...
		if (ip->label_width)
			label_size.x = ip->label_width;
		item_adjust_label_size(ip->item_type, (int)image_type(label), &label_size,
				ip->menu ? TRUE : FALSE, panel->ginfo);
		ip->label_rect.r_width = label_size.x;
		ip->label_rect.r_height = label_size.y;
		label_size_changed = TRUE;
	...
	if (panel->status.painted && !panel->no_redisplay_item)
		panel_redisplay_item(ip, ip->repaint);

	/* this modification might also solve the problem in
	 * p_utl.c: REF (hklesbrfhklbserf)
	 */
	if (label_to_be_freed) xv_free(label_to_be_freed);
	...
	/* label_type: PIT_STRING or PIT_SVRIM */
		...
		switch (ip->layout) {
			case PANEL_HORIZONTAL:
				...
				ip->value_rect.r_left = rect_right(&ip->label_rect) + 1 +
						(ip->label_rect.r_width ? panel_label_x_gap(ip->panel) : 0);
			...
			case PANEL_VERTICAL:
				...
				ip->value_rect.r_top = rect_bottom(&ip->label_rect) + 1 +
						(ip->label_rect.r_height ? panel_label_y_gap(ip->panel) : 0);
				break;
			default:
				break;
		}
	...
	switch (ip->layout) {
		...
		case PANEL_VERTICAL:
			...
			if (ip->label_rect.r_height > 0)
				ip->label_rect.r_top -= ip->label_rect.r_height
...
Xv_private void panel_item_parent(Panel_item item, Panel parent)
{
	...
	if (new_panel != NULL) {
		...
		panel_append(ip);
		if (ip->ops.panel_op_restore)
			(*ip->ops.panel_op_restore) (ITEM_PUBLIC(ip), PANEL_NO_CLEAR);
