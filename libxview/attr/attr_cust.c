...
EXTERN_FUNCTION(Attr_attribute db_get_data, ( XID db, XrmQuarkList instance_qlist, char *attr_name, Attr_attribute attr, Attr_attribute default_value ));
...
static Cust_pkgs *attr_search_cust_pkg(const Xv_pkg *pkg)
{
	...
	for (cur_pkg = customizable_pkgs; cur_pkg; cur_pkg = cur_pkg->next) {
		if (cur_pkg->pkg == pkg) {
			return (cur_pkg);
		}
	}
...
static Xv_server attr_get_server(Xv_object obj, Xv_object passed_owner)
{
	Xv_object owner;
	Xv_server server;
	...
	if (xv_get(obj, XV_IS_SUBTYPE_OF, WINDOW)) {
		...
		server = attr_get_server(owner, XV_NULL);
	}
			...
			case ATTR_NONE:	/* not a list */
				*dest++ = attr;
				if (!use_db) {
					...
					if (attr_name) {
						default_value = avlist_get(avlist);

						*dest++ = db_get_data(db, instance_qlist,
						...
						if (cardinality != 0)	/* don't collapse inline */
							*dest++ = avlist_get(avlist);
							...
							if (new_avlist)
								 ...
								 */
								dest = attr_copy_customize(obj, pkg,
										instance_name, owner, see_db, dest,
										new_avlist);
						}
						break;
				}
...
Xv_public void xv_add_custom_attrs(const Xv_pkg *pkg, ...)
{
	...
	Cust_attrs *c_attrs;
	char *attr_name;
#  define LOCAL_ATTR_TYPE Xv_generic_attr
	LOCAL_ATTR_TYPE attr;
	...
	VA_START(list, pkg);
	attr = va_arg(list, LOCAL_ATTR_TYPE);
	while (attr) {
		 ...
		 */
		c_attrs = attr_new_cust_attr((Attr_attribute)attr, attr_name);
