#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)attr_cu.c 20.12 90/06/21 Copyr 1984 Sun Micro  DRA: $Id: attr_cu.c,v 4.1 2024/03/28 19:32:21 dra Exp $ ";
#endif
#endif
 ...
 * encoded_value.
 */
static void attr_decode_cu(u_int encoded_value, int *char_part, int *pixel_part)
{
...
int attr_rc_unit_to_y(u_int encoded_value, int row_height, int top_margin,
						int row_gap)
{
...
int attr_rc_unit_to_x(u_int encoded_value, int col_width, int left_margin,
								int col_gap)
{
    int             char_part, pixel_part;
    int             length;
 ...
 */
static Attr_avlist attr_rc_unit_to_pixel(Attr_base_type  base_type,
    Attr_avlist avlist, int col_width, int row_height, int left_margin,
	...
	switch (base_type) {
		...
		case ATTR_BASE_X:	/* single x-coordinate */
			*avlist = (unsigned long)
					attr_rc_unit_to_x((u_int) * avlist, col_width, left_margin,
					col_gap);
			avlist++;
			break;
		...
		case ATTR_BASE_Y:	/* single y-coordinate */
			*avlist = (unsigned long)
					attr_rc_unit_to_y((u_int) * avlist, row_height, top_margin,
					row_gap);
			avlist++;
			break;
...
void attr_replace_cu(Attr_avlist avlist, Xv_font	font, int left_margin,
...
void attr_rc_units_to_pixels(Attr_avlist avlist, int col_width, int row_height,
    				int left_margin, int top_margin, int col_gap, int row_gap)
{
