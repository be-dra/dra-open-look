...
typedef void (*ui_req_proc_t)(Xv_server, Xv_opaque, const char *);
typedef void (*trace_proc_t)(Xv_server, int lvl,const char *f, int l,
											const char *txt);
typedef void (*extXeventProc_t)(Xv_server, Display *, XEvent *, Xv_opaque);

typedef struct server_proc_list {
	Xv_sl_link	next;
	Xv_opaque	id;  	/* unique id, typically xview handle */
	extXeventProc_t extXeventProc;
	extXeventProc_t pvtXeventProc;
} Server_proc_list;
...
#endif
	ui_req_proc_t ui_reg_proc;
	trace_proc_t trace_proc;
	char *app_help_file;
