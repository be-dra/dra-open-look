...
#define LMSGSIZE 128
	...
	for (attrs = avlist; *attrs; attrs = attr_next(attrs)) {
		switch ((int)attrs[0]) {
			case ERROR_BAD_ATTR:
				sprintf(buf, XV_MSG("bad attribute, %s"), attr_name(attrs[1]));
				break;
			case ERROR_BAD_VALUE:
				sprintf(buf,
						XV_MSG("bad value (0x%lx) for attribute %s"), attrs[1],
						attr_name(attrs[2]));
				break;
			case ERROR_CANNOT_GET:
				sprintf(buf, XV_MSG("cannot get %s"), attr_name(attrs[1]));
				break;
			case ERROR_CANNOT_SET:
				sprintf(buf, XV_MSG("cannot set %s"), attr_name(attrs[1]));
				break;
			...
			case ERROR_INVALID_OBJECT:
				sprintf(buf, XV_MSG("invalid object (%s)"), (char *)attrs[1]);
				break;
			case ERROR_LAYER:
	...
	switch (layer) {
		...
		case ERROR_SERVER:
			...
			if (xerror) {
				dpy = xerror->display;
				XGetErrorText(dpy, (int)xerror->error_code, layer_msg,
						LMSGSIZE);
			}
			else
	...
	if (error_string) {
		...
		if (length) {
			if (layer_msg[0] || buf[0])
				...
				strcat(msg, " ");
			strncat(msg, error_string, (size_t)length);
		}
	}
	if (pkg_name) {
		char tmpmsg[ERROR_MAX_STRING_SIZE];
...
Xv_public int xv_error(Xv_object object, ...)
{
	va_list valist;
