...
static int can_change_to_dir(File_list_private *private, char *path)
{
	...
	if (private->dir_ptr) (void)closedir(private->dir_ptr);
	private->dir_ptr = dir;
...
static File_list_row * flist_next_row(File_list_row **rows, int row_num)
{
	...
	if (!*rows)
		*rows = xv_malloc(ALLOC_INCR * 4 * sizeof(File_list_row));
		...
		num_allocs++;
		*rows = xv_realloc(*rows,
				num_allocs * ALLOC_INCR * sizeof(File_list_row));
	}
...
static int flist_load_dir(File_list_private *private, char *directory)
{
	 ...
	 */
	cwd = getcwd(NULL, (long)MAXPATHLEN);
	...
	while ((entry = readdir(private->dir_ptr))) {
		...
		if (private->regex_pattern
				&& !flist_match_regex(entry->d_name, private))
		 ...
		 */
		(void)strxfrm(xfrm_buf, entry->d_name, (long)MAXPATHLEN + 1);
		current->xfrm = xv_strcpy(NULL, xfrm_buf);
		 ...
		 */
		current->vals.extension_data = (S_ISDIR(current->stats.st_mode))
	...
	if (private->compare_func) {
		typedef int (*qsf_t)(const void *, const void *);
		qsf_t qsf = (qsf_t) (private->compare_func);

		qsort(&rows[1], (unsigned long)row_num - 1, sizeof(File_list_row), qsf);
	}
 ...
 * directories.
 */
static int flist_list_notify(Panel_item item, char *entry_name,
 ...
 * xv_create() method
 */
static int file_list_init(Xv_opaque owner, File_list slf, Attr_avlist avlist, int *u)
{
	File_list_public *public = (File_list_public *) slf;
	...
	private->directory = (char *)getcwd(NULL, (long)MAXPATHLEN);
	...
	list_avlist =
		attr_create_list((Attr_attribute) PANEL_NOTIFY_PROC, flist_list_notify,
			(Attr_attribute) PANEL_LIST_DO_DBL_CLICK, TRUE,
			(Attr_attribute) PANEL_READ_ONLY, TRUE,
			(Attr_attribute) PANEL_LIST_DISPLAY_ROWS, DEFAULT_LIST_ROWS,
			(Attr_attribute) PANEL_LIST_ROW_HEIGHT, row_height,
			(Attr_attribute) PANEL_PAINT, PANEL_NONE,
			NULL);
...
static void flist_update_list(File_list_private *private, File_list_row rows[], int num_rows)
{
    ...
    Xv_opaque (* plist_set)(Xv_opaque, Attr_avlist) = xv_panel_list_pkg.set;
 ...
 * xv_set() method
 */
static Xv_opaque file_list_set(File_list public, Attr_avlist avlist)
{
	...
	for (attrs = avlist; *attrs; attrs = attr_next(attrs)) {
		switch ((int)attrs[0]) {
			...
			case FILE_LIST_DIRECTORY:
				{
					...
					if (no_string(new_dir)) {
						...
						xv_set(public,
								PANEL_LIST_DELETE_ROWS, 0,
											xv_get(public, PANEL_LIST_NROWS),
					...
					if (!validate_new_directory(private, real_path)) {
						...
						(void)sprintf(msg, XV_MSG("Can't change to %s"),
								new_dir);
						xv_error(public,
								ERROR_PKG, FILE_LIST,
								ERROR_STRING, msg,
			...
			case FILE_LIST_FILTER_STRING:
				...
				if (private->regex_pattern) {
					if (flist_compile_regex(private)) {
						if (private->f.auto_update) do_update = TRUE;
					}
					else {
						do_update = FALSE;
					}
			...
			case FILE_LIST_FILTER_FUNC:
				...
				private->client_filter =
						(File_list_op(*)(char *, File_list_row *))attrs[1];
				if (private->f.auto_update) do_update = TRUE;
				break;
			...
			case FILE_LIST_COMPARE_FUNC:
				...
				private->compare_func =
						(int (*)(File_list_row *, File_list_row *))attrs[1];
				if (private->f.auto_update) do_update = TRUE;
				break;
			...
			case FILE_LIST_CHANGE_DIR_FUNC:
				ATTR_CONSUME(attrs[0]);
				private->cd_func =
						(int (*)(File_list, char *, struct stat *,
								File_list_op))attrs[1];
				break;

			case FILE_LIST_SHOW_DOT_FILES:
				ATTR_CONSUME(attrs[0]);
				private->f.show_dotfiles =
			...
			case FILE_LIST_SHOW_DIR:
				...
				xv_set(public, PANEL_LIST_TITLE, (private->f.show_dir)
			...
			case XV_END_CREATE:
				...
				if (private->directory) do_update = TRUE;
				break;
 ...
 * xv_get() method
 */
static Xv_opaque file_list_get(File_list public, int *status, Attr_attribute attr, va_list valist)
{
	File_list_private *private = FILE_LIST_PRIVATE(public);
	...
	switch ((int)attr) {
		case FILE_LIST_ROW_TYPE:
			return xv_get(FILE_LIST_PUBLIC(private),
					PANEL_LIST_EXTENSION_DATA, va_arg(valist, int));
 ...
 * xv_destroy() method
 */
static int file_list_destroy(File_list public, Destroy_status status)
{
	File_list_private *private = FILE_LIST_PRIVATE(public);
