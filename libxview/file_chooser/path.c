 ...
 */
/* ARGSUSED */
static int path_init_avlist(Xv_opaque owner, Path_name self, Attr_avlist avlist, int *u)
{
	Path_public *public = (Path_public *)self;
	...
	if (!path_avlist)
		path_avlist
				= attr_create_list((Attr_attribute)PANEL_NOTIFY_PROC, xv_path_name_notify_proc,
	 ...
	 * by calling xv_super_set_avlist on PanelItem package as being the parent
	 * of our parent, the Panel Text package.  what we really need here is an
	 * xv_super_duper_set_avlist() function.
	 */
	xv_super_set_avlist(self, path_pkg.parent_pkg, path_avlist);
 ...
 * xv_set() method
 */
static Xv_opaque path_set_avlist(Path_name public, Attr_avlist avlist)
{
 ...
 */
/* ARGSUSED */
static Xv_opaque path_get_attr(Path_name public, int *status, Attr_attribute attr, va_list args)
{
	Path_private *private = PATH_PRIVATE(public);
 ...
 * xv_destroy() method
 */
static int path_destroy_private(Path_name public, Destroy_status status)
{
    Path_private *private = PATH_PRIVATE(public);
