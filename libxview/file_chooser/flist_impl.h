...
typedef struct {
    ...
    File_list_op	(* client_filter)(char *, File_list_row *);	/* filter func */
    int			(* client_notify)(Panel_item, char *, char *, Xv_opaque , Panel_list_op, Event *, int);	/* client's PANEL_NOTIFY_PROC */
    int			(* compare_func)(File_list_row *, File_list_row *);	/* qsort function */
     
    int			(* cd_func)(File_list, char *, struct stat *, File_list_op);		/* tell client we are changing dir */
