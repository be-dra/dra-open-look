...
static void		(* default_goto_event_handler)(Path_name path_name, Event *event);
 ...
 * xv_create() method
 */
static int file_chooser_init(Xv_opaque owner, File_chooser slf, Attr_avlist avlist, int *u)
{
	File_chooser_public *public = (File_chooser_public *) slf;
	...
	private->state->directory = (char *)getcwd(NULL, (long)MAXPATHLEN);
	...
	for (attrs = avlist; *attrs; attrs = attr_next(attrs)) {
		switch ((int)attrs[0]) {
			...
			case FILE_CHOOSER_ADD_UI_FUNC:
	    		ATTR_CONSUME(attrs[0]);
	    		private->add_ui_func = (fchsr_add_ui_func_t) attrs[1];
	    		break;
 ...
 * xv_get() method
 */
static Xv_opaque file_chooser_get(File_chooser	public, int *status, Attr_attribute attr, va_list args)
{
	Fc_private *private = FC_PRIVATE(public);
	...
	switch ((int)attr) {
		...
		case FILE_CHOOSER_CHILD:{
				File_chooser_child child = va_arg(args, File_chooser_child);
				...
				switch (child) {
					...
					case FILE_CHOOSER_PATTERN_TEXT_CHILD:
						return (Xv_opaque) private->ui.pattern_text;
				}
				break;
			}
		...
		case FILE_CHOOSER_SHOW_PATTERN:
	    	return (Xv_opaque)private->show_pattern;
 ...
 * xv_destroy() method
 */
static int file_chooser_destroy(File_chooser public, Destroy_status status)
{
    Fc_private *private = FC_PRIVATE(public);
...
static Notify_value fc_event_interposer(Notify_client client, Notify_event event, Notify_arg arg, Notify_event_type type)
{
	...
	if (event_action((Event *) event) == WIN_VISIBILITY_NOTIFY) {
		xv_set((Xv_opaque) client,
				WIN_IGNORE_EVENT, WIN_VISIBILITY_NOTIFY,
				NULL);
...
static void fc_goto_event_proc(Path_name path_name, Event *event)
{
	...
	if (event_action(event) == ACTION_DEFAULT_ACTION
		...
		xv_path_name_notify_proc(path_name, event);
...
#define USER_DIR_DELIMITER	"\n"
	extern char *xv_strtok(char *, char *);
	History_list hl;
	...
	if (!hl) {
		...
		char *user_entries =defaults_get_string("openWindows.gotoMenu.userDirs",
				"OpenWindows.GotoMenu.UserDirs", NULL);
		...
		while (user_dir) {
			...
			if (!xv_isdir(real_path)) {
				...
				/* Dear people from SUN: we get these user_entries from the
				 * RESOURCE_MANAGER property on the root window, remember?
				 *
				 * However, the directories on this root window may be
				 * light years away from where this application is running.
				 * So, if something on this list is not a directory, 
				 * let us omit it silently instead of screaming and shouting
				 */
...
static Panel_setting note_pattern(Panel_item txt, Event *event)
{
    Fc_private *private = (Fc_private *)xv_get(txt, XV_KEY_DATA, fc_key);
    char *str = (char *) xv_get(txt, PANEL_VALUE);

	xv_set(private->ui.list, FILE_LIST_FILTER_STRING, str, NULL);
    return panel_text_notify(txt, event);
}
...
static void fc_create_ui(Fc_private *private)
{
	...
	private->ui.hist_menu = xv_create(server, HISTORY_MENU,
			HISTORY_MENU_HISTORY_LIST, private->hist_list,
	...
	private->ui.goto_btn = xv_create(private->ui.panel, PANEL_BUTTON,
	...
	r = *((Rect *)xv_get(private->ui.goto_btn, XV_RECT));

	private->ui.goto_txt = xv_create(private->ui.panel, PATH_NAME,
			XV_HELP_DATA, "xview:file_chooser_goto_path",
			XV_Y, r.r_top, /* the panel put this sometimes in the next line */
			XV_X, rect_right(&r) + 6,
			PANEL_ITEM_LAYOUT_ROLE, PANEL_ROLE_FOLLOWER,
	...
	if (!default_goto_event_handler)
		default_goto_event_handler =
				(void (*)(Path_name path_name,
						Event * event))xv_get(private->ui.goto_txt,
				PANEL_EVENT_PROC);
	xv_set(private->ui.goto_txt, PANEL_EVENT_PROC, fc_goto_event_proc, NULL);

	private->ui.folder_txt = xv_create(private->ui.panel, PANEL_TEXT,
	...
	if (private->show_pattern) {
		private->ui.pattern_text = xv_create(private->ui.panel, PANEL_TEXT,
				...
				PANEL_ITEM_LAYOUT_ROLE, PANEL_ROLE_LEADER,
				PANEL_VALUE_DISPLAY_LENGTH, 37,
				PANEL_NOTIFY_PROC, note_pattern,
				NULL);
	}

	font = xv_get(private->ui.panel, XV_FONT);
    dcw = (int) xv_get(font, FONT_DEFAULT_CHAR_WIDTH);
	private->ui.list = xv_create(private->ui.panel, FILE_LIST,
			...
			PANEL_NEXT_ROW, -1,
			PANEL_ITEM_LAYOUT_ROLE, PANEL_ROLE_LEADER,
			PANEL_LABEL_STRING, XV_MSG("Files:"),
			PANEL_LIST_WIDTH, 30 * dcw,
			FRAME_ITEM_RESIZE_WIDTH,
			NULL);

	if (private->type != FILE_CHOOSER_OPEN) {
		private->ui.document_txt = xv_create(private->ui.panel, PANEL_TEXT,
				XV_HELP_DATA, "xview:file_chooser_document_name",
				PANEL_LABEL_STRING, (private->type == FILE_CHOOSER_SAVE)
										? XV_MSG("Save:")
										: XV_MSG("Save As:"),
				PANEL_VALUE_STORED_LENGTH, MAXPATHLEN + 1,
		...
		private->document_default_event =
			(fchsr_document_default_event_t) xv_get(private->ui.document_txt,
												PANEL_EVENT_PROC);
		xv_set(private->ui.document_txt,
	...
	if (private->add_ui_func) {
	    (*(private->add_ui_func))(FC_PUBLIC(private));
	}

	private->ui.open_btn = xv_create(private->ui.panel, PANEL_BUTTON,
				XV_HELP_DATA, "xview:file_chooser_open_button",
				PANEL_LABEL_STRING, ((private->type == FILE_CHOOSER_OPEN) && !private->custom)
								? XV_MSG("Open")
								: XV_MSG("Open Folder"),
	...
	private->ui.cancel_btn = xv_create(private->ui.panel, PANEL_BUTTON,
	...
	if (private->type != FILE_CHOOSER_OPEN)
		private->ui.save_btn = xv_create(private->ui.panel, PANEL_BUTTON,
	 ...
	 */
	if (private->custom)
		private->ui.custom_btn = xv_create(private->ui.panel, PANEL_BUTTON,
	...
	xv_set(private->ui.panel,
			...
			? private->ui.goto_txt
			: private->ui.document_txt, PANEL_DEFAULT_ITEM, default_item,
			PANEL_BORDER, TRUE,
			NULL);

	xv_set(private->ui.panel, PANEL_DO_LAYOUT, NULL);
	window_fit(FC_PUBLIC(private));
...
static void fc_hist_notify(History_menu hm, char *label, char *value)
{
	...
	Event event;
	fchsr_document_default_event_t event_proc
	    = (fchsr_document_default_event_t) xv_get(private->ui.goto_txt, PANEL_EVENT_PROC);
		...
		case PANEL_LIST_OP_DBL_CLICK:
			 ...
			 */
				...
				type = (File_list_row_type) xv_get(private->ui.list,
						FILE_LIST_ROW_TYPE, row);
				if (type != FILE_LIST_FILE_TYPE) {
					xv_error_sprintf(FC_PUBLIC(private), TRUE,
							XV_MSG("Selection is not a document!"));
					status = XV_OK;
				}
				else
			...
			if (status == XV_OK)
				xv_set(FC_PUBLIC(private), XV_SHOW, FALSE, NULL);
			break;
 ...
 * successfully.
 */
static int fc_do_open(Fc_private *private, int row, char *dir, char *file,
								Xv_opaque client_data)
{
	...
	xv_set(private->ui.notice,
			...
			NOTICE_BUSY_FRAMES,
				FC_PUBLIC(private),
				xv_get(FC_PUBLIC(private), XV_OWNER),
				NULL,
			XV_SHOW, TRUE,
			NULL);
	xv_free_ref( path );
...
static int fc_do_save(Fc_private *private, char *dir, char *file)
{
	...
	if (!have_file_name && !private->save_to_dir) {
		xv_error_sprintf(FC_PUBLIC(private),
				TRUE, XV_MSG("Please enter a name in the Save: field.")
				);
		return XV_ERROR;
	}
	...
	if (canceled == NOTICE_NO) {
...
#endif
		{
			...
			/* so ein unsaegliches Misskonzept... */

			status = (*note_save)(FC_PUBLIC(private), path, exists);
		}
	}
...
static int fc_confirm_overwrite(Fc_private *private, char *path, char *file, struct stat *sbuf)
{
	 ...
	 */
		...
		if (is_absolute(file)) {
			...
			(void)sprintf(buf, XV_MSG("The file \"%s\" already exists."), file);
			(void)sprintf(buf2,
					XV_MSG
					("Do you want to overwrite the existing file \"%s\"?"),
					file);
			xv_set(private->ui.notice, NOTICE_MESSAGE_STRINGS, buf, buf2, NULL,
					NULL);
		}
	}  /* can write to file */
	/*
	 ...
	 */
		...
		if (is_absolute(file)) {
			...
			(void)sprintf(buf,
					...
					file);
			(void)sprintf(buf2,
					XV_MSG("Do you want to overwrite the file \"%s\"?"), file);
			xv_set(private->ui.notice,
					NOTICE_MESSAGE_STRINGS, buf, buf2, NULL, NULL);
		}
	}  /* owned by user */
	/*
...
static int fc_compare_func(File_list_row *row1, File_list_row *row2)
{
    Fc_private *private = (Fc_private *) xv_get(row1->file_list,
										XV_KEY_DATA, fc_key);
    int status;
...
extern int xv_strcasecmp(char *str1, char *str2);
