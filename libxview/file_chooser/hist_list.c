...
static void add_fixed_entry(History_list_private *private, char *label, char *);
 ...
 * xv_create() method
 */
static int hist_list_init(Xv_opaque owner, History_list self, Attr_avlist avlist, int *u)
{
     History_list_public *public = (History_list_public *)self;
 ...
 * xv_set() method
 */
static Xv_opaque hist_list_set(History_list public, Attr_avlist avlist)
{
		...
		switch ((int)attrs[0]) {
			...
			case HISTORY_ADD_ROLLING_ENTRY:{
					...
					if ((!private->duplicate_labels
									&& is_duplicate_entry(private, label))
							|| (!private->duplicate_values
									&& is_duplicate_entry(private, value))
							)
						/*
						 * since the client may not really know if this string
						 * is already in the list, we cannot reasonably say it
						 * is an error.  hence, don't beep or xv_error --
						 * just punt.
						 */
						break;
 ...
 * xv_get() method
 */
static Xv_opaque hist_list_get(History_list public, int *status, Attr_attribute attr, va_list args)
{
    History_list_private *private = HIST_LIST_PRIVATE(public);
 ...
 * xv_destroy() method
 */
static int hist_list_destroy(History_list public, Destroy_status status)
{
	History_list_private *private = HIST_LIST_PRIVATE(public);
 ...
 */
static void add_fixed_entry(History_list_private *private, char *label, char *value)
{
     ...
     */
    new = calloc(1L, sizeof(struct hist_entry));
    if ( label ) {
 ...
 */
static void add_rolling_entry(History_list_private *private, char *label,
								char *value)
{
    struct hist_entry	*new;
     ...
     */
    if ( !private->blank )
	private->blank = xv_create(XV_NULL, MENUITEM_SPACE,
						MENU_RELEASE,
						NULL);
