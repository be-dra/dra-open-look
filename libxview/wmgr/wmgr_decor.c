#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)wmgr_decor.c 1.27 93/06/28 RCSId $Id: wmgr_decor.c,v 2.3 2024/11/16 15:35:54 dra Exp $";
#endif
#endif
...
Xv_private int wmgr_set_win_attr(Frame frame_public, WM_Win_Type *win_attr)
{
	 ...
	 */
	new_format = xv_get(xv_screen(info), SCREEN_CHECK_SUN_WM_PROTOCOL,
			"_SUN_OL_WIN_ATTR_5");

	if (new_format) {
		XChangeProperty(xv_display(info), xv_xid(info),
				atom, atom, 32, PropModeReplace, (unsigned char *)win_attr, 5);
	}
	else {
		...
		if (win_attr->pin_initial_state == WMPushpinIsOut) {
			old_win_attr.pin_initial_state =
					(Atom) xv_get(server_public, SERVER_WM_PIN_OUT);
		}
		if (win_attr->pin_initial_state == WMPushpinIsIn) {
			old_win_attr.pin_initial_state =
					(Atom) xv_get(server_public, SERVER_WM_PIN_IN);
		}
...
Xv_private void wmgr_set_rescale_state(Frame frame_public, int state)
{
    ...
    XChangeProperty(xv_display(info), xv_xid(info),
		    ...
		    XA_INTEGER, 32, PropModeReplace, (unsigned char *)&st, 1);
}
