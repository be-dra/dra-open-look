...
Xv_public XID win_getlink(Xv_object window, int linkname)
{
	...
	if (XQueryTree(xv_display(info), window_xid,
		...
		fprintf(stderr, XV_MSG("win_getlink: XQueryTree failed!\n"));
		goto deallocate;
	}
	switch (linkname) {
		case WL_ENCLOSING:
		...
		case WL_COVERING:	/* looking for the younger sibling */
			...
			if (nchildren) {
				...
				if (*c != window_xid) {
					fprintf(stderr,
							XV_MSG
							("win_getlink(sibling): window not in tree\n"));
...
Xv_public void win_setlink(Xv_object window, int linkname, XID number)
{
		...
		case WL_BOTTOMCHILD:
			...
			if (number == 0) {
				xv_error(window,
						ERROR_STRING,
						XV_MSG
						("Call to win_setlink() with ZERO link value would orphan a child. Ignored. (Win package)"),
						NULL);
				return;
			}
			sub_root = number;
	...
	XConfigureWindow(display, sub_root, (unsigned)(link_info.sibling ?
...
{
    (void) sprintf(name, "%s:%d", XDisplayString(display), (int)xid);
    return (name);
}
...
Xv_public void win_numbertoname(int winnumber, char *name)
{
    ...
    win_name_for_qualified_xid(name,
                (Display *)xv_get(xv_default_display, XV_DISPLAY),
				(XID)winnumber);
}
...
Xv_private Xv_object win_data(Display *display, XID xid)
{
    ...
    if (XFindContext(display, xid, CONTEXT, &ptr)) {
		result = 0;
    }
	else result = (Xv_object)ptr;
    return result;
}
...
Xv_private void win_get_property(Xv_object window, Attr_attribute property_name, long data_offset, long data_count, Atom property_type, unsigned long *returned_data_count, unsigned long *bytes_after, unsigned char **property_data)
{
	register Xv_Drawable_info *info;
	...
	DRAWABLE_INFO_MACRO(window, info);
	property = (Atom) xv_get(xv_server(info), (Attr32_attribute)property_name);
	XGetWindowProperty(xv_display(info), xv_xid(info),
