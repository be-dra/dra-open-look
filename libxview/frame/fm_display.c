...
Pkg_private void frame_display_footer(Frame frame_public, int clear_first)
{
	...
	int footer_width;
	int quarter_width;
	unsigned long save_black;	/* saved value for OLGX_BLACK */
	unsigned long new_black;	/* new value for OLGX_BLACK */
	...
	if (change_black) {
		olgx_set_single_color(frame->ginfo, OLGX_BLACK,
				new_black, OLGX_SPECIAL);
	}
 ...
 *				changed.
 */
Pkg_private void frame_update_status_win_color(Frame frame_public,
			Xv_Window status_window, Cms new_frame_cms,
			unsigned long new_frame_fg, int new_frame_fg_set,
			int *repaint_needed)
{
