#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)frame.c 20.73 93/06/28 DRA: $Id: frame.c,v 4.1 2024/03/28 12:57:19 dra Exp $ ";
#endif
#endif
...
Xv_public void frame_get_rect(Frame frame, Rect *rect)
{
	...
	if ((xwin_attr.map_state == IsViewable) || status_get(frame_info, iconic)) {
		...
		XTranslateCoordinates(display, win_xid,
				(XID) xv_get(xv_root(info), XV_XID), 0, 0, &x, &y, &notused);
		xwin_attr.x = x - left;
		xwin_attr.y = y - top;
	}
...
Xv_public void frame_set_rect(Frame frame, Rect *rect)
{
    ...
    XMoveResizeWindow(xv_display(info),  xv_xid(info), 
      rect->r_left + x_adj,
      rect->r_top + y_adj,
      (unsigned)(rect->r_width - (left + right)), 
      (unsigned)(rect->r_height - (top + bottom)));
}
