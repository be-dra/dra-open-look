#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)frame_sw.c 20.26 93/06/28 DRA: $Id: frame_sw.c,v 4.1 2024/03/28 12:57:19 dra Exp $ ";
#endif
#endif
...
Pkg_private void frame_layout_subwindows(Frame frame_public)
{
	...
	(void) win_get_outer_rect(sw, &rect);
	    ...
	    if (avs < MIN_SW_SIZE)
	        avs = MIN_SW_SIZE;
	    desired_width = (int) xv_get(sw, WIN_DESIRED_WIDTH);
	    ...
	    if (avs < MIN_SW_SIZE)
	        avs = MIN_SW_SIZE;
	    desired_height = (int) xv_get(sw, WIN_DESIRED_HEIGHT);
