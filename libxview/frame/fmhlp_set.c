#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)fmhlp_set.c 1.26 93/06/28 DRA: $Id: fmhlp_set.c,v 4.1 2024/03/28 12:57:19 dra Exp $ ";
#endif
#endif
	...
	if (add_decor || delete_decor) {
		...
		if (status_get(frame, show_label))
			add_decor_list[add_decor++] =
								(Atom) xv_get(server_public,
											  SERVER_WM_DECOR_HEADER);
		else
