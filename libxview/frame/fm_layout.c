...
Pkg_private int frame_layout(Xv_Window frame_public, Xv_Window child, Window_layout_op op, Xv_opaque d1, Xv_opaque d2, Xv_opaque d3, Xv_opaque d4, Xv_opaque d5)
{
	...
	if (! is_frame) {
		...
		if (op == WIN_ADJUST_RECT) {
			frame_adjust_rect(frame_public, NULL, child, is_subframe,
					(Rect *) d1);
		}
		return TRUE;
	}
	...
	switch (op) {
		case WIN_CREATE:{
				...
				if (is_subframe)
					 ...
					 */
					while (last_not_icon
							&& xv_get(last_not_icon, XV_IS_SUBTYPE_OF, ICON)) {
						last_not_icon =
								frame_prev_child(frame->first_subwindow,
								last_not_icon);
					}

					(void)frame_position_sw(frame, child, last_not_icon,
							WIN_EXTEND_TO_EDGE, WIN_EXTEND_TO_EDGE, &rect);
				...
				if (!is_footer &&
					...
					if (last_child)
						xv_set(last_child, XV_KEY_DATA, FRAME_NEXT_CHILD, child,
								NULL);
					else if (is_subframe)
					...
					xv_set(child, XV_KEY_DATA, FRAME_PREVIOUS_CHILD, last_child,
							NULL);
				}
				...
				if (!is_subframe && (int)!xv_get(child, XV_IS_SUBTYPE_OF, ICON)
					...
					if (xv_deaf(child, TRUE) != XV_OK) {
						xv_error(child,
								ERROR_STRING,
								XV_MSG
								("Attempt to make frame subwindow deaf failed"),
								NULL);
					}
				}
				break;
		...
		case WIN_ADJUST_RECT:
			frame_adjust_rect(frame_public, frame, child, is_subframe,
					(Rect *) d1);
			break;
		...
		default:{
				...
				(void)sprintf(temp,
						...
						op);
				xv_error((Xv_opaque) frame, ERROR_STRING, temp, ERROR_PKG,
						FRAME, NULL);
				return FALSE;
			}
