		...
		if (frame->geometry_flags & (XValue | YValue)) {
			...
			if (frame->geometry_flags & XNegative) {
				width = (frame->geometry_flags & WidthValue) ?
						frame->user_rect.r_width : (int)xv_get(frame_public,
						XV_WIDTH, 0);
				x += DisplayWidth(xv_display(info),
						screen) - width - (2 * FRAME_BORDER_WIDTH);
			}
			if (frame->geometry_flags & YNegative) {
				height = (frame->geometry_flags & HeightValue) ?
						frame->user_rect.r_height : (int)xv_get(frame_public,
						XV_HEIGHT, 0);
				y += DisplayHeight(xv_display(info),
						screen) - height - (2 * FRAME_BORDER_WIDTH);
			}
...
Pkg_private int frame_all_set_cmdline_options(Frame frame_public)
{
	...
	if (defaults_get_boolean("window.reverseVideo", "Window.ReverseVideo",
					FALSE)) {
		XColor tmp;
	 ...
	 * "openwindows.regularfont" resource which is probably set....
	 *
	 * This change was made together with the attempt to implement
	 * popup window scale.....
	 */
	if ((string = defaults_get_string("font.name.cmdline", "Font.Name.Cmdline", NULL)))
	{
		Xv_font font;
		char *save_name;
	...
	if (!font_found && (xv_font_bold() == (char *)NULL) &&
		...
		if (string) {
			if ((strcmp(string, "small") == 0)
					|| (strcmp(string, "Small") == 0)) {
				scale = WIN_SCALE_SMALL;
				scale_found = TRUE;
			}
			else {
				if ((strcmp(string, "medium") == 0)
						|| (strcmp(string, "Medium") == 0)) {
					scale = WIN_SCALE_MEDIUM;
					scale_found = TRUE;
				}
				else {
					if ((strcmp(string, "large") == 0)
							|| (strcmp(string, "Large") == 0)) {
			...
			if (scale_found) {
				 ...
				 */
				scaled_font =
						xv_find(frame_public, FONT, FONT_SCALE, scale, NULL);
				if (scaled_font) {
