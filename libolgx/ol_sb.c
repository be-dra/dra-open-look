...
static XRectangle *olgx_compute_intersection(XRectangle *, XRectangle *);
...
void olgx_draw_scrollbar(Graphics_info  *ginfo, Window win, int x, int y,
    ...
    int prop_pos, int prop_length, int state)
{
	...
	if (state & OLGX_UPDATE) {
	    ...
	    XClearArea(ginfo->dpy, win,
                          ...
                          (unsigned)(result_rect->width + 1),
						  (unsigned)(result_rect->height + 1),0);
	...
	olgx_draw_elevator(ginfo, win, elev_pos, y, state);
...
void olgx_draw_elevator(Graphics_info *info, Window win, int x, int y,
							int state)
{
    char            string[2];
    int             initial;
    ...
    if ((state & OLGX_ERASE))
	XClearArea(info->dpy, win, x - 1, y - 1,
		       (state & OLGX_HORIZONTAL) ? 
		          (unsigned)(sbheight + 2) :
				  (unsigned)((ScrollbarElevator_Width(info)) + 2),
		       (state & OLGX_VERTICAL) ? 
		          (unsigned)(sbheight + 2) :
				  (unsigned)((ScrollbarElevator_Width(info)) + 2),0);

    XFillRectangle(info->dpy, win, info->three_d ? info->gc_rec[OLGX_BG1]->gc :
                                             info->gc_rec[OLGX_WHITE]->gc,x,y,
           	   (state & OLGX_HORIZONTAL) ? 
	            (unsigned)(sbheight-1 ) : (unsigned)((ScrollbarElevator_Width(info))-1),
		   (state & OLGX_VERTICAL) ? 
		    (unsigned)(sbheight-1):
			(unsigned)((ScrollbarElevator_Width(info))-1));
...
void olgx_scroll_stipple_rects(Graphics_info *info, Window win,
...
static XRectangle *olgx_compute_intersection(XRectangle *rect1, XRectangle *rect2)
{
