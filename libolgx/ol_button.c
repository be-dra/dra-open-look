...
static void olgx_draw_ximage_label(Graphics_info *info, Window win, XImage *xim,
				int x, int y, int width, int height, int state);
void            olgx_set_busy_stipple(void);

static void olgx_draw_accel_label_internal(Graphics_info *info, Window win,
				int texty, int x, int y, int width, int height,
				void *main_label, int m_pos, void *qualifier_label,
				int	q_pos, int	mark_type, int	mark_pos, void *key_label,
				int	key_pos, int state, int centerflag);
static void olgx_draw_diamond_mark(Graphics_info *info, Window win, int x, int y, int state);

void olgx_draw_button(Graphics_info *info, Window win, int x, int y,
					int width, int height, void *label, int state)
{
    XTextItem       item;
    ...
    int             top_color, bottom_color, fill_color = 0;
	...
	if (info->three_d) {
		...
		if (state & OLGX_ERASE) {
		   ...
		   XFillRectangle(info->dpy, win,info->gc_rec[OLGX_BG1]->gc, 
                        x, y, (unsigned)width, (unsigned)Button_Height(info));
		}
...
#else
	    olgx_draw_text(info, win, (const char *) label,
#endif /* OW_I18N */
			   x + info->endcap_width,
...
void olgx_draw_varheight_button(Graphics_info  *info, Window win, int x,
								int y, int width, int height, int state)
{
	...
	if (state & OLGX_INVOKED)
	    XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BG2]->gc, x + 1,
			   y + 1, (unsigned)(width - 2), (unsigned)(height - 2));
	else
	...
	if (state & OLGX_INVOKED)
	    XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x + 3,
			   y + 3, (unsigned )(width - 6), (unsigned )(height - 6));
	else
	...
	XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BUSYGC]->gc, x + 2,
		       y + 2, (unsigned )(width - 4), (unsigned )(height - 4));
    }
}


void olgx_draw_menu_mark(Graphics_info *info, Window win, int x, int y,
							int state, int fill_in)
{
    char            string[3];
    GC              gc;

    if (state & OLGX_VERT_MENU_MARK) string[0] = VERT_MENU_MARK_UL;
    else if (state & OLGX_HORIZ_MENU_MARK) string[0] = HORIZ_MENU_MARK_UL;
    ...
    if ((state & OLGX_INVOKED) && (!info->three_d)) {
		...
		if (info->three_d) gc = info->gc_rec[OLGX_BG3]->gc;
		else gc = info->gc_rec[OLGX_BLACK]->gc;
	}
    ...
    XDrawString(info->dpy, win, gc, x, y, &string[0], info->three_d ? 1 : 2);
    if (state & OLGX_INACTIVE) {
									...
									int state)
{
	...
	olgx_draw_text(info, win, dots,
						x + ((Abbrev_MenuButton_Width(info) - width) / 2),
						y + ((Abbrev_MenuButton_Height(info) * 3) / 4),
						0, state & ~OLGX_INACTIVE);
}

void olgx_draw_abbrev_button( Graphics_info  *info, Window win,
	...
	if (!info->three_d) {	/* 2d */
		...
		if (state & OLGX_ERASE)
			XClearArea(info->dpy, win, x, y,
					(unsigned)Abbrev_MenuButton_Width(info),
					(unsigned)Abbrev_MenuButton_Width(info), 0);
		...
		if (state & OLGX_BUSY) {
			...
			XDrawText(info->dpy, win, info->gc_rec[OLGX_BUSYGC]->gc, x, y,
					&item, 1);
		}
		if (state & OLGX_INVOKED) {

			XFillRectangle(info->dpy, win, info->gc_rec[OLGX_WHITE]->gc, x + 2,
					y + 2, (unsigned)(Abbrev_MenuButton_Width(info) - 4),
					(unsigned)(Abbrev_MenuButton_Width(info) - 4));
			if (state & OLGX_HORIZONTAL) {
				string[0] = TEXTSCROLLBUTTON_RIGHT_INV;
				XDrawText(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x, y,
								&item, 1);
			}
			else if (state & OLGX_VERTICAL) {
				string[0] = OLG_ABBREV_MENU_BUTTON_INVERTED;
				XDrawText(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x, y,
								&item, 1);
			}
			else if (state & OLGX_ABBREV) {   /* abbreviated window button */
				string[0] = OLG_ABBREV_MENU_BUTTON_INVERTED;
				XDrawText(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x, y,
								&item, 1);
				XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc,
								x + 2, y + 3,
								(unsigned)(Abbrev_MenuButton_Width(info) - 5),
								(unsigned)(Abbrev_MenuButton_Width(info) - 7));
				draw_window_mark(info, win, x, y, state);
			}
		}
		else {
			...
			if (state & OLGX_HORIZONTAL) {
				...
				XFillRectangle(info->dpy, win, info->gc_rec[OLGX_WHITE]->gc,
								x + 2, y + 3,
								(unsigned)(Abbrev_MenuButton_Width(info) - 5),
								(unsigned)(Abbrev_MenuButton_Width(info) - 6));
				draw_window_mark(info, win, x, y, state);
			}
		}
		...
		if (state & OLGX_HORIZONTAL) {
			olgx_draw_menu_mark(info, win, x + ((Abbrev_MenuButton_Width(info)
					...
					y + ((1 + Abbrev_MenuButton_Height(info) -
									info->mm_height) >> 1),
					OLGX_HORIZ_MENU_MARK & ~OLGX_INACTIVE, 1);
		}
		else if (state & OLGX_VERTICAL) {
			olgx_draw_menu_mark(info, win, x + ((Abbrev_MenuButton_Width(info)
									- info->mm_width) >> 1),
...
void olgx_stipple_rect(Graphics_info  *info, Window win,
...
void olgx_draw_text(Graphics_info *info, Window win,
#ifdef OW_I18N
    void *string,
#else
    const char *string,
#endif /* OW_I18N */
    int x, int y, int max_width, int state)
{
	...
	if (Olgx_Flags(info) & OLGX_FONTSET) {
		if (state & OLGX_LABEL_IS_WCS) {
			...
			if (max_width && width > max_width) {
				for (i = 0; (i < len && current_width <= max_width); i++) {
					current_width +=
							XwcTextEscapement(info->textfontset, &wcs[i], 1);
				}
				len = i - 2;
		...
		if (max_width && width > max_width) {
			for (i = 0; (i < len && current_width <= max_width); i++) {
				current_width += XTextWidth(TextFont_Struct(info), &mbs[i], 1);
			}
			len = i - 2;
			if (state & OLGX_MORE_ARROW)
				more_flag = 1;
	...
	if (max_width && XTextWidth(TextFont_Struct(info),
		...
		width = XTextWidth(TextFont_Struct(info), string, (int)len);
	}
#endif /* OW_I18N */
...
#else /* OW_I18N */
	XDrawString(info->dpy, win, gc, x, y, string, (int)len);
#endif /* OW_I18N */
	...
	if (more_flag) {	/* render a more arrow at the end of the string */
		olgx_draw_menu_mark(info, win, x + width + 1, y - MenuMark_Height(info),

#ifdef BEFORE_DRA_CHANGED_IT
				OLGX_HORIZ_MENU_MARK, 1);
#else
				(state & OLGX_INVOKED) | OLGX_HORIZ_MENU_MARK, 0);
#endif
	}
}
				...
				int x, int y, int width, int height, int state)
{
	...
	XPutImage(info->dpy, win,	/* dest */
			info->gc_rec[OLGX_TEXTGC]->gc, xim, 0, 0,	/* src x,y */
			x, y,	/* dest x,y */
			(unsigned)width, (unsigned)height);
...
void olgx_draw_textscroll_button(Graphics_info  *info, Window win, int x, int y, int state)
{
	...
	if (state & OLGX_ERASE)
	    XClearArea(info->dpy, win,x, y, (unsigned)width,
                           (unsigned)height,0);
...
void olgx_draw_numscroll_button(Graphics_info  *info, Window win, int x, int y, int state)
{
	...
	if (state & OLGX_ERASE)
	    XClearArea(info->dpy, win,x, y,
                           (unsigned)NumScrollButton_Width(info), (unsigned)height,0);
...
static void olgx_draw_accel_label_internal(Graphics_info *info, Window win, int texty, int x, int y, int width, int height, void *main_label, int m_pos, void *qualifier_label, int	q_pos, int	mark_type, int	mark_pos, void *key_label, int	key_pos, int state, int centerflag)
{
	...
	if (state & OLGX_LABEL_IS_PIXMAP) {
	    ...
	    olgx_draw_pixmap_label(info, win,
				   ...
				   plabel->width ,
				   (plabel->height > height) ? height : plabel->height,
				   state);
	} else if (state & OLGX_LABEL_IS_XIMAGE) {
	    int              newy;
	    Ximlabel        *ximlabel;
	    ...
	    XCharStruct overall;
	    void *string_label;
	    Underlinelabel *ulabel, *hlabel = 0;
...
void olgx_draw_accel_label(Graphics_info *info, Window win, int x, int y,
						int width, int height, void *main_label, int m_pos,
						void *qualifier_label, int	q_pos, int	mark_type,
						int	mark_pos, void *key_label, int	key_pos,
						void *background_pixmap, int state)
{
...
void olgx_draw_accel_button(Graphics_info *info, Window win, int x, int y,
						int width, int height, void *main_label, int m_pos,
						void *qualifier_label, int	q_pos, int	mark_type,
						int	mark_pos, void *key_label, int	key_pos,
						void *background_pixmap, int state)
{
    ...
    olgx_draw_accel_label_internal(info,win,
				   y + ((height) ? 
				    ...
				    info->button_height) - info->base_off,
				   x+ButtonEndcap_Width(info),
				   y, width-2*ButtonEndcap_Width(info), height,
				   main_label,m_pos, qualifier_label,q_pos,
				   mark_type,mark_pos, key_label,key_pos,
				   state & ~OLGX_INACTIVE, 1);
    if (state & OLGX_INACTIVE) {
	olgx_stipple_rect(info, win, x, y, width, 
                          (height) ? height + 1 : Button_Height(info));
    }
...
void olgx_draw_accel_choice_item(Graphics_info *info, Window win, int x, int y,
						int width, int height, void *main_label, int m_pos,
						void *qualifier_label, int	q_pos, int	mark_type,
						int	mark_pos, void *key_label, int	key_pos,
						void *background_pixmap, int state)
...
/*     int             flag = 0; */
    /* don't want to duplicate code, so draw a choice with a null label,
       no menu mark, and not inactive (will be done later) */
    olgx_draw_choice_item(info,win, x,y,width,height,
			  NULL, state&~OLGX_MENU_MARK&~OLGX_INACTIVE);
