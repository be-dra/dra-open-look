...
int calc_add_ins(int width, short *add_ins)
{
...
void olgx_draw_box(Graphics_info *info, Window win, int x, int y,
				int width, int height, int state, Bool fill_in)
{
	...
	if (fill_in && width > 1 && height > 1) {
	    ...
	    XFillRectangle(info->dpy, win,
			   ...
			   x + 1, y + 1, (unsigned)(width - 1), (unsigned)(height - 1));
	...
	if (state & OLGX_ERASE)
	   XFillRectangle(info->dpy, win, info->three_d ?
                          ...
                          x, y, (unsigned)(width + 1), (unsigned)(height + 1));
...
void olgx_draw_choice_item(Graphics_info *info, Window win, int x, int y,
						int width, int height, void *label, int state)
{
	...
	olgx_draw_box(info, win, x, y, width, height, state, (info->three_d)?1:0);
	...
	if (state & OLGX_DEFAULT) {
		...
		XDrawRectangle(info->dpy, win, (info->three_d) ?
				...
				x + 2, y + 2, (unsigned)(width - def_decr),
				(unsigned)(height - def_decr));
	}
	/*
	 * Now place the label
	 */
...
void olgx_draw_drop_target(Graphics_info *info, Window win, void *label, int x, int y, int state)
{
     ...
     if (info->three_d) 
       ...
       XSetLineAttributes(info->dpy,info->gc_rec[OLGX_BLACK]->gc,
	   			(unsigned)info->dtarget_swidth,
	 ...
	 olgx_initialise_gcrec(info,OLGX_BUSYGC);
       ...
       XFillRectangle(info->dpy,win,info->gc_rec[OLGX_BUSYGC]->gc,
	   			x + ewidth,y+ewidth,
		      (unsigned)(width - (2 * ewidth)),
			  (unsigned)(height - (2 * ewidth)));
