...
#else
    string[0] = VERT_ABBREV_SB_UL;
    XTextExtents(info->glyphfont, string, 1, &direct, &ascent, &descent,
		 &overall);
    info->abbsb_height = overall.ascent + overall.descent;
#endif
...
Graphics_info *olgx_initialize(Display *dpy, int screen, int d_flag,
    XFontStruct *glyphfont_struct, XFontStruct *textfont_struct,
    unsigned long *pixvals, Pixmap *stipple_pixmaps)
{
    return olgx_main_initialize(dpy, screen, (unsigned)DefaultDepth(dpy,screen),
				d_flag, glyphfont_struct, textfont_struct, pixvals,
				stipple_pixmaps);
}
...
void olgx_set_glyph_font(Graphics_info *info, XFontStruct *font_info, int flag)
{
	...
	for (i = 0; i < num_colors; i++) {
		...
		info->gc_rec[i] = olgx_get_gcrec(perdispl_res_ptr, 
						 ...
						 info->gc_rec[i]->valuemask, &values);
...
#ifdef BEFORE_DRA_CHANGED_IT
#else
		if (info->gc_rec[OLGX_TEXTGC_REV] && info->gc_rec[OLGX_TEXTGC_REV]->gc)
#endif
...
#else
			if (info->gc_rec[OLGX_TEXTGC_REV] &&
				info->gc_rec[OLGX_TEXTGC_REV]->gc)
#endif
				XSetFont(info->dpy, info->gc_rec[OLGX_TEXTGC_REV]->gc,
                         		info->textfont->fid);
...
void olgx_error(const char *string)
{
...
void olgx_set_single_color(Graphics_info *info, int index,
...
{
    return ((char *)calloc((size_t)nbytes, (size_t)1));
}
...
GC_rec *olgx_get_gcrec(per_disp_res_ptr perdispl_res_ptr, Drawable drawable,
    int depth, unsigned long valuemask, XGCValues *values)
/*
...
GC_rec *olgx_gcrec_available(per_disp_res_ptr perdispl_res_ptr,
...
Pixmap olgx_get_busy_stipple(per_disp_res_ptr perdispl_res_ptr)

/* returns a standard busy  stipple pixmap .. Creates one if nothing exists  */

{
	if (!perdispl_res_ptr->busy_stipple)
		perdispl_res_ptr->busy_stipple =
			XCreatePixmapFromBitmapData(perdispl_res_ptr->dpy,
				RootWindow(perdispl_res_ptr->dpy, perdispl_res_ptr->screen),
				(char *)busy_bits, busy_width, busy_height,
...

{
	if (!perdispl_res_ptr->grey_stipple)
		perdispl_res_ptr->grey_stipple =
				XCreatePixmapFromBitmapData(perdispl_res_ptr->dpy,
				RootWindow(perdispl_res_ptr->dpy, perdispl_res_ptr->screen),
				(char *)grey_bits, grey_width, grey_height, (unsigned long)1,
				(unsigned long)0, 1);
 ...
 */
{
    ...
    cur->busy_stipple = None;
    cur->grey_stipple = None;
    cur->next = olgx_perdisp_res_list;
    olgx_perdisp_res_list = cur;
...

{
	...
	if (gcrec->ref_count > 1) {
	    ...
	    gcrec = olgx_get_gcrec(perdispl_res_ptr, info->drawable[0],
                                   (int)info->depth, gcrec->valuemask, &values);
...
void olgx_initialise_gcrec(Graphics_info  *info, int index)
{
