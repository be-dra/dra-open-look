...
void olgx_draw_slider_control(Graphics_info  *info, Window win,
...
void olgx_draw_slider(Graphics_info  *info, Window win,
...
void olgx_draw_horizontal_slider(Graphics_info  *info, Window win,
...
void olgx_update_horizontal_slider(Graphics_info  *info, Window win,
    int x, int y, int width, int old_value, int new_value, int state)
{
	...
	XClearArea(info->dpy, win, xstart, y,
		       (unsigned)xwidth, (unsigned)info->slider_height, 0);
	...
	XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc,
		       xstart, y_slider_offset, (unsigned)xwidth, (unsigned)info->se_height);
	...
	if (old_value < 4) {	/* Draw the leftendcap when necessary */
		...
		XFillRectangle(info->dpy, win, info->gc_rec[OLGX_WHITE]->gc, x,
			  y_slider_offset, (unsigned)info->se_width, (unsigned)info->se_height);
		...
		XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BG1]->gc, x,
                               y_slider_offset, (unsigned)info->se_width,
							   (unsigned)info->se_height);
	 ...
	 * erase the old slider
	 */
       XClearArea(info->dpy, win,xstart, y, (unsigned)xwidth,(unsigned)info->slider_height + 1,0);
	...
	XFillRectangle(info->dpy, win, info->three_d ?
                      ...
                      y_slider_offset + 1, (unsigned)xwidth, (unsigned)(info->se_height - 2));
	...
	if (old_value + info->slider_width > width - 4) {
		...
		XFillRectangle(info->dpy, win, info->gc_rec[OLGX_WHITE]->gc,
			 x + inside_width + info->se_width, y_slider_offset,
			       (unsigned)info->se_width, (unsigned)info->se_height);
...
void olgx_update_vertical_slider(Graphics_info  *info, Window win,
    int x, int y, int height, int old_value, int new_value, int state)
{
	...
	XClearArea(info->dpy, win, x, ystart,
		       (unsigned)info->slider_height, (unsigned)yheight,0);
	...
	XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x_slider_offset,
		       ystart, (unsigned)info->se_height, (unsigned)yheight);
	...
	if (old_value + info->slider_width > height - 4) {
		...
		XFillRectangle(info->dpy, win, info->gc_rec[OLGX_WHITE]->gc,
                               ...
                               (unsigned)info->se_height, (unsigned)info->se_width);
	...
	XClearArea(info->dpy, win, x, ystart, (unsigned)info->slider_height, (unsigned)yheight,0);
	...
	XFillRectangle(info->dpy, win, info->three_d ? info->gc_rec[OLGX_BG2]->gc :
		  info->gc_rec[OLGX_WHITE]->gc, x_slider_offset + 1, ystart,
		       (unsigned)(info->se_height - 2), (unsigned)yheight);
	...
	if (old_value < 4) {	/* Draw the endcap when necessary */
		...
		XFillRectangle(info->dpy, win, info->gc_rec[OLGX_WHITE]->gc,
                               x_slider_offset, y, (unsigned)info->se_height, 3);
...
void olgx_draw_vertical_slider(Graphics_info  *info, Window win,
    int x, int y, int height, int value, int state)
{
	...
	XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc,
		       x_slider_offset, y + value, (unsigned)info->se_height,
		       (unsigned)(height - value - info->se_width));
	...
	XFillRectangle(info->dpy, win, info->gc_rec[OLGX_WHITE]->gc,
	     x_slider_offset + 1, y + info->se_width, (unsigned)(info->se_height - 2),
		       (unsigned)value);
	...
	XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BG2]->gc,
		       ...
		       (unsigned)(info->se_height - 2), (unsigned)value);
...
void olgx_draw_gauge(Graphics_info  *info, Window win, int x, int y, int width,
	int oldval, int newval, int state)
{
    if (state & OLGX_VERTICAL) {
...
void olgx_draw_horiz_gauge(Graphics_info *info, Window win,
...
void olgx_update_horiz_gauge(Graphics_info  *info, Window win, int x, int y,
...

{
	...
	XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x + oldval,
                       y + left_offset, (unsigned)(newval - oldval), 
                       (info->gauge_height == 13) ?
		         (unsigned)info->se_height + 1 : (unsigned)info->se_height);

    else
	XFillRectangle(info->dpy, win, (info->three_d) ? 
                                         info->gc_rec[OLGX_BG2]->gc :
		                         info->gc_rec[OLGX_WHITE]->gc,
                       x + newval, y + left_offset, (unsigned)(oldval - newval),
                       (info->gauge_height == 13) ?
		         (unsigned)info->se_height + 1 : (unsigned)info->se_height);
...
void olgx_draw_vertical_gauge(Graphics_info *info, Window win, int x, int y,
								int width, int value)
{
	...
	XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BG2]->gc, x + 1,
                       y + info->gauge_width, (unsigned)info->gauge_height - 2,
					   (unsigned)inside_width);
    ...
    XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x + left_offset,
	       y + value, (info->gauge_height == 13) ? (unsigned)info->se_height + 1 :
	 (unsigned)info->se_height,
	 		(unsigned)(width - value - left_offset - info->se_width - 1));
...
void olgx_update_vertical_gauge(Graphics_info *info, Window win, int x, int y,
							int width, int oldval, int newval)
{
    int left_offset;
