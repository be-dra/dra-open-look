...
void olgx_draw_resize_corner(Graphics_info *info, Window win,
...
void olgx_draw_pushpin(Graphics_info *info, Window win, int x, int y, int type)
{
	...
	if (type & OLGX_ERASE)
		/*
		 * 29 and 17 is a hack,max width is 29 for all point sizes and 17 is
...
void olgx_draw_check_box(Graphics_info *info, Window win, int x, int y, int state)
{
    ...
    if (!(state & OLGX_CHECKED)) {
        XClearArea(info->dpy,win,x,y,
				(unsigned)CheckBox_Width(info),
				(unsigned)CheckBox_Height(info),0);
    }
...
void olgx_draw_text_ledge(Graphics_info *info, Window win,
